...
- 客户档案：维护客户编码、客户名称、联系人、电话、账期、负责人和状态。
- 询价报价：录入客户、产品、数量、单价、有效期和负责人，生成 `QT-日期-流水号`。
- 报价转单：客户确认后可把报价单一键转为销售订单，并回写报价状态，避免重复录入。
...
- 库存盘点调整：仓库录入实盘数量后，系统自动计算差异，更新库存余额，并生成盘点调整记录与库存流水。
...
- 供应商档案：维护供应商编码、供应商名称、联系人、电话、账期、等级和负责人。
...
- 应收结算：从销售订单生成应收单，记录客户、来源单号、金额、到期日和未结余额。
- 应付结算：从采购单生成应付单，可按采购明细金额或手工金额形成供应商应付。
- 收付款登记：按应收登记收款，按应付登记付款，自动更新“未结清、部分结清、已结清”状态。
- 财务概要：统计应收总额、已收款、应收余额、应付总额、已付款、应付余额和现金净流入。
...
- 默认角色：系统管理员、经营负责人、采购、工程、生产、仓库、品质、销售、财务。
- 写入权限：采购、工程、生产、仓库、品质、财务各自只能提交对应业务动作；管理员拥有系统运维权限。
...
- `chenyida_erp_app/`：已开始开发的本地 Web 应用，当前覆盖物料主数据治理、产品工程卡、BOM 管理、齐套检查、缺料采购建议、采购单、采购收货入库、生产工单、工单领料、完工入库、IQC/IPQC/FQC 品质检验、询价报价、销售订单和成品出货。
