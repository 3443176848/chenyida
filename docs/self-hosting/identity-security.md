...
Planning Handoff 及同模块的保存解析、生成交接包、提交、退回后修订/重提统一使用同一共享客户端。客户端在每次发送时从当前 `CYD_ERP_CSRF` Cookie 读取双提交值，发送 `X-CSRF-Token`、`POST` 和 `credentials: same-origin`；不把页面初始 `/api/session` 快照作为长期权威。页内 Idempotency-Key 只绑定当前 CSRF Session 标识、method/path 和 canonical 正文，仅在结果未知且正文/会话未变时重放；Cookie 变更、logout、认证失效、pagehide 或历史恢复会清空旧上下文。缺失/错误/旧 Token 在客户端或服务端 fail closed，不因便利性放宽 Origin/CSRF。
