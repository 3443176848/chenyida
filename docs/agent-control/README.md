...
`native_mvp.py`只读取`--bundle`明确指定的单个常规JSON文件，拒绝symlink、hardlink、重复JSON key、缺字段和未知字段。它校验Task Packet、候选链、角色专属Context Manifest、Message、checkpoint、Minority Report、`RESULT_UNKNOWN`恢复和最终候选门禁，并向stdout输出确定性JSON；退出码`0`为`PASS`，`2`为失败关闭。它没有输出文件、Git、subprocess、网络或数据库能力。
...
`pilots/AGENT-R1-5/valid-bundle.json`是完全合成的试点：candidate v1被ERP/对抗/安全/QA拒绝，candidate v2修复后由新Context重新签核；旧候选PASS不复用，Minority Report有证据化处置，`RESULT_UNKNOWN`先对账再重试。`blackbox/`只含公开合成接口、按风险选择的Persona、预期、确定性harness和观察报告。
