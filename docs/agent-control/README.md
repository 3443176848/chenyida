...
活动Packet位于`task-packets/<TASK-ID>.json`。v2额外固定D-113/D-114、唯一产品写者、六个不复用角色、四项独立门禁、危险能力拒绝和低资源上限。JSON Schema定义严格结构，并直接约束每个角色唯一出现一次及其profile、visibility和写权限；Python标准库验证器继续强制JSON Schema不能表达的跨数组`agent_id`唯一性及`product_writer_agent_id == CHANGE_BUILDER.agent_id`关系，并实现摘要、身份、候选、lease/revision与恢复语义。两层验证都拒绝未知字段；Task Packet进入执行前必须同时通过，不能把Schema-only通过视为可执行。

`pilots/AGENT-R1-5/valid-bundle.json`是完全合成的`chenyida-erp-native-pilot-bundle/v2`试点：candidate v1被ERP/对抗/安全/QA拒绝，candidate v2修复后由新Context重新签核；旧候选PASS不复用，Minority Report有证据化处置，`RESULT_UNKNOWN`先对账再重试。v2内每个Context/Message locator都必须存在于严格artifact注册表，摘要按规范JSON payload重算；Context文档集合再按角色和candidate白名单匹配，Message的`input.artifacts`必须与Evidence locator相等，测试必须绑定`TEST_REPORT`或公开黑盒观察及同一exit code。`BLACK_BOX_OBSERVATION`只允许源盲黑盒角色使用；QA/黑盒的`PASS`、`FAIL`、`RESULT_UNKNOWN`和`VETOED`还必须与测试集合的聚合结果一致，任何`PASS/COMPLETE`消息携带的测试也只能全部为`PASS`。关闭Minority Report的最终对抗`PASS/VERIFICATION`必须至少含一项唯一绑定的`TEST_REPORT`，且全部测试为`PASS`、exit code为`0`；空测试、`FAIL`、`NOT_RUN`或`RESULT_UNKNOWN`均不能处置claim。所有测试类Evidence必须被唯一测试引用。未引用、伪造、跨角色、状态矛盾或摘要不一致的artifact全部失败关闭。`blackbox/`只含公开合成接口、按风险选择的Persona、预期、确定性harness和观察报告。
