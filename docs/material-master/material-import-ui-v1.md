...
### Implementation Gate: BLOCKED_BY_MAPPING_TARGET_CATALOG

当前 `/mapping` 不返回完整动态目标 catalog，也无独立权威 metadata 读接口。完整 `MaterialImportTargetSelector`、动态属性 Mapping、metadata 恢复和完整确认流实施前，必须另行设计、批准、实现并测试最小只读 catalog API，然后更新 OpenAPI。

未来接口至少安全返回 target_namespace、target_code、display_name、value_type、unit_policy、enabled/selectable、必填/约束摘要、metadata_digest；动态属性来自运行时数据库 metadata。它必须认证，以 `material.import.map` 或批准的只读 capability 授权，返回有界分组，不暴露 SQL、列名、内部 ID 或禁用目标。禁止 seed、前端硬编码、测试数据枚举、attribute_id、历史 Mapping 反推或只实现基础字段冒充完整 UI。
...
| 11 动态目标选择 | 硬编码 / 平铺 / 可搜索分组 catalog | **Recommended: 可搜索分组 Catalog**；动态、稳定、可核对。**Implementation Gate: BLOCKED（BLOCKED_BY_MAPPING_TARGET_CATALOG）** | 必须后续新增最小只读 catalog 并更新 OpenAPI | catalog digest 绑定编辑/preview | 认证授权、不泄露内部 ID | 可见 label、键盘搜索/分组 | 有界分组 | 高 | PROPOSED |
