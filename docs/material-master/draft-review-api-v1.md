...
本规格只完成书面设计。规格确认前不得修改 API、Schema、migration、业务服务或测试代码，不得连接生产 D1 或部署生产。
...
为保证 Material 的错误方法也进入专用 responder，实施时为 catch-all 显式增加 `PUT`、`PATCH`、`DELETE`、`HEAD`、`OPTIONS` 转发。Material namespace 对 `PUT/PATCH/DELETE/OPTIONS` 返回本规格嵌套 405；`HEAD` 按 HTTP 语义返回无正文 405，并通过 `X-Request-Id` 与 `X-Error-Code: METHOD_NOT_ALLOWED` 暴露稳定结果。legacy namespace 保持现有扁平 405，不执行任何业务写入。
...
| --- | ---: | ---: | ---: | ---: | --- |
| `admin` | ✓ | ✓ | 拟议 ✓ | 拟议 ✓ | 审核权限待人工确认 |
| `manager` | ✓ | ✓ | 拟议 ✓ | 拟议 ✓ | 审核权限待人工确认 |
...
`material.read` 映射所有现有角色，`material.draft.create` 映射当前已经拥有粗粒度 `material` 的角色。现有系统没有数据管理员角色，也没有审核权限等价项；批准/驳回矩阵只是推荐基线，未获确认前不得实现为正式规则。
...
- `attempt_count`：所有物理写尝试，`0..120`。
- `new_key_count`：已获得预留资格的新 Key，`0..30`。
...
1. 每个通过 CSRF 和有界 JSON/Key 解析的写请求生成 attempt Token 摘要，以单条条件 Upsert 在 `attempt_count < 120` 时递增并写 `last_attempt_token_digest`。未获得物理槽位时不读取或写入逐 Key 幂等数据。
...
4. 并发同 Key 的唯一冲突使整个新 Key batch 回滚，随后读取既有幂等记录；不同 Key 的第 30 个获得槽位，第 31 个没有槽位。
...
两次 Upsert 都必须通过 Token 子查询或结果元数据确认获得槽位，禁止应用层 `COUNT` 后单独写。速率桶按 `(bucket_start)` 建清理索引，只保留 24 小时并每次有界删除 100 条。每用户每分钟至多一行，因此重复或不同 Key 洪泛不会导致无界表增长。每分钟 120 次物理尝试和 30 个新 Key 的阈值属于本规格拟议值，需在规格确认时一并批准。
...
| 既有宽泛 `MATERIAL_ATTRIBUTE_STORAGE_INVALID` | 在完成下述服务 code 拆分前一律 fail closed 为 `INTERNAL_ERROR`，禁止按中文 message 判断 |
...
审计日志保留期当前没有已确认规则，应列为业务/合规决策，不能与 24 小时幂等清理混为一谈。
...
30. 并发第 30/31 个新 Key和第 120/121 个物理尝试严格执行条件槽位；大量重复或不同 Key 的 429 只增长一个用户/分钟桶和一条聚合审计，不创建逐 Key 幂等行。
...
以下事项没有现有 `ACCEPTED` 决策，规格确认时必须明确；未确认项不得写入正式业务规则：

1. **审核角色**：是否接受 V1 暂由 `admin`、`manager` 获得批准/驳回权限；当前没有数据管理员角色，B04 尚未确认。
2. **创建人自审**：是否禁止 `created_by == reviewer`。推荐禁止所有角色自审；是否为 `admin` 设置受审计的紧急例外也需明确。
3. **多角色审核**：B11 尚未确认。V1 API 只开放现有单步最终审核服务，不代表工程、品质、仓库和数据管理员多节点流程已完成；是否接受此阶段边界。
4. **审核驳回权限是否相同**：批准与驳回是否使用同一角色集合；本规格拟议相同。
5. **幂等保留期**：是否接受已完成记录保留 24 小时，`PENDING` 只能租约接管、不得直接清除。
6. **写速率**：是否接受每用户每分钟 120 次物理写尝试、其中最多 30 个新 Key。
7. **API 审计保留期与可见角色**：当前尚无已确认规则；不得随幂等记录在 24 小时后删除。
8. **人工创建来源**：是否确认 V1 公共创建 API 只允许 `MANUAL`，其他来源继续等待独立适配器。
