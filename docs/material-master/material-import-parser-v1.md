...
本任务只交付书面规格、数据模型补充、OpenAPI 草案和流程图。以下内容均不在本任务实施：

- Parser、OOXML 适配器、CSV 解析器或 Repository 代码；
- `0005` migration、Drizzle schema、Queue、Cron、R2 binding 或部署配置；
- 前端、清洗、AI、自动分类、候选匹配、Material Draft 或正式物料写入；
- 生产 D1/R2、生产数据、迁移、部署或权限变更。
...
| 方案 | 内容 | 优点 | 风险 | 结论 |
| --- | --- | --- | --- | --- |
| A | `zip.js` + `sax-wasm` + 受限 OOXML 适配层；CSV 使用 `csv-parse` browser ESM | Web Streams、有界读取、保留原始类型、可按 Sheet 恢复 | 不是完整 Excel 引擎；WASM、Bundle、Workers 行为尚未验证 | `Recommended / PROPOSED` |
...
下一阶段发送失败不得丢失已经完成的解析阶段。生产调度器推荐 Cloudflare Queue，仍是 `PROPOSED` 且需独立基础设施授权。Queue 至少一次投递要求见 [Cloudflare Queues delivery guarantees](https://developers.cloudflare.com/queues/reference/delivery-guarantees/)。

### 5.3 Queue 与租约建议

- Parser consumer `max_batch_size = 1`，`PROPOSED`；
...
- 支持 UTF-8、UTF-8 BOM 和 GB18030，均为 `PROPOSED`；严格 UTF-8 失败后才尝试 GB18030，不进行静默替换解码。
...
全部数值为 `PROPOSED`，必须同时生效；最终值需使用脱敏现场样本、并发批次数、30 天原始行保留期和 D1 容量/成本压测确认。
...
由于当前运行时从未写入 `material_import_rows`，实施 migration 仍必须先做只读计数。若发现既有行，升级必须 fail closed 并输出人工处置报告，不得伪造 legacy `parse_run` 或静默丢行。已有 `FILE_READY` 批次原样保留，可在新 API 中开始解析。
...
- `0005` 空库升级、已有 `FILE_READY`、意外旧行拒绝、重复升级、失败回滚、Down 数据保护、外键与唯一约束；
...
下列决定虽已获准写入规格，仍未获得实施或生产批准。

| # | 决定 | 可选方案 | 推荐方案与理由 | Schema/API 影响 | 安全/性能/成本影响 | Status |
| ---: | --- | --- | --- | --- | --- | --- |
| 1 | 同步或异步 | 上传内同步；HTTP 后台；持久任务 | 持久异步任务，避免请求生命周期承载解析 | 状态、run、Outbox、202 API | 至少一次；增加调度运维 | PROPOSED |
| 2 | parse runs | 复用 batch；独立 run | 独立 `parse_runs`，隔离任务和业务状态 | 新表/FK/租约字段 | 可审计重试，增加存储 | PROPOSED |
| 3 | XLSX 库 | 组合流式；SheetJS；外部服务 | 方案 A，需兼容门禁 | 无直接 API；实现模块化 | 有界内存但适配复杂 | PROPOSED |
| 4 | CSV 库 | csv-parse；自研；其他库 | `csv-parse` browser ESM，需 Workers 测试 | 解析器依赖 | 避免自研状态机风险 | PROPOSED |
| 5 | CSV 编码 | 仅 UTF-8；UTF-8+GB18030 | 后者，满足现场历史文件 | 错误码和 encoding 元数据 | decoder 必须严格且有界 | PROPOSED |
| 6 | 隐藏 Sheet | 全解析；显式选择；跳过业务行 | 保存元数据但跳过业务行和 Mapping | Sheet 可见性/汇总 | 降低隐蔽数据误导入 | PROPOSED |
| 7 | Sheet/header 建议 | 自动事实；suggestion；全人工 | 有界 suggestion，人工确认 | 建议表与 Mapping draft | 不把启发式变业务事实 | PROPOSED |
| 8 | 资源上限 | 平台硬限；应用组合限 | 本规格组合限，样本压测后确认 | 统计/失败字段 | 提前止损；需容量规划 | PROPOSED |
| 9 | 行重放版本 | 删除重写；run 隔离；临时复制 | `parse_run_id` 隔离 + current pointer 原子发布 | 重建 rows 唯一约束 | 幂等恢复，历史占用增加 | PROPOSED |
| 10 | Mapping 模型 | 单 JSON；主从关系表 | 主表+item 表 | 新表、版本/CAS API | 防止任意字段/表达式 | PROPOSED |
| 11 | 分类语义 | category_id；code；hint | 仅 `category_hint` | 目标 catalog | 不自动分类或污染正式 FK | PROPOSED |
| 12 | 确认后编辑 | 原地编辑；新版本；禁止一切 | 不原地编辑；新版本需重新确认 | Mapping 状态/版本 | 审计完整，增加版本行 | PROPOSED |
| 13 | 权限 | 角色硬编码；capability | parse/map/read/read_any capability | Session 授权 | 最小权限；需角色映射审批 | PROPOSED |
| 14 | Queue | 同步；Cron polling；Queue+Outbox | Queue+Outbox；基础设施另批 | Outbox/调度器 | 可靠投递，增加 Queue 成本 | PROPOSED |
| 15 | Shared Strings | D1；R2 索引；全内存 | D1 run 级分块 + 有界预取，压测失败再评估 R2 | 暂存表/清理 | 避免逐 cell 查询和常驻内存 | PROPOSED |
| 16 | 恢复粒度 | 行游标；Sheet；整本 | Sheet 为恢复边界，行 checkpoint 仅观测 | Sheet 完成状态 | 重跑有限、语义可信 | PROPOSED |

## 21. 停止条件

本规格提交后，`PHASE2-TASK03` 文档任务停止并等待“规格确认”。不得自动创建 `0005`、安装解析依赖、实现 Parser/Mapping API、创建 Queue/R2/Cron、执行 migration、连接生产或开始后续清洗/匹配任务。
