...
## 25. 2026-07-30 运行面决策上下文补充

`SELFHOST-OPS-UAT-MATERIAL-REVIEW-BLOCKERS-03-RETRY` 在 alpha.34/0034 兼容 Web 补齐已实施的只读审核上下文，不改变本文件的审核写合同：

- 详情原样显示完整“待审名称”，不得按 `·` 拆分或推断外部编号；分类、单位、来源类型、创建人、当前 SUBMIT 提交人/时间、版本和状态一并展示。
- 工程说明取当前版本既有 `SUBMIT.change_reason`；空值明确显示“未保存”，不新增字段或伪造内容。
- 页面固定说明本次只审核主数据身份、名称、分类、单位和工程说明；供应商映射、采购报价与价格由对应模块独立治理。
- 批准后果、退回后果、正式编码尚未生成，以及批准后由工程继续建立 BOM 的下一步必须可见。
- 页面不显示伪造的供应商、价格、采购报价或 UAT 外部标识，operations 仍无正文编辑控件。

真实 operations 只读验收中 Dashboard 与队列均为 4，533—536 的详情 4/4 可见、正文编辑控件 0、approve/reject 请求 0；四条记录保持 PENDING_REVIEW/V2/MANUAL/PCS/空编码。
