...
状态：`PROPOSED_AWAITING_CONFIRMATION`
...
本规格只定义后续实现契约，不修改 Schema、migration、API、服务代码、页面或生产环境。规格确认前不得开始实施。
...
收到“规格确认”及上述选择前，不得修改 Schema、migration、API、Material Service 或业务代码。
