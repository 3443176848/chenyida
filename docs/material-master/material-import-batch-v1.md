...
本规格定义物料历史文件批量导入的 V1 基础：批次、单个原始文件、不可变事件、后续解析所需的原始行契约，以及围绕私有对象存储的可恢复上传流程。该基础已由 `PHASE2-TASK02` 实现，并为后续 `PHASE2-TASK03` 的 Excel/CSV 行解析提供稳定边界。

### 1.1 V1 范围

- 创建一个导入批次。
- 每个批次接收一个 `.xlsx`、`.xls` 或 `.csv` 文件；`.xls` 在服务端进入独立 OLE/BIFF 解析路径，批次来源分类继续兼容 V1 的 `XLSX`。
...
`material_import_rows` 的结构在本规格中冻结为 `PHASE2-TASK03` 的写入契约；`PHASE2-TASK02` 本身不产生行记录。
...
本节已获批准并由 `0004_material_import_batch_foundation.sql` 实现；仍不得据此执行生产 Migration。
...
| `total_rows` | INTEGER NOT NULL DEFAULT 0 CHECK >= 0 | TASK03 后续写入 |
...
此表将由 `PHASE2-TASK03` 写入，本规格只冻结边界。
...
原始行一经写入不得由映射、清洗或审核步骤覆盖；后续结果必须存入独立结构。字符串中的 HTML、脚本或公式前缀只作为数据保存，绝不能解释为可执行内容。TASK03 可以添加向后兼容字段，但不得破坏上述类型边界。
...
| `FILE_READY` | 文件满足所有 ready 不变量，可交给后续解析任务 | 取消；未来 TASK03 扩展 |
...
- 实现使用有界 multipart 流式解析、增量 SHA-256 和对象存储流式写入；共享失败传播会中止上传，且禁止整文件 `arrayBuffer()`。
...
- 匹配时将安全检查状态设为 `PENDING`，批次保持 `UPLOAD_PENDING`，写 `FILE_STORED`；基础检查通过后再进入 `FILE_READY`。
...
V1 应用上限批准为 **10 MiB**。仓库中没有 PCB/FPC/SMT 历史物料文件样本分布或峰值证据，因此该值是保守起点，不是已验证业务容量；后续仍应以脱敏样本统计大小、行数、工作表数和压缩比，并按独立决定调整。
...
- 后续 TASK03 的解析内存、CPU、行数和公式/共享字符串成本尚未压测。
...
- CSV 可拒绝 NUL 字节、明显完整 HTML 文档、错误二进制签名和不在获批编码策略中的内容。V1 支持 UTF-8（含 BOM）与 GB18030；仍需在 TASK03 行解析前用脱敏样本验证。
- 不执行公式、宏、外部链接、HTML、脚本或嵌入对象。

CSV 单元格包含 `<script>`、`<html>`、`=`、`+`、`-`、`@` 不能作为整文件拒绝理由；它们可能是合法业务文本。TASK03 应把公式样文本保持为数据，未来导出还必须单独防范 CSV/Spreadsheet Formula Injection。
...
所有列表过滤、详情、事件和重试来源检查都必须在 SQL/服务端查询层施加所有者条件，不得先读出再在浏览器隐藏。隐藏资源统一返回 404。初始角色映射中 admin/manager 拥有 `read_any`，purchase/engineering 仅可读取本人批次。
...
原始 R2 对象和原始行从 `terminal_at` 起保留 30 天；批次元数据与事件从 `terminal_at` 起保留 1095 天。两项均已批准；生产生命周期或定时清理仍需独立授权。
...
`PHASE2-TASK02` 已创建 `0004_material_import_batch_foundation.sql`、Drizzle 快照与带数据保护的 Down 文件；未对生产 D1 执行迁移。
...
| `rows(batch_id, sheet_index, row_number)` | TASK03 原始行顺序读取 | 批次过滤 + 工作表/行排序 | 已由唯一约束提供索引时不重复创建 |
...
以下 12 项均已由项目负责人批准，并以 `Status: APPROVED` 记录。
...
- 推荐方案：A，V1 每批次恰好一个 `.xlsx`、`.xls` 或 `.csv` 文件。
...
- 推荐理由：仓库没有历史文件大小证据；需控制 128 MB isolate 内存风险、代理上传超时、XLSX 压缩炸弹和 TASK03 解析成本。平台请求体上限不是业务容量。
...
- Status: APPROVED
- 可选方案：A. 仅 `CREATED`；B. `CREATED`、`UPLOAD_PENDING`、`FILE_READY`；C. 任意非终态；D. 包括已终态。
...
12 项决定已批准，`PHASE2-TASK02` 的非生产实现已完成。任务在提交后停止：不得创建生产 R2 bucket/binding/Cron，不得连接或迁移生产 D1/R2，不得部署；Excel/CSV 行解析、`material_import_rows` 写入、映射、匹配和 Material Draft 创建必须由后续独立任务授权。
