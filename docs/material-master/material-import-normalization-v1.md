...
> 任务：`PHASE3-TASK02`
> 状态：`APPROVED / IMPLEMENTED (NON-PRODUCTION)`；16 项决定已全部批准
> 范围：运行时、`0006`、五个 API、隔离测试与治理文档；不含生产迁移、部署、分类、匹配或 Material Draft 创建
...
| --- | --- |
| `id` | INTEGER PK |
| `batch_id` | 由 insert/update trigger 强制校验 batches 存在，语义等价 RESTRICT；见 6.6 |
| `parse_run_id` | FK parse_runs, RESTRICT；永久绑定 |
| `mapping_id` | FK mappings, RESTRICT；永久绑定 |
| `mapping_version` | 正整数快照 |
| `mapping_digest` | 64 位小写 hex；对 run 冻结的 Mapping item canonical 内容摘要 |
...
`material_import_batches.current_normalization_run_id` 可空并由 trigger 强制指向同批次 `SUCCEEDED` run，只有发布成功时切换。处理中、失败或取消不得清空或覆盖旧 current pointer。
...
正式设计原拟让 normalization run/row 的 `batch_id` 直接外键引用 `material_import_batches`。`0006` 需要在 D1 开启外键时重建 batches 及其既有直接子表；为避免形成父表重建循环，实际以 insert/update trigger 执行同等的存在性和跨批次一致性校验，并由受保护 Down、迁移测试和 Drizzle snapshot 固化。`parse_run_id`、`mapping_id`、`requested_by`、row-to-run、row-to-parse、issue-to-run/row 仍使用数据库外键且不使用 cascade。该差异不放宽引用完整性，也不授权生产迁移；未来若 D1 迁移基线允许无循环的物理 batch FK，应通过新版本迁移收敛，禁止修改已执行的 `0006`。
...
- payload 同时固化 `normalization_run_id`、`mapping_digest`、行状态与 issue 汇总；关系列仍是分页和约束权威。
...
取消复用既有 `POST /api/material-master/import-batches/{batchId}/cancel`；`0006` 与运行时已扩展其状态/CAS/Normalization run 协作语义，不新增第二个取消路由。
...
新增能力 `material.import.normalize`。它不由 `read_any` 自动推导；V1 在集中权限矩阵中显式授予 `admin/manager`，`purchase/engineering` 不获得该能力，浏览器不按角色名裁决。
...
实施验收结果：`54/54 PASS`。1–48 由 `material-import-normalization.test.mjs` 的纯 normalizer、服务、调度器、API、安全与资源边界断言覆盖；49 由同文件 OpenAPI 断言及仓库级 OpenAPI 校验覆盖；50–51 由 `material-import-normalization-migration.test.mjs` 的一次性 Miniflare D1 Up/受保护 Down/重升/失败回滚覆盖；52 由内存 scheduler/outbox drain 与重复投递覆盖；53 复用仓库环境守卫/生产 URL 拒绝回归；54 由本任务完整 `npm test` 回归覆盖。一个顶层测试可包含多个独立矩阵断言，但不得以静态清单替代行为验证。
...
## 21. 已批准决定（全部 `Status: APPROVED`）

以下每项均待项目负责人确认；未确认前不得实施。

### 决定 1：批次状态名称

- Status: APPROVED（PHASE3-TASK02）
...
`PHASE3-TASK02` 在运行时、`0006`、五个 API、隔离 Miniflare/D1 测试和项目治理文档完成并形成独立提交后结束。没有创建 Material Draft 或正式物料，没有修改 R2/Queue/hosting 生产 binding，没有连接、迁移或部署生产环境。
