...
任务：`PHASE1-TASK10`（设计）、`PHASE1-TASK12`（实施）

日期：2026-07-14（设计），2026-07-15（实施）

目标运行面：`chenyida_erp_site/` 在线 Site

实施状态：PHASE1-TASK12 已按本规格完成非生产前端、测试和隔离浏览器验收；未修改 API、Schema、Migration、业务服务或生产环境。
...
- `last_rejection` 是正式前端实施前置的最小只读 API 兼容项；PHASE1-TASK11 已实现，PHASE1-TASK12 已消费并展示。
...
3. 项目负责人已另行指定 PHASE1-TASK12 实施前端。
4. PHASE1-TASK12 已按本规格执行隔离测试和人工验收。
5. 任何生产 migration 或部署仍需单独明确授权。

PHASE1-TASK11 解除了 `last_rejection` API 前置缺口，PHASE1-TASK12 随后完成非生产前端。该实施不构成生产迁移、发布或公开站点可用授权。
...
以上是 PHASE1-TASK10 文档阶段回归结果。`last_rejection` API 后续已由 PHASE1-TASK11 实施，页面由 PHASE1-TASK12 在非生产开发代码中实施；任何生产功能仍未迁移或部署。

## 26. PHASE1-TASK12 实施记录

### 26.1 实际组件与路由

- `app/materials/new/page.tsx`：创建入口。
- `app/materials/[materialId]/edit/page.tsx`：DRAFT 编辑入口。
- `app/materials/_components/material-draft-page.tsx`：创建/编辑共用控制器、布局 C、写状态机、对话框、Validation 和冲突界面。
- `app/materials/_lib/material-draft.ts`：严格序列化、dirty 规范化、权限能力、未知属性、幂等操作和 Retry-After 纯函数。
- `public/erp/api-client.js`：唯一共享浏览器 Client 对受保护 Material 写请求 fail-closed 注入显式 Idempotency-Key 与 CSRF，不创建第二个 Key。
- 列表与详情工作区只新增 permission-driven 创建/编辑入口；未增加审核队列、批准、驳回、导入或 AI 操作。

### 26.2 实际写流程

- 创建：本地格式检查 → `POST /drafts` → 程序内进入编辑页 → 统一详情 GET 加载完整聚合。
- 编辑保存：严格构造完整可编辑聚合 → `PATCH /drafts/:id` → 统一详情 GET → 更新表单快照与 `expected_version`。
- 保存并提交：PATCH（有 dirty 时）→ GET → ERROR 阻断 / WARNING 绑定版本确认 → `POST /submit` → 只读详情。
- 网络结果不确定时只保留原 Key、method、endpoint 和不可变 payload；禁止启动第二个写操作。409 不自动覆盖，422 保留输入并重新加载当前 Schema，429 读取 Retry-After。
- 成功程序内跳转显式绕过 beforeunload；真实未保存修改、RESULT_UNKNOWN 和页面外导航仍受保护。

### 26.3 实际验证结果

- `npm run lint`：PASS，0 error；保留 1 个任务外既有未使用变量 warning。
- `npm test`：PASS，build 成功，全量 Node 158/158；其中 Draft UI 新增 54/54，Material 只读 UI 37/37 继续通过。
- `npm run test:api`：PASS，一次性本地隔离 D1 返回 `ok: true`。
- `npm run security:credentials`：PASS，224 个仓库文件通过凭证扫描。
- 临时 SQLite：环境守卫 4/4、`SELF_TEST_OK`、`SMOKE_TEST_OK`、`BACKUP_RESTORE_TEST_OK`、`GO_LIVE_CHECK_OK`；临时目录已清理。
- 隔离浏览器：创建 POST、编辑 PATCH、详情 GET、submit 全链路通过并进入 `PENDING_REVIEW`；`0`、`false`、单位和五种属性正确保留；dirty 离开确认、程序内成功跳转、只读 source_ref 和权限入口通过。
- 1366、1280、1024、768 宽度均无页面横向溢出；动态属性三列按断点降为两列/一列，sticky 操作区不覆盖最终字段。
- `git diff --check` 与最终敏感信息/差异范围在提交前复核；未连接生产 URL/D1，未迁移真实数据或部署。

### 26.4 与规格差异和已知限制

- 统一详情契约当前没有历史 `schema_version`。V1 只能使用当前叶子 Schema、未知 attribute code 保护和服务端 422 重新加载进行 fail-closed；不自动迁移旧属性，也不推断旧 Schema。
- 规格中的 54 项验收以可执行 Node 契约/领域测试覆盖，关键创建、编辑、提交和响应式行为另用真实隔离浏览器验证；没有引入新的浏览器测试框架或生产环境依赖。
- 本任务未新增 validate-only API，服务端仍是业务 Validation、权限、状态、版本和幂等的最终权威。
