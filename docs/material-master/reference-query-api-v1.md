...
- `last_rejection`：最近一次完整驳回历史的 `{version, reason, reviewed_by, reviewed_at}`；从未驳回时为 `null`。
...
`/materials/:materialId` 与 `/drafts/:materialId` 复用同一个 Material Query Service 投影，不维护第二套查询逻辑。规范来源是不可变 `material_versions`：当前 REJECT 写事务在同一行完整保存 `event_type=REJECT`、`version_no`、`change_reason`、`reviewed_by` 和 `reviewed_at`，不需要关联 `material_change_logs`。
...
- 选择顺序不依赖数据库默认行序；先按版本降序，再按审核时间和稳定事件主键降序。
- 投影读取完整不可变历史，与 `history_summary` 的最近 5 条窗口无关；后续编辑、重新提交或批准不会删除旧驳回。
- `reason` 原样作为 JSON 纯文本返回，长度继续遵守 `1..1000`，客户端不得解释为 HTML。
- `reviewed_by` 只返回现有安全账号标识；`reviewed_at` 使用统一 ISO 8601 API 时间格式。
- 任一已选 REJECT 行缺少合法版本、非空原因、审核人或有效审核时间时 fail-closed，返回脱敏 `500 INTERNAL_ERROR` 和当前 `request_id`，不返回部分详情或内部历史结构。
- 投影只在详情通过行级可见性后执行；不可见对象仍统一返回 `404 MATERIAL_NOT_FOUND`，不泄露是否存在驳回历史。
...
- 最近 REJECT 业务历史不完整或损坏时 fail-closed 为 `500 INTERNAL_ERROR`，并由服务端审计记录当前 `request_id`。
...
- `last_rejection` 仅在单条详情中以 `event_type='REJECT'`、确定性 `ORDER BY` 和 `LIMIT 1` 查询；列表不执行该查询，不产生 N+1，也不把全部历史加载到应用内存。
...
37. 从未驳回、单次/多次驳回、摘要窗口外驳回、驳回后编辑/重提/批准均返回正确 `last_rejection`。
38. `/materials` 与 `/drafts` 的投影一致，drafts 状态限制和两类详情的隐藏 404 行为保持不变。
39. `reason` 以纯文本原样返回，损坏/不完整历史 fail-closed，且版本/日志分页和最近 5 条摘要保持原契约。
40. 投影 SQL 明确包含 `ORDER BY version_no DESC, reviewed_at DESC, id DESC LIMIT 1`，并记录隔离 D1 的查询计划。
...
### 21.1 PHASE1-TASK11 最近驳回投影补充实施

2026-07-15 在不修改 Schema、migration、索引或驳回写服务的前提下，为统一详情增加 `last_rejection`。两个详情入口继续共用 Material Query Service；投影只查询 `material_versions` 的完整 REJECT 历史，并按 `version_no DESC, reviewed_at DESC, id DESC LIMIT 1` 选择。

隔离 Miniflare D1 的 `EXPLAIN QUERY PLAN` 使用现有唯一索引 `material_versions_material_version_uq (material_id, version_no)` 搜索单物料版本历史；未新增索引。实现与测试确认列表无该查询、详情最近 5 条摘要和两个历史分页保持不变、损坏历史返回带 `request_id` 的脱敏 `INTERNAL_ERROR`。本补充未连接、迁移或部署生产 D1。
