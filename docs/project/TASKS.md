...
| 任务编号 | 任务名称 | 状态 | 负责人 | 开始时间 | 完成时间 | 依赖任务 | 说明 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| PHASE1-TASK07 | 完善物料草稿生命周期、重新提交和审核队列 | DOING | Codex（设计）、项目负责人（规格与九项设计选择确认） | 2026-07-14 | - | PHASE1-TASK06 | 第一阶段书面规格已写入 `docs/material-master/draft-lifecycle-v1.md`；当前等待“规格确认”，未经确认不得修改 Schema、migration、API 或业务代码 |

当前唯一 `DOING` 任务为 `PHASE1-TASK07`，且只完成第一阶段书面设计。范围冻结为草稿编辑、提交/重新提交、`PENDING_REVIEW` 单步审核、创建人与最后实质修改人职责分离、审核队列、现有安全/幂等复用和版本化 migration 方案；禁止页面、导入、AI、批量审核、下游业务、legacy SQLite、生产 D1 和部署。`PHASE0-TASK03` 仍在台账中等待后续安排。
