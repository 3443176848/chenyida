...
| PHASE3-MATERIAL-LIBRARY-SERVER-RUNTIME | 服务器本地运行面与 18888 端口切换 | DONE | Codex（开发服务器配置与验证） | 2026-07-18 | 2026-07-18 | PHASE3-MATERIAL-LIBRARY-EXCEL-COMPAT | 根据 D-028/D-030 将 Python 应用作为后续默认交付目标，统一端口为 `18888`；systemd 服务已 `enabled/active`，本机及公网健康检查通过。Site `.xls` 代码尚未迁移到本地运行面。 |
