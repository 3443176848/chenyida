...
- 内容绑定/定向验证：operator policy文件/逻辑摘要为`e92f6d21…df74`/`b2e5cc0f…1f65`，catalog文件/artifact为`87596b31…fb4cf`/`694daeb7…30b`；inventory `248/224/24`的SHA-256为`5d7e4be5…81a9b`，test runtime policy为`2324f8ac…93e65`。operator16/16、Supervisor29/29、release/catalog34/34及真实system adapter通过；首次干净Node全套在业务断言前发现release gate测试摘要漏绑，修正后第二次又发现postdeploy probe测试仍要求每个wrapper复制锁路径。现将断言改为helper/mjs固定唯一路径、shell wrapper必须source helper并禁止复制路径，原3/3通过；下游摘要重新绑定后全套从头执行。
