...
- 当前采购接收只读基线：Package ID 2/v2、Material Requirement Plan ID 1/v1 和 PRQ ID 1 / `PRQ-00000001` 均为 `ACCEPTED`；Plan 属于 D-090 分支 A 的采购交接状态，v1 计算快照、行、分配和来源摘要没有改写。Purchase ACCEPT/RETURN `1/0`、待接收/已处理 `0/1`；权威 `PURCHASE_ACCEPTED` actor `uat_20260729_purchase`、时间 `2026/08/04 06:06:15 Asia/Shanghai`、request_id `80568b28-47f5-4f58-8901-afc053871998`，已提交 Event 精确投影为 SUCCESS。Material 533—536 各为毛需求/净采购/PRQ `10/10/10 PCS`、四项快照供应 0，九项当前供应也全部 0；Inventory Balance/Planning Allocation/Delivery Plan/Receipt/Ledger/AP/Work Order保持0，RFQ/Quote当前事实由下一条独立基线记录。FIX-18当时保护指纹`814811509c476e270f9cd82badb85aa8bb1bf8e1f01e8bb72b4cd9fec9c9a4ff`只描述其历史验收时点，不覆盖后续已获授权的RFQ生命周期。
- 当前 RFQ/Award/转PO基线：PostgreSQL `bigint` Binding/Quote/Comparison/Candidate/Award/Line ID均以规范十进制字符串进入页面DTO，稳定数据库ID和关系外键是身份；数组位置和显示文本不构成业务键。主`RFQ-00000001`为CLOSED v7、Binding8、Supplier A/B Quote各1且均SUBMITTED v1；Comparison Version1/CURRENT，Line/Candidate/Event为`4/8/4`；Award ID1/v1/AWARDED、Award Line1—4、Award Event9、PO/Delivery Plan为`0/0`。四条获选Candidate为`2/4/6/8`，固定Quote1/v1、Supplier1、Material533—536；持久化Award摘要`7ac6bf2e…a66e55`与派生`AWARD_DECISION_V1`摘要`7beca9f3…fa7651a`分栏，Comparison output摘要为`79554d88…619ec`。FIX32入口仅以权威GET打开确认窗口；最终POST只接收CAS/摘要/Line ID/PO计数断言和最多2,000字正常PO备注，服务端从不可变Award重读Supplier、Material、数量、价格、币种和交期。当前PO模型没有外部参考字段；Delivery Plan没有Header/Line两层，每条PO Line直接对应一个计划聚合，故隔离权威结果为`1 PO / 4 PO Line / 4 Delivery Plan`。主UAT只允许purchase桌面/390×844打开、核验、填写备注后取消，business POST必须为0并保持Award/PO/计划`1/0/0`。
- 当前固定Mapping资格基线：Award Line1—4分别沿Candidate`2/4/6/8`、Quote Line1—4和RFQ Binding1—4闭合到Supplier A Mapping UUID`224d1965…07ff8`、`43ca04d8…18030`、`aa16f7e7…f257e`、`9659ad2d…c63f`，对应fact1—4/v1/row CAS3、Material533—536和PCS→PCS正数1:1。FIX33的GET/POST共用`AWARD_PO_MAPPING_QUALIFICATION_V1`和单一transaction as-of；legacy `base_unit_id=NULL/base_uom=PCS`只可精确解析唯一启用PCS Unit。主UAT只执行一次purchase预览后取消：四行qualified、`po_convertible_now=true`、`preview_get=1`、`business_post=0`、Session0；唯一失败请求`f30a7801-1cd0-4849-95a8-9c61d5c52e67`、四条Mapping和Award/RFQ均保留，PO/Line/Plan/queue全0。
- Supplier Mapping 治理基线：`supplier_mappings`继续是唯一正文/版本权威；0038新增稳定`mapping_uid`、版本号、提交/审核事实、不可变Event、Supplier part claim和ACTIVE 1:1有效期排他约束。主UAT八条Mapping均为ACTIVE，覆盖Supplier 1/2×Material 533—536；八条固定Binding仍存在。当前RFQ为CLOSED v7、两家Supplier各一条Quote、Comparison v1/CURRENT、Award/Line/Event/PO`1/4/1/0`；`po_convertible_now=true`不构成转PO授权。
