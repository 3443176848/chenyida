...
`SELFHOST-OPS-POSTGRES-RUNTIME-PRIVILEGE-56`现为唯一`DOING`，严格起点为`fb1f7e8893b2affba0ca07ecd9629ae2726adca9`/tree`13fe6ce3d04b60bbc724f63b9fa7b5bdc5d16d3e`。只读UAT catalog摘要确认只有1个非内置LOGIN且为superuser、拥有数据库及全部433个public relation，Web/Worker活动连接也共用该角色；Compose仍通过环境交付数据库/初始化/Setup/Admin秘密且PostgreSQL没有custom tablespace持久mount。仓库已关闭Web锁与Backup control/capture权限边界；随后把共享Parser错误合同、lease reader/writer、Web enqueuer/Worker queue、初始Mapping publisher和normalization staging writer拆成单向模块，Web raw candidate本身不再包含Worker-only写入。Web最终表权限为`9/190/209/79`、sequence USAGE为173，职责组统一为`chenyida_erp_{web,worker,admin,backup}_priv`，access intent SHA-256为`b2defe95…3aa1`。D-132 v1 `ACTUAL_OFFHOST/RECOVERY_READY`现已由V4 validate/create/publish与Dashboard双层拒绝，v1 synthetic保持可解析但永不ready。PG17编译catalog已内容寻址冻结234表、211序列、394 routine、6独立type、3 extension及3132列/1709约束/957索引/285非内部trigger，31类unsupported计数全零；目录文件/制品/逻辑SHA-256分别为`4ca22dfa…1162`、`93af15b7…7674`、`40c8c620…7f8f`。源码`8675efd…`在干净Git归档上通过固定PG17正式84文件/401项和独立新空catalog重建，manifest-only直接子提交`633b42d…`绑定50文件、bundle SHA-256`baf820f4…ddc1`；该链只冻结catalog检查点。当前进入v2完整角色/ACL policy与reconciler，后续还需secret-file、operator、mount合同及TASK56最终bundle。不创建真实角色/凭据/Volume，不修改UAT/生产或业务数据，系统继续`PRODUCTION NO-GO`。
