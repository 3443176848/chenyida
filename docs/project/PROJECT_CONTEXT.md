...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量loopback；生产类别不能启用。身份和全部业务写均要求Origin和Cookie/Header CSRF双提交。当前Award历史修复alpha.40 Web为`sha256:bb544f89ac405c9565fa551c4120c89d4cc58022220db9a3f46c548a6533a81d`，Worker仍为`sha256:32d1ae335610c097d9fa38dd411acabc525c0fe17cfcb863271e32317afe96aa`；0039未变。PostgreSQL、Worker、Caddy和四卷均未更换；旧Web`f1184385…`已有FIX31精确回退tag。
...
- 当前 RFQ稳定ID、CAS、Comparison与Award Candidate基线：PostgreSQL `bigint` Binding/Quote/Comparison/Candidate ID以规范十进制字符串进入页面DTO，稳定数据库ID和关系外键是身份；数组位置和显示文本不构成业务键。主`RFQ-00000001`为ISSUED v6、Binding8、Supplier A/B Quote`1/1`且均SUBMITTED v1；Comparison Version1/CURRENT，Line/Candidate/Event为`4/8/4`，Award/Award Line/PO`0/0/0`。四行Candidate精确为`1/2`、`3/4`、`5/6`、`7/8`，B固定Quote2/v1，A固定Quote1/v1；option和Award DTO都使用Candidate ID。确认窗口从同一Comparison DTO固定展示Quote 1/v1与2/v1，精确声明一次不可变Award操作/恰好四条Line、上游不修改及PO/Delivery Plan/Receipt/Ledger/AP/Work Order/生产/财务零自动创建，并把“定标转PO与到货计划”定义为独立后续任务；Award提交DTO未改。没有独立Comparison Header ID；Version身份为RFQ1/Round1/Version1/四个持久化basis，状态CURRENT为服务端投影。确定性输出摘要`79554d88ccdb643a860c0c69e77222abce80eb4d3d8314d88135d3966fb619ec`，完整保护指纹`16d70f1865e3a2e3b0e840f289d13b340e4f6b87800b1c79d98865112d0cf5bc`在备份恢复、部署和主UAT前后保持；主UAT只登录purchase、桌面/390×844打开确认后取消，business POST0、Session0。
