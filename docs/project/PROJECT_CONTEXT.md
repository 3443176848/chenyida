...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量loopback；生产类别不能启用。身份和全部业务写均要求Origin和Cookie/Header CSRF双提交。当前Award→PO确认修复alpha.40 Web为`sha256:2396c8bc4fd5658c26cef11c4a438b2edb474607b73b2b8ee7fe337b125575ed`，Worker仍为`sha256:32d1ae335610c097d9fa38dd411acabc525c0fe17cfcb863271e32317afe96aa`；0039未变。PostgreSQL、Worker、Caddy和四卷均未更换；旧Web`bb544f89…`已有FIX32精确回退tag。
...
- 当前固定Mapping资格基线：Award Line1—4分别沿Candidate`2/4/6/8`、Quote Line1—4和RFQ Binding1—4闭合到Supplier A Mapping UUID`224d1965…07ff8`、`43ca04d8…18030`、`aa16f7e7…f257e`、`9659ad2d…c63f`，对应fact1—4/v1/row CAS3、Material533—536和PCS→PCS正数1:1。FIX33的GET/POST共用`AWARD_PO_MAPPING_QUALIFICATION_V1`和单一transaction as-of；legacy `base_unit_id=NULL/base_uom=PCS`只可精确解析唯一启用PCS Unit。主UAT唯一失败请求`f30a7801-1cd0-4849-95a8-9c61d5c52e67`必须保留，不得重试转换或修改Mapping。
