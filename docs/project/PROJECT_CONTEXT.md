...
- 发布标识：包名为 `chenyida-erp-selfhosted`；源码与受控公网并行 UAT Web 均为 `0.1.0-alpha.39`，PostgreSQL 为 38/head `0038_supplier_mapping_governance.sql`。alpha.39 是并行非生产 UAT 部署记录，不是生产 release。
- 原始发布基线：PHASE0-TASK03 于 `39946f6` 上定义 `0.1.0-alpha.1` / PostgreSQL `0001`—`0005`，并由 `12d3ea3` 提交。该历史定义不改写；当前源码包已演进到 `alpha.39`。
...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量 loopback；生产类别不能启用。身份和全部业务写均要求 Origin 和 Cookie/Header CSRF 双提交。当前 alpha.39 Web 为 `sha256:c1576bd22a209fb6f524e304bcf12cc38af4d67a35c76f37fa8dc1311c2922c8`；原 alpha.38 `sha256:6622029f…` 与首次 alpha.39 `sha256:28a08d40…` 分别保留 FIX-20 predeploy/pre-hotfix 精确回退 tag。0038 已串行部署；后续 legacy Unit 修复只替换 Web，PostgreSQL/Worker/Caddy 未重建。
...
- Supplier Mapping 治理基线：`supplier_mappings` 继续是唯一正文/版本权威；0038 新增稳定 `mapping_uid`、版本号、提交/审核事实、不可变 Event、Supplier part claim 和 ACTIVE 1:1 有效期排他约束。purchase 仅可读/创建/编辑本人 DRAFT/提交，operations 仅可读队列/批准/退回且无正文编辑权，engineering 保持只读；创建人自审永远禁止。RFQ page/create/issue 共用当前有效 1:1 coverage 查询，0/4 或部分覆盖禁选并以 `SUPPLIER_MAPPING_INCOMPLETE` fail closed。主 UAT 两家 Supplier 当前均 0/4，精确缺 Material 533—536，Mapping/RFQ/Quote/Award/PO 均为 0；purchase 只读 Chromium 通过、business POST 0，operations 因既有 Canonical `must_change_password=true` 未登录且未绕过，因此结论为 `SUPPLIER MAPPING GOVERNANCE DEPLOYED — MAIN UAT NOT VERIFIED`。
...
`SELFHOST-UAT-FIX-20` 已完成：alpha.39/0038 的受控 Supplier Mapping 生命周期、purchase 维护、operations 只读异人审核、不可变版本/Event 与 RFQ 1:1 coverage 门禁已经实现、恢复验证并部署到并行非生产 UAT。隔离八 Mapping 与两家 4/4/RFQ DRAFT 旅程通过；主 UAT purchase 只读为 0/4×2 且业务 POST 0，operations 因既有 Canonical 强制改密而未验证，主库 Mapping/RFQ/Quote/Award/PO 仍为 0。现在没有自动执行任务；不得开始八条主 UAT Mapping，除非先以独立身份任务解除 operations 登录阻断并获得新的业务写授权。此前身份恢复和历史 PARTIAL/BLOCKED 记录均保持。
