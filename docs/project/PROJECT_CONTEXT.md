...
- 控制面基线：D-113已接受，`AGENT-R1`无状态只读控制器已完成。它只读取本地Git、权威Markdown、package和Migration文件，核对状态/路径/版本/Migration漂移并向stdout输出确定性清单；台账空闲时返回`IDLE`且不自动认领任务。控制器不建立Control Store、租约、能力代理或Agent Runtime，不连接UAT/生产或数据库，不执行Migration/build/deploy，不修改ERP业务逻辑；R2—R5仍未实施或授权。
