...
- 当前 RFQ 稳定 ID 与追溯基线：PostgreSQL `bigint` Binding ID 以十进制字符串进入页面 DTO，Repository 直接投影 `b.id::text` 并通过稳定 RFQ Supplier、RFQ Line、Mapping fact/version 外键连接；排序只能移动完整行，数组位置、Material/Supplier 顺序和 `index + 1` 均不得定义身份。主 `RFQ-00000001` 为 generation 1 / DRAFT v2、四行 Material 533—536各10 PCS、Supplier 1/2、截止日2026-08-31/CNY；Binding 8、唯一 `RFQ_MAPPING_CONFIRMED` Event 1、`RFQ_ISSUED` 0、Quote/Award/PO 0。权威关联按 PK 是 1→S1/M533/`224d…`、2→S1/M534/`43ca…`、3→S1/M535/`aa16…`、4→S1/M536/`9659…`、5→S2/M533/`45a3…`、6→S2/M534/`5bd2…`、7→S2/M535/`3ac2…`、8→S2/M536/`5432…`；固定摘要 `9765f8fdef768335a25b314867dd3e077429a84848cba067ff8394c8a017848d` 已以源码现有规范化逻辑复算一致。FIX-26后主UI固定按Binding ID 1—8，独立显示Binding ACTIVE、Mapping ACTIVE、邀请INVITED及两类漂移；旧`3,4,1,2,7,8,5,6`序列退出身份主字段，只能解释为历史摘要规范化输入顺序且不得按位置配对。
- Supplier Mapping 治理基线：`supplier_mappings` 继续是唯一正文/版本权威；0038 新增稳定 `mapping_uid`、版本号、提交/审核事实、不可变 Event、Supplier part claim 和 ACTIVE 1:1 有效期排他约束。purchase 仅可读/创建/编辑本人 DRAFT/提交，operations 仅可读队列/批准/退回且无正文编辑权，engineering 保持只读；创建人自审永远禁止。主 UAT 八条 Mapping 均为 ACTIVE，覆盖 Supplier 1/2×Material 533—536；FIX-24 起点前的独立授权已把历史主草稿固定为八条精确版本 Binding并写唯一成功 Event，当前 RFQ为 DRAFT v2。FIX-25 只读确认该固定关系和摘要一致；这不构成发出授权，仍不能直接发出。
...
`SELFHOST-UAT-FIX-26` 已完成：0039独立`binding_status`与Mapping/邀请状态分栏，确认窗口最终按钮为“确认发出”，Receipt、AP、Work Order等完整下游保护逐项列出；主展示按Binding ID 1—8读取数据库同行外键，摘要规范化排序不再构成身份配对。隔离发出、正式备份/第二库恢复、Web-only部署和purchase-only桌面/390×844只读取消均通过；业务POST 0、Session 0，主RFQ仍DRAFT v2、Binding 8、Mapping Event 1、ISSUED/Quote/Award/PO及全部下游0。现在没有自动执行任务；正式发出必须在新的明确授权任务中重新校验CAS/Binding/摘要/Mapping/PRQ/截止日，不能直接开始Quote、Award或PO。此前身份恢复和历史PARTIAL/BLOCKED记录均保持。
