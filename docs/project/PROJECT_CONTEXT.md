...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量 loopback；生产类别不能启用。身份和全部业务写均要求 Origin 和 Cookie/Header CSRF 双提交。当前alpha.40 Web为`sha256:20b41bd34741758e707f3748baaa1018232df6be5d44cd63bed290fd49c9f4f9`，Worker仍为`sha256:32d1ae335610c097d9fa38dd411acabc525c0fe17cfcb863271e32317afe96aa`；0039未变，FIX-27只替换Web。PostgreSQL、Worker、Caddy和四卷未更换，旧Web`c8c3fdd5…`已有精确FIX-27回退tag。
...
- 当前 RFQ稳定ID、CAS与Quote追溯基线：PostgreSQL `bigint` Binding/Quote ID以十进制字符串进入页面DTO，稳定数据库ID和关系外键是身份；数组位置和显示文本不构成业务键。主`RFQ-00000001`为generation 1 / ISSUED v4、四行Material 533—536各10 PCS、Supplier 1/2、截止日2026-08-31/CNY；Binding 8、`RFQ_MAPPING_CONFIRMED` 1、`RFQ_ISSUED` 1。Supplier A为RESPONDED，保留Quote ID 1/SUBMITTED v1、外部参考`UAT-Q-A-042576`、有效期2026-09-30、四行各10 PCS×12.00 CNY/120.00、总额480.00、承诺2026-10-20相对需求2026-10-30提前10天；唯一Quote Event为`QUOTE_SUBMITTED/SUCCESS`且未记录版本转换，没有CREATE Event或独立业务编号。Supplier B仍INVITED且Quote 0，Award/PO 0/0。权威Binding关联仍为1—4→S1/M533—536、5—8→S2/M533—536及原八Mapping UUID；固定摘要`9765f8fdef768335a25b314867dd3e077429a84848cba067ff8394c8a017848d`和保护指纹`597eb456837e0cda35d3544c1aeae94f3a190eed373d1145de5a72261fe37f9f`部署/UAT前后不变。RFQ v3→v4与A邀请INVITED→RESPONDED是D-099定义的正常聚合响应，不是Mapping漂移。
- Supplier Mapping 治理基线：`supplier_mappings`继续是唯一正文/版本权威；0038新增稳定`mapping_uid`、版本号、提交/审核事实、不可变Event、Supplier part claim和ACTIVE 1:1有效期排他约束。主UAT八条Mapping均为ACTIVE，覆盖Supplier 1/2×Material 533—536；八条固定Binding、Mapping Version/Row CAS/content digest与摘要仍未改变。RFQ后续已在独立授权任务中发出并由Supplier A响应至ISSUED v4；这不授权Supplier B报价、Quote修订、Comparison、Award或PO。
