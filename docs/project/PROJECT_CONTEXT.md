...
`SELFHOST-PRODUCTION-READINESS-40`是当前持续交付主线的事实起点，完整门禁见[PRODUCTION_READINESS.md](PRODUCTION_READINESS.md)。当前结论仍为`PRODUCTION NO-GO`：TASK41已完成四域V2工具，TASK54又按D-131补齐签名密文外层、双向ACK、调度/保留及V3 readiness，并以合成密文双集群恢复验证；但没有真实异机锚点、真实密钥/调度/WORM、当前数据恢复、cluster roles/ACL/default privileges/tablespace或RTO。源码alpha.46/0045与非生产UAT alpha.42/0040仍不一致。TASK46/TASK47已关闭完整typecheck和Browser子门，TASK49/TASK50关闭仓库监控及未来候选容器最小权限合同；TASK53按D-130关闭首次晋升生命周期自锁，TASK54最新内容寻址链为`fd0a9cff`→`315b1f3d`的47文件bundle。TASK51本机候选以及TASK53旧bundle均为`STALE / NOT AUTHORIZABLE`，当前仍无与最新源码一致的外部候选、host supervisor、正式镜像证据或19步PASS，且这些合同都未部署到UAT。TASK43—TASK45已关闭导入fallback、会话绝对寿命及health/Worker/storage误报的仓库风险，但运行UAT未部署；真实数据迁移、完整跨岗位验收、员工试运行和正式切换均未完成。
...
- 发布标识：包名为`chenyida-erp-selfhosted`；当前源码为`0.1.0-alpha.46`、head 0045及TASK54内容寻址链`fd0a9cff`→`315b1f3d`。当前47文件bundle只代表仓库supervisor输入；尚未从该提交重建Web/Worker。TASK53 bundle及TASK51的manifest `sha256:249d0ce4…5b7f`/`sha256:0e07fded…8370`、诊断和loopback身份均已过期且不可授权。受控公网并行UAT Web仍运行`0.1.0-alpha.42`原镜像，UAT PostgreSQL仍为40/head`0040_warehouse_receipt_readiness.sql`；0041—0045均未部署到UAT。alpha.42仍只是Web-only非生产UAT记录，不是production release。
...
- 备份恢复V2/V3：D-115/TASK41的四域manifest、root-only libpq凭据、精确writer/database guard、内容reconciliation和内层恢复保持稳定；D-131/TASK54以签名密文外层、双向ACK、操作策略和`chenyida-erp-backup-verification/v3`关闭仓库来源/误报缺口。旧V2人工复制只显示`LEGACY_V2_INNER_ONLY`，只有真实offhost、已安装调度、有效dry-run保留及当前runtime/database/Migration全部匹配才可能ready。合成/隔离测试通过，但真实异机目标、密钥、timer/WORM、角色/ACL/default privileges/tablespace、真实恢复/RTO继续阻塞G2。
