...
- 发布标识：包名为`chenyida-erp-selfhosted`；当前源码为`0.1.0-alpha.47`、head 0046。TASK61当前canonical输入为source`b057f81`→monitor manifest-only`3327be4`→Supervisor manifest-only`222584c`，27/105文件bundle为`6782ec58…aea07`/`56157a68…efcb`；TASK60、TASK57及更早Web/Worker镜像、bundle、诊断和loopback身份均已过期且不可授权，当前没有源码匹配的镜像候选。受控公网并行UAT Web仍运行`0.1.0-alpha.42`原镜像，UAT PostgreSQL仍为40/head`0040_warehouse_receipt_readiness.sql`；0041—0046均未部署到UAT。alpha.42仍只是Web-only非生产UAT记录，不是production release。
