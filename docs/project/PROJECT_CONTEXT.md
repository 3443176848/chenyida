...
`SELFHOST-PRODUCTION-READINESS-40`是当前持续交付主线的事实起点，完整门禁见[PRODUCTION_READINESS.md](PRODUCTION_READINESS.md)。当前结论仍为`PRODUCTION NO-GO`：TASK41/TASK54已完成四域V2与签名密文外层，TASK55又按D-132补齐cluster roles/memberships/settings、owner/ACL/default privileges、custom tablespace、秘密重新绑定、加密cluster capsule/joint transfer v2、readiness v4和崩溃安全executor，并以合成双PostgreSQL cluster验证；但没有真实异机锚点、真实密钥/调度/WORM、当前数据恢复或真实RTO。当前Compose还使用共享初始化superuser、环境变量秘密和superuser backup/restore operator，没有获批custom tablespace持久mount。源码alpha.46/0045与非生产UAT alpha.42/0040仍不一致。TASK55最新内容寻址链为`b93d838`→`2136aa3`的49文件bundle；TASK54 bundle、TASK51本机候选及全部更早候选均为`STALE / NOT AUTHORIZABLE`。当前仍无与最新源码一致的外部候选、host Supervisor、正式镜像证据或19步PASS，且这些合同都未部署到UAT；真实数据迁移、完整跨岗位验收、员工试运行和正式切换均未完成。
...
- 发布标识：包名为`chenyida-erp-selfhosted`；当前源码为`0.1.0-alpha.46`、head 0045及TASK55内容寻址链`b93d838`→`2136aa3`。当前49文件bundle只代表仓库Supervisor输入；尚未从该提交重建Web/Worker。TASK54及更早bundle、TASK51的manifest `sha256:249d0ce4…5b7f`/`sha256:0e07fded…8370`、诊断和loopback身份均已过期且不可授权。受控公网并行UAT Web仍运行`0.1.0-alpha.42`原镜像，UAT PostgreSQL仍为40/head`0040_warehouse_receipt_readiness.sql`；0041—0045均未部署到UAT。alpha.42仍只是Web-only非生产UAT记录，不是production release。
