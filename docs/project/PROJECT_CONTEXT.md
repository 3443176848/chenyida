...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量loopback；生产类别不能启用。身份和全部业务写均要求Origin和Cookie/Header CSRF双提交。当前确认合同修复alpha.40 Web为`sha256:f11843852426478828c87cf6ec1e889949614beb5ce54df49c23557d16b75e34`，Worker仍为`sha256:32d1ae335610c097d9fa38dd411acabc525c0fe17cfcb863271e32317afe96aa`；0039未变。PostgreSQL、Worker、Caddy和四卷均未更换；旧Web`f239ffe3…`已有FIX30精确回退tag。
...
- Supplier Mapping 治理基线：`supplier_mappings`继续是唯一正文/版本权威；0038新增稳定`mapping_uid`、版本号、提交/审核事实、不可变Event、Supplier part claim和ACTIVE 1:1有效期排他约束。主UAT八条Mapping均为ACTIVE，覆盖Supplier 1/2×Material 533—536；八条固定Binding仍存在。当前RFQ已到ISSUED v6、两家Supplier各有一条Quote并已有Comparison v1；这不授权Quote修订、人工定标、Award或PO。
