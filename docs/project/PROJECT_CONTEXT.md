...
`SELFHOST-PRODUCTION-READINESS-40`是当前持续交付主线的事实起点，完整门禁见[PRODUCTION_READINESS.md](PRODUCTION_READINESS.md)。当前结论仍为`PRODUCTION NO-GO`：TASK41/TASK54已完成四域V2与签名密文外层，TASK55按D-132补齐cluster security/tablespace恢复和readiness v4，但没有真实异机锚点、密钥/调度/WORM、当前数据恢复或真实RTO。TASK56已在alpha.47/0046闭合Web行锁、Backup control/capture、D-132 v1 actual失败关闭、PG17精确catalog、9角色/1261条ACL，以及runtime/migration session、六份secret-file、容器运行边界和tablespace mount/recovery map静态合同；最新源码`6ddfae92…`与manifest-only`ef409bbb…`形成53文件bundle`bac5e882…cd9e`，完整适用回归通过。该检查点仍不包含生产受控operator和可信预授权`runtime_configuration_sha256`探针，也没有创建真实角色、secret或Volume。当前UAT仍使用共享superuser、环境变量秘密且为alpha.42/0040；全部旧bundle/候选均不可授权，当前仍无最新源码匹配镜像、host Supervisor、正式镜像证据或19步PASS，真实迁移、跨岗位验收、员工试运行和正式切换均未完成。
...
`SELFHOST-OPS-POSTGRES-RUNTIME-PRIVILEGE-56`现为唯一`DOING`，严格起点为`fb1f7e8893b2affba0ca07ecd9629ae2726adca9`/tree`13fe6ce3d04b60bbc724f63b9fa7b5bdc5d16d3e`。只读UAT摘要仍确认唯一非内置LOGIN为共享superuser、数据库及433个public relation owner，Web/Worker共用该身份；运行面仍通过环境交付秘密且无custom tablespace持久mount。仓库侧已经固定Web最终表权限`9/190/209/79`与sequence USAGE173、9角色/5 LOGIN/4 NOLOGIN、1261条非owner ACL、849个物理ACL对象、2条global default ACL及五身份正反探针；runtime session只接受`pg_catalog,public,pg_temp`，migration只接受`public,pg_temp`，checkout身份/设置漂移会销毁连接。UAT/production拒绝秘密环境值并要求六份独立32-byte base64url文件、唯一inode/value、no-follow/单硬链接及逐服务gid/mode；容器策略又固定进程身份、mount/tmpfs/network/resource与独立tablespace Volume/PG17 child/recovery map。依赖lock/tree摘要为`9c3949bf…eb5`/`e3b36304…659`，源码`6ddfae92bf3ed95314944e95043240fbe26fdee3`/tree`73bafe754b07bc99d5f2268daf2a1b1d001405c9`和manifest-only`ef409bbb8d8cefe0ce596759fc57b3d222bd6ea2`/tree`018fb3f8cc47b9c96296f53576e6aee6450fae83`形成53文件静态bundle`bac5e882…cd9e`。当前进入生产受控root operator、host lock、持久intent/backup fence和可信预授权运行配置摘要探针；任何后续源码会使该bundle失效。没有真实角色、凭据、secret、Volume、UAT/生产或业务数据变更，系统继续`PRODUCTION NO-GO`。
...
- 发布标识：包名为`chenyida-erp-selfhosted`；当前源码为`0.1.0-alpha.47`、head 0046。TASK55内容寻址链`b93d838`→`2136aa3`及TASK51的manifest `sha256:249d0ce4…5b7f`/`sha256:0e07fded…8370`、诊断和loopback身份均已过期且不可授权；尚未从alpha.47/0046重建Web/Worker。受控公网并行UAT Web仍运行`0.1.0-alpha.42`原镜像，UAT PostgreSQL仍为40/head`0040_warehouse_receipt_readiness.sql`；0041—0046均未部署到UAT。alpha.42仍只是Web-only非生产UAT记录，不是production release。
