...
- 备份恢复V2/V3/V4：D-115/TASK41的四域manifest、root-only libpq凭据、精确writer/database guard、内容reconciliation和内层恢复保持稳定；D-131/TASK54增加签名密文外层、双向ACK、操作策略与V3；D-132/TASK55又增加加密cluster capsule、joint transfer v2、角色/ACL/default privileges/tablespace恢复、凭据重新绑定和`chenyida-erp-backup-verification/v4`。旧V1—V3与synthetic结果都不能ready；只有真实offhost、恢复机消费、已安装调度、有效保留、cluster三类回执及当前runtime/database/Migration全部匹配才可能ready。合成/隔离测试通过，但真实异机目标、密钥、timer/WORM、当前数据恢复/RTO继续阻塞G2。
