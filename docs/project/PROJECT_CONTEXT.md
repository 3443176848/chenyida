...
`SELFHOST-UAT-FIX-25` 已按分支 B完成：PostgreSQL 八条 Binding 外键链、Mapping fact/version 和现有 `canonicalDigest` 固定摘要全部一致，页面也直接使用同一 DTO 行的真实主键。错误旧基线由把 FIX-24 当次显示顺序 `3,4,1,2,7,8,5,6` 与 RFQ Line/Material 顺序按位置 zip 产生；现已由逐行权威关联表替代。任务未改代码、数据库、Migration或运行面，未登录、备份恢复或部署，业务 POST 0；主 RFQ仍 DRAFT v2、Binding 8、Mapping Event 1、ISSUED/Quote/Award/PO 0。现在没有自动执行任务；正式发出必须在新的明确授权任务中重新校验 CAS/Binding/摘要/Mapping/PRQ/截止日，不能直接开始 Quote、Award 或 PO。此前身份恢复和历史 PARTIAL/BLOCKED 记录均保持。
