...
- 页面：TASK10 已把根 `app/page.tsx` 改为原生经营工作台；`SELFHOST-UI-REFRESH-DEPLOY-02` 完成统一企业级视觉，`SELFHOST-DASHBOARD-ROLE-HUB-DEPLOY-04` 又把登录后根工作台简化为管理员、采购、市场、计划、工程、财务、生产、仓库八入口，`SELFHOST-UI-STATUS-LOCALIZATION-DEPLOY-06` 已部署共享中文状态，`SELFHOST-UAT-FIX-28`把RFQ Comparison复合身份、摘要、Supplier/Material汇总、Event凭证和390px卡片部署到当前18888非生产UAT，`SELFHOST-UAT-FIX-29`部署逐Line Candidate稳定选择，`SELFHOST-UAT-FIX-30`补齐正式Award确认合同，`SELFHOST-UAT-FIX-31`进一步部署现有Award的稳定聚合/Line引用、原因、持久化与派生摘要、Event/Audit CAS分栏及Award后状态投影，`SELFHOST-UAT-FIX-32`部署Award→PO两阶段确认；`SELFHOST-UAT-FIX-33`已部署固定Binding/Mapping逐行资格凭证和GET/POST统一资格合同，`SELFHOST-UAT-FIX-36`又部署现有PO的受限聚合、完整谱系、Line、Plan/queue、目标Event/Audit/Idempotency摘要、下游零状态及桌面/390×844只读详情。D-105只在治理文档维持受控保留与非追溯边界，产品不声称授权已验证。legacy `public/erp/index.html` 保留为显式兼容业务工作区和回滚入口，不再作为根 iframe 默认依赖。Material Master 和 Import Workspace 使用 `app/materials/` 原生 Vinext 路由。
...
- 发布标识：包名为 `chenyida-erp-selfhosted`；源码与受控公网并行 UAT Web 均为 `0.1.0-alpha.40`，PostgreSQL 为 39/head `0039_rfq_traceability.sql`。alpha.40 是并行非生产 UAT 部署记录，不是生产 release。
- 原始发布基线：PHASE0-TASK03 于 `39946f6` 上定义 `0.1.0-alpha.1` / PostgreSQL `0001`—`0005`，并由 `12d3ea3` 提交。该历史定义不改写；当前源码包已演进到 `alpha.40`。
...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量loopback；生产类别不能启用。身份和全部业务写均要求Origin和Cookie/Header CSRF双提交。当前PO History Traceability alpha.40 Web为`sha256:664e0ac6bd289251f289a8785ac05d955470064a3f921c3ae834f79665a4ec89`，Worker仍为`sha256:32d1ae335610c097d9fa38dd411acabc525c0fe17cfcb863271e32317afe96aa`；0039未变。PostgreSQL、Worker、Caddy和四卷均未更换；旧Web`83c1bff3…`已有FIX36精确回退tag。
...
49. SELFHOST-UI-STATUS-LOCALIZATION-DEPLOY-06 已把 `943c7fa` 共享状态中文化 Web-only部署到公开非生产UAT；该任务完成时Web为`sha256:89e7677538751f2c0a049a113f3d24372a18edaf752bf837038580ac951bd153`，上一版`f45d734b…`有精确回退tag，当前Web以本文件上方FIX29基线为准。正式root-only dump SHA-256为`2beeaeb2ba2d7f7e5c07c7099d0d5985df1bb2ac6a67cc240bcfda0121418d99`，第二新库恢复39/head、226表和相同业务指纹通过。匿名HTTPS/在线资产SHA/中文状态/401和连续60秒稳定性通过；未登录、发业务POST或运行Migration，PostgreSQL/Worker/Caddy及四卷保持。
...
`SELFHOST-UAT-DECISION-35`已完成：D-105作为控制事件受控保留`PO-00000001`，只提供前向授权并明确“不追溯性授权”；Award→PO转换不再重试。当前没有自动执行任务；下一任务只能是PO历史追溯页面修复/验收，且UAT只允许只读PO追溯验收。warehouse、quality和finance试用、到货、收货、IQC、入库、库存、AP、付款和生产均未授权，Receipt/IQC/Ledger/AP/生产记录必须保持0。真实数据迁移、生产部署或切流仍须独立明确授权。
