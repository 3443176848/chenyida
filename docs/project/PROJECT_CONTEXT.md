...
- 页面：TASK10 已把根 `app/page.tsx` 改为原生经营工作台；`SELFHOST-UI-REFRESH-DEPLOY-02` 完成统一企业级视觉，`SELFHOST-DASHBOARD-ROLE-HUB-DEPLOY-04` 又把登录后根工作台简化为管理员、采购、市场、计划、工程、财务、生产、仓库八入口，`SELFHOST-UI-STATUS-LOCALIZATION-DEPLOY-06` 已把共享中文状态/角色/审核与执行结果部署到当前18888非生产UAT。legacy `public/erp/index.html` 保留为显式兼容业务工作区和回滚入口，不再作为根 iframe 默认依赖。Material Master 和 Import Workspace 使用 `app/materials/` 原生 Vinext 路由。
...
- 公网与 UAT 来源校验：公网继续由显式、规范化、单值 `ERP_PUBLIC_ORIGIN` 精确限制，不读取任意转发头；当前唯一公网值为 `https://43.135.148.43.nip.io:18888`。只有 `ERP_DEPLOYMENT_CLASS=uat` 与 `ERP_UAT_ALLOW_LOOPBACK_ORIGIN=true` 同时启用时才额外允许浏览器 Origin 与 Request URL origin 均为严格字面量 loopback；生产类别不能启用。身份和全部业务写均要求 Origin 和 Cookie/Header CSRF 双提交。当前状态中文化 alpha.40 Web为`sha256:89e7677538751f2c0a049a113f3d24372a18edaf752bf837038580ac951bd153`，Worker仍为`sha256:32d1ae335610c097d9fa38dd411acabc525c0fe17cfcb863271e32317afe96aa`；0039未变。PostgreSQL、Caddy和四卷未更换，Worker仅在一致性备份窗口短停并恢复；旧Web`f45d734b…`已有精确回退tag。
...
- 当前 RFQ稳定ID、CAS与Quote追溯基线：PostgreSQL `bigint` Binding/Quote ID以十进制字符串进入页面DTO，稳定数据库ID和关系外键是身份；数组位置和显示文本不构成业务键。2026-08-06状态中文化部署起点与终点只读事实均为主`RFQ-00000001` ISSUED v6、Binding 8、Supplier A/B Quote `1/1`、Quote/Award/PO `2/0/0`；业务指纹为`590579989e2c2c14d37a3970a2392cd5d486f61385adf171eacbb481d6bdbc24`。这组事实晚于FIX-27的ISSUED v4/Supplier B Quote0历史验收快照；本次UI部署不解释、回退或补写其间已发生的业务变化，详细Quote正文须在另获对象范围授权后重新核验。
- Supplier Mapping 治理基线：`supplier_mappings`继续是唯一正文/版本权威；0038新增稳定`mapping_uid`、版本号、提交/审核事实、不可变Event、Supplier part claim和ACTIVE 1:1有效期排他约束。主UAT八条Mapping均为ACTIVE，覆盖Supplier 1/2×Material 533—536；八条固定Binding仍存在。当前RFQ已到ISSUED v6且两家Supplier各有一条Quote；这不授权Quote修订、Comparison、Award或PO。
