...
- 当前 RFQ 稳定 ID 与追溯基线：PostgreSQL `bigint` ID 在页面 DTO 中可能为十进制字符串；RFQ页面/Handler/Service只使用并规范稳定 PRQ/Supplier ID，不按标签反查。0039 新增 generation 2 和逐 Supplier×Line Mapping version binding；新 RFQ 在创建事务固定绑定并写不可变 `RFQ_CREATED/SUCCESS` Event，发出写唯一 `RFQ_ISSUED/SUCCESS` Event且不自动建下游。主 `RFQ-00000001` 为 generation 1 / DRAFT v1、来源 PRQ 1 ACCEPTED、四行 Material 533—536 各10 PCS、Supplier 1/2、截止日2026-08-31/CNY；Binding 0、RFQ Event 0、Quote/Award/PO及全部下游 0。其创建 SUCCESS 只来自精确成功 Audit，八条 ACTIVE Mapping 只代表当前资格/拟绑定，不冒充创建时绑定。
- Supplier Mapping 治理基线：`supplier_mappings` 继续是唯一正文/版本权威；0038 新增稳定 `mapping_uid`、版本号、提交/审核事实、不可变 Event、Supplier part claim 和 ACTIVE 1:1 有效期排他约束。purchase 仅可读/创建/编辑本人 DRAFT/提交，operations 仅可读队列/批准/退回且无正文编辑权，engineering 保持只读；创建人自审永远禁止。主 UAT 当前八条 Mapping 均为 ACTIVE，覆盖 Supplier 1/2×Material 533—536。FIX-22 的绑定表只引用精确版本事实；历史主草稿仍未固定，后续必须在新授权任务中显式确认，不能直接发出。
