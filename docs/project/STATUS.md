...
| 角色边界 | PARTIAL / V2 POLICY AND ISOLATED RECONCILER PASS / PRODUCTION RUNNER OPEN | 9角色/5 LOGIN/4 NOLOGIN职责组、4条membership、1261条非owner ACL、849个物理ACL对象和2条global default ACL已精确闭合；生产root runner、host lock、持久intent/backup fence联锁和真实secret绑定尚未实现，UAT角色/ACL未改变 |
...
| Git/bundle检查点 | PASS / TWO-COMMIT CONTENT ADDRESSED | 源码`88c9f1d25ee08debdf3ef06a533f0596a9047074`/tree`044bf539b0174b93229162c158a5d89c3290bcf3`；manifest-only直接子提交`1bc4ed5a8574c710aacd6e94f2f1ae67bd6ea440`/tree`accfcdc6774e3c3f4cc8ac4f82b192ad37e51c83`，50文件bundle SHA-256为`2fadb84c…0edb`。后续secret/operator/tablespace源码会要求TASK56最终bundle再次重建 |
| 当前测试绑定 | PASS / CLEAN COMMIT / CONTENT ADDRESSED | inventory为`246/222/24`、SHA-256`5c631c1d…6cb`，test/container runtime policy SHA-256为`6746de6d…575`/`74d3f8d2…c1b`；policy5/5、PG17角色/ACL演练、Node119文件/1001项、release6文件/52项、Supervisor31/31、credentials1649及JSON/Shell/diff门通过。Node前两次只因缺同级只读挂载停止，补入既有路径后原断言全绿 |
| Tablespace | OPEN / REPOSITORY TARGET DEFINED | 当前PostgreSQL仅有PGDATA持久卷；未来声明独立`erp_postgres_tablespaces`固定mount，不创建或改动真实Volume |
| 验收范围 | IN PROGRESS | Web锁、Backup、Web源码图、v1实际恢复门禁、PG17精确catalog、v2角色/ACL及五身份合成探针已闭合；secret-file Compose集成、生产受控operator、tablespace mount、完整release与TASK56最终bundle仍待完成 |
...
| 当前资源/清理 | PASS / BELOW STOP LINES | 角色/ACL PG17、Node、manifest与credentials收口后available约1.7GiB、Swap526MiB/1.0GiB、根盘16GiB、Load`0.20/0.86/0.89`、`oom_kill=0`；Web/PostgreSQL healthy，Worker/Caddy running，四服务restart0/OOM false，任务容器和临时清单清零。无secret shell的`docker compose ps`在插值阶段失败关闭，未读取或伪造凭据，改用四个精确容器只读inspect核验 |
