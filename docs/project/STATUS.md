...
| 控制请求合同 | PASS / NON-EXECUTING | 七类项目派生root、五个技术数据库角色、六份runtime Secret+backup service、`EMPTY → 0046`及精确非零Git/镜像/Compose输入已冻结；reader GID同源固定`65532`，D-182当前policy SHA `b9fabb5e…ade8e` |
| 控制请求负向门 | PASS / 4 UNIT / FAIL-CLOSED | 生产项目/root、旧head、浮动镜像、运行动作、source/角色/runtime漂移、重复key及人员数字段失败关闭；人数不成为基础设施基数 |
| one-shot计划入口 | PASS / PLAN V6 / READ-ONLY / 13 UNIT | Host/SNI policy/closure/capability/status及v5/v4基础摘要使当前计划升级为plan/v6；只输出Host/SNI expectation和`NOT_RUN_NO_BASE_EVIDENCE_INTENT`，返回前二次核对source state；同一request确定性输出九步计划、binding及摘要，默认不执行，篡改/并发漂移失败关闭 |
| one-shot执行门 | PASS / DISABLED BEFORE BACKEND | 当前`execute`在输出计划及唯一runtime backend seam之前返回`ISOLATED_UAT_ONE_SHOT_EXECUTION_NOT_AUTHORIZED`；测试证明backend调用0 |
| D-179 receipt policy | PASS / PURE VALIDATOR / 16 UNIT | 八类回执、五类证据体及固定producer/字段/摘要/时间语义通过；深层恶意嵌套稳定失败关闭；policy内部/raw/source closure SHA为`58c34e46…9f6e`/`1eee47ed…7aac`/`0a343c32…90d8` |
| D-179内部前驱链 | PASS / EXTERNAL ANCHORS UNVERIFIED | 成功值固定为`VALIDATED_PURE_INTERNAL_CONTRACT_CHAIN_FROM_UNVERIFIED_EXTERNAL_DIGEST_ANCHORS`；四业务external anchors和control plan为`NOT_EVALUATED`，runtime evidence为`NOT_ESTABLISHED_BY_PURE_VALIDATION` |
| D-179语义边界 | PASS / FAIL-CLOSED / CALLER TIME UNATTESTED | 46项allowlist/applied ledger、五角色完整属性、ACL、release/image、非零且唯一容器身份、网络/health、loopback、OID/system identifier、规范UTC、300秒future skew和从链首起算1小时链龄精确验证；freshness仅相对`CALLER_INJECTED_NOT_ATTESTED`时间成立 |
| D-179 policy roots | PASS / DEPENDENCIES FIXED / RECEIPT ROOTS CALLER-SUPPLIED | Intent/privilege/binding三个raw SHA由执行validator常量锁定；receipt policy raw/internal SHA匹配调用方expected roots，自洽伪造两层均被拒绝，但不把调用方本身描述为受信根 |
| D-180 external anchor policy | PASS / PURE CONTRACT / CALLER SOURCE NOT ATTESTED | Plan/v4、七root/ancestor/mount、七普通凭据metadata、PostgreSQL容器完整network/port/mount/tmpfs及cluster identity形成五节点外层连续性；成功固定为`PURE_EXTERNAL_ANCHOR_CONTRACTS_VALID`，授权和runtime evidence仍未建立 |
...
| D-180后续边界 | SUPERSEDED BY D-181—D-186 | D-181已join外锚/内部链/owner日志，D-182补Host/SNI intent，D-183补声明source graph，D-184以FD观察83成员，D-185完成verified-byte只读plan交接，D-186安装/回读仓库外摘要文件；可信launcher/CPython attestation及host/Docker/PostgreSQL/HTTP runtime仍未实现 |
...
| D-181证据边界 | PASS / CALLER SOURCE NOT ATTESTED | 成功仍固定`SOURCE_CALLER_INJECTED_NOT_ATTESTED / AUTHORIZATION_NOT_ESTABLISHED / NOT_PUBLISHED / NOT_ESTABLISHED_BY_PURE_VALIDATION`；publisher、runtime observer和backend固定未实现，新UAT仍未创建 |
| D-182静态Origin合同 | PASS / LOCALHOST + HTTPS | TCP连接固定`127.0.0.1`，HTTP authority/Host、TLS SNI、`ERP_DOMAIN`和Origin host固定`localhost`；Web Origin为`https://localhost:<published Caddy HTTPS>`，Caddy内部HTTPS端口443。HTTP Origin和Caddy环境漂移失败关闭 |
| D-182 Host/SNI policy | PASS / PURE CONTRACT / 11 UNIT | 固定HTTP/HTTPS未来探针意图、受信链+精确DNS name，禁用insecure skip verify、runtime DNS、proxy环境和redirect following；所有公开入口重验policy/source bytes，D-178/D-179 upstream raw与closure member及注入bytes必须三方一致 |
| D-182 evidence intent v2 | PASS / THREE COMPLETE PLAN OBJECTS | 完整重验D-178 evidence v1；active v6、owner v5、external/receipt v4必须角色/合同/摘要正确并按v6→v5→v4确定性投影，request/project/ports/policy/source/Compose/Web/Worker镜像与基础证据连续。状态明确不重验完整active-plan语义 |
| D-182摘要与binding | PASS / V6 ACTIVE / V5 OWNER BASE / V4 RECEIPT BASE | Host policy内部/raw/closure/module为`dad404da…3010`/`c3edf759…f39a`/`cde30bd6…cedc`/`53283460…a6ff`；v6 body/raw为`f1a3fd38…6dc5`/`459bb65d…f1f0`；one-shot raw、control内部/raw为`5ec47d3c…a674`、`b9fabb5e…ade8e`/`a4809ee3…67f0`；v1—v5 raw字节不变 |
| D-182诚实边界 | PASS / RUNTIME NOT OBSERVED | 纯合同不证明本地CA信任、证书SAN、错误Host/SNI拒绝、反代或HTTP高端口Location；历史readiness仍为`NOT_VALIDATED_MISSING_BOUND_SERVER_NAME`，未追加伪runtime receipt，publisher/observer/backend固定未实现 |
...
| D-184 FD快照 | PASS / 83 MEMBERS / 2,092,585 BYTES / VERIFY ONLY | 固定`/usr/bin/python3 -I -S -B`和bootstrap派生site root；逐组件dir-FD、`O_NOFOLLOW`、root owner/mode、regular/nlink/device、读取前后identity及固定SHA通过。全部成员hash后才从已验bytes执行D-183 validator |
| D-184摘要/边界 | PASS / BOOTSTRAP NOT EXTERNALLY ATTESTED / HANDOFF CLOSED | Bootstrap raw为`ccb9365c…a9c5`，policy内部/raw为`c5359216…2a45`/`708c96cc…cf1a`；输出固定`NOT_EXECUTED_BY_THIS_BOOTSTRAP / PRIOR_PROCESS NOT ATTESTED / HANDOFF NOT IMPLEMENTED / NOT PUBLISHED`。Bootstrap raw只作下一外部锚输入；CPython/stdlib、direct one-shot和payload handoff未建立信任 |
| D-184验证/复核 | PASS / 10 UNIT / AGGREGATE 102 / P0=P1=P2=0 | 专项10/10；聚合`5+13+7+16+13+15+11+12+10=102/102`；隔离Compose policy/config连续两次双PASS。Symlink、双斜杠、可写/owner/type/hardlink/FIFO、self-resign、sentinel、读中变化、FD泄漏及危险AST旁路均失败关闭；最终三路复核无P0/P1/P2 |
| D-184资源/完整性 | PASS / STATIC ONLY / NO TASK RESIDUE | 08:24→08:46 available memory `2,147,934,208 → 2,125,922,304`B，Swap `180,957,184 → 182,300,672`B（+1,343,488B），根盘 `17,484,529,664 → 17,505,542,144`B，Load `0.46/0.22/0.25 → 0.36/0.34/0.29`，PSI/OOM0。Docker保持6/75/277/6，四服务ID不变、restart0/OOM false，Web/PostgreSQL healthy，四保护卷完整；fixture/pycache/精确临时报告残留0 |
| D-185 verified-byte交接 | PASS / 84 MEMBERS / 2,100,283 BYTES / READ-ONLY PLAN | 83个D-183成员加固定D-174 policy；路径集合/路径+值SHA为`5cd4a2e2…03a0`/`80fafed8…843`。唯一活动主线程内按8条module/path边提供258次/78路径读取，read-set/read-trace为`0403a5e1…6969a`/`b6d158d6…025b01`；捕获后不重开原仓source/pyc |
| D-185摘要/信任边界 | PASS / MANIFEST DECLARED / HOST PIN SUPERSEDED BY D-186 | Bootstrap/policy/manifest raw为`bc33d4da…e028`/`809989aa…241d`/`ba8e7337…a7e5`，policy/manifest内部为`4358ef2d…fe1`/`b71662a4…672a`；固定专项测试request的计划SHA为`04e12045…95ea`。D-185当时未钉扎，现由D-186完成外部路径pin；bootstrap/CPython/stdlib和runtime仍未attest/实现 |
| D-185验证/复核 | PASS / 20 UNIT / AGGREGATE 112 / P0=P1=P2=0 | 专项20/20；聚合`5+13+7+16+13+15+11+12+20=112/112`；隔离Compose policy/config连续两次双PASS。原路径替换、未消费成员、source set/value、错误请求恢复、后台线程、execute/额外参数及残留负测失败关闭；三路复核无P0/P1/P2 |
| D-185资源/完整性 | PASS / STATIC ONLY / NO TASK RESIDUE | 起点→09:37 available memory `2,130,964,480 → 2,127,458,304`B，Swap `182,296,576 → 182,812,672`B（+516,096B），根盘 `17,551,732,736 → 17,520,386,048`B，Load `0.05/0.07/0.15 → 0.44/0.28/0.29`，PSI/OOM0。Docker保持6/75/277/6；四服务ID不变、restart0/OOM false，Web/PostgreSQL healthy，四保护卷完整。Compose `ps`因缺`ERP_DEPLOYMENT_CLASS`失败关闭并改用Docker metadata；D184/D185 fixture/临时目录/新增pyc为0，历史pyc未清理 |
| D-186仓库外pin | PASS / INSTALLED + DOUBLE READBACK | 固定路径`/etc/chenyida-erp-isolated-uat-pre-import-v1/manifest.sha256`；目录/文件root:root `0700/0400`，pin为regular、65 bytes、nlink1、device64769/inode101991324，目录仅此一项。Manifest/pin raw为`ba8e7337…a7e5`/`83bea3c…6065`；install/verify报告为`ae0bbd55…fdcfa`/`9dc339be…65c6`，两次verify逐字一致且identity不变 |
| D-186安全边界 | PASS / EXTERNAL PATH ONLY / TRUSTED LAUNCH NOT ESTABLISHED | Create-only、dir-FD/nofollow、root:root、same-device、stable FD+path-after identity、同目录fsync+`RENAME_NOREPLACE`成立；错误对象不覆盖。外部路径不是独立writer trust root，installer/CPython未attest，writer separation/launcher/runtime/UAT未建立。Crash prepared残留严格停止且不得自动删除 |
| D-186验证/复核 | PASS / 12 UNIT / AGGREGATE 124 / P0=P1=0 | 专项12/12；聚合`5+13+7+16+13+15+11+12+20+12=124/124`；隔离Compose policy/config连续两次双PASS。错误摘要/内容/type/mode/owner/group/hardlink/FIFO/目录/symlink/可写祖先、并发、source drift、故障清理、FD泄漏及prepared residue均失败关闭；三路复核无P0/P1阻断 |
| D-186资源/完整性 | PASS / EXPECTED PIN ROOT + FILE / NO TEMP RESIDUE | 10:00→10:21 available memory `2,102,263,808 → 2,101,612,544`B，Swap `182,804,480 → 184,582,144`B，根盘 `17,453,727,744 → 17,463,406,592`B，Load `0.21/0.12/0.13 → 0.25/0.28/0.27`，PSI/OOM0。四服务running/restart0/OOM false，Web/PostgreSQL healthy，四保护卷存在；D186 fixture/pyc/prepared残留0。Compose `ps`缺当前必填环境值时失败关闭，改用Docker metadata核对 |
...
| D-180静态验证 | PASS / 5 + 12 + 7 + 16 + 13 = 53 UNIT / COMPOSE DOUBLE PASS | 控制合同5/5、one-shot 12/12、runtime contracts 7/7、runtime receipts 16/16、external anchors 13/13；聚合runner与隔离Compose policy/config双PASS，最终diff/凭据扫描随提交门复核 |
| D-180独立复核 | PASS / 2 REVIEWERS / P0=0 / P1=0 | 多轮只读复核发现的plan重签、稳定错误、ancestor/mount、路径规范化及完整容器集合问题均修复进入负测；稳定树两路最终结论均无P0/P1，一路1342个逐字段变体为0绕过/0异常泄漏 |
| D-180资源/完整性 | PASS / STATIC ONLY / NO TASK RESIDUE | 21:42→22:52：available memory `2,363,518,976 → 2,256,683,008`B、Swap `179,556,352 → 179,613,696`B、根盘 `17,812,017,152 → 17,746,034,688`B、Load `0.03/0.35/0.25 → 0.23/0.20/0.22`，Memory PSI/OOM0。Docker保持6/75/277/6、Build Cache 0；四服务ID不变、restart0/OOM false、Web/PostgreSQL healthy，四保护卷完整。精确删除5个任务`.pyc`，任务pyc/Compose临时目录残留0 |
| D-179静态验证 | PASS / 5 + 7 + 16 + 11 = 39 UNIT / COMPOSE DOUBLE PASS | 控制合同5/5、runtime contracts 7/7、runtime receipts 16/16、one-shot 11/11；聚合runner与隔离Compose policy/config双PASS，最终diff/凭据扫描随提交门复核 |
| D-179资源/完整性 | PASS / STATIC ONLY / NO TASK RESIDUE | 19:09→20:05：available memory `2,395,295,744 → 2,338,922,496`B、Swap `179,580,928 → 179,576,832`B、根盘 `17,755,828,224 → 17,713,516,544`B、Load `0.01/0.04/0.09 → 0.58/0.39/0.32`，Memory PSI/OOM0。Docker保持6/75/277/6，四服务restart0/OOM false、Web/PostgreSQL healthy；精确清理2个任务`.pyc`后任务pyc/Compose临时目录残留0 |
| D-178静态验证 | PASS / 4 + 10 + 7 UNIT / COMPOSE DOUBLE PASS | 控制合同4/4、one-shot 10/10、runtime contracts 7/7、隔离Compose policy/config双PASS；最终diff/凭据扫描随提交门复核 |
| D-178资源/完整性 | PASS / STATIC ONLY / NO TASK RESIDUE | 18:31→18:50：available memory `2,399,928,320 → 2,383,667,200`B，Swap `179,617,792 → 179,580,928`B，根盘 `17,775,542,272 → 17,746,591,744`B，Load `0.05/0.09/0.08 → 0.62/0.33/0.21`，Memory PSI/OOM0。Docker保持6/75/277/6；四服务restart0/OOM false、Web/PostgreSQL healthy。任务`.pyc`/临时资源0；既有26个历史UAT promotion目录和历史pycache未清理 |
...
| 生产控制面保护 | PASS / UNCHANGED | 请求明确禁止生产supervisor/runner；既有生产policy和默认路径未改。控制policy仍为`deployment_authorized=false / CONTRACT_ONLY_NOT_EXECUTABLE` |
...
| 验证项 | 结果 | 说明 |
| --- | --- | --- |
| 当前状态 | DONE / REPOSITORY UAT ROLLBACK HANDLERS VERIFIED / CATALOG AND HOST ACTIVATION BLOCKED / DYNAMIC VALIDATION DEFERRED / PRODUCTION NO-GO | TASK82已释放active slot；TASK83只读归因资源，TASK70保持受阻 |
| 不可变链 | PASS / CONTENT ADDRESSED | source`c2f071c`/tree`3e262bd0`→Supervisor`aa77732`/tree`a734aa13`；156文件/7,285,043-byte manifest raw SHA-256为`3674e011…35fb`，逐blob和唯一父子拓扑通过 |
| handler/协议 | PASS / FIXED AND FAIL CLOSED | writer、PG、三业务卷、backup_status、runtime、前代Web/Worker和十三postverify均使用固定FD/schema/argv及PREPARE/EXECUTE/PROBE/CONTAIN；无shell或operator自由输入 |
| 耐久恢复 | PASS / PER-EFFECT RECEIPTS | action+execution_mode幂等、intent/started/逐副作用receipt/probe/terminal追加链闭合；commit-before-receipt只从精确live OID/marker/layout或release状态恢复，前缀/漂移返回typed UNKNOWN |
| 数据库/动态事实 | BLOCKED / HONEST | pre/postactivation读取内容、46 Migration ledger、ACL/default privileges、session、identity和layout；但双rename事务/锁/故障窗口未在隔离PG17动态执行，catalog继续`BLOCKED_MISSING_UAT_CAPABLE_HANDLERS` |
| 卷/镜像 | PASS / REPOSITORY CONTRACT ONLY | 新目标不得与active/candidate重叠，内容+metadata+读写探针闭合；helper镜像绑定source/SBOM/Trivy树与资源门，无外部registry/DB更新回执时明确降级，不冒充actual证据 |
| 审计 | PASS / DYNAMIC EXECUTION BLOCKED | inventory262/238/24；跨岗/回退审计链闭合，但能力/host activation、隔离回退演练和人工UAT继续阻断，`assert-ready`不得放行 |
| 自动验证 | PASS / SCOPED APPLICABLE | 最终轻量组合201/201、manifest后installer21/21、11 Python/7 JSON/2 shell解析、262 inventory零漂移、35文件高置信敏感门和diff门通过；三路复核无残留P0/P1 |
| 资源/清理 | STOP LINE ACTIVE / CONTAINER OOM NONE | 收口available约1.2GiB、Swap897/1024MiB、根盘约11GiB、Load`0.24/0.22/0.18`；四服务restart0/OOM false，宿主`oom_kill=2`无增量，无任务临时容器/manifest temp残留 |
| 授权/运行面 | UNCHANGED / NOT AUTHORIZED | 未运行Node全量/build、backup、Compose/PostgreSQL、Migration、镜像、部署、真实员工UAT、恢复、回滚或业务写；四个受保护Volume未触碰 |
| 系统是否可用 | NO | 仓库handler和fake-root结果不等于actual UAT恢复/回退；仍缺动态证明、真实恢复、正式门、业务批准、试运行和切换 |
...
| 验证项 | 结果 | 说明 |
| --- | --- | --- |
| 当前状态 | DONE / REPOSITORY FENCED MIGRATION COMMIT ADAPTER VERIFIED / DYNAMIC DATABASE VALIDATION DEFERRED / NO REAL DATABASE OR UAT ACTION / PRODUCTION NO-GO | TASK74已释放active slot；TASK75接入checkpoint 9 Compose部署，TASK70保持受阻 |
| 不可变链 | PASS / CONTENT ADDRESSED | source`ce7bb23`/tree`f5043439`→fix source`5610a0d`/tree`26edea69`→Supervisor`52242f8`/tree`6a20ec8f`；130文件manifest raw SHA-256为`17efe85d…aad5`且重放一致 |
| 依赖决策 | PASS / D-149 | checkpoint 8使用独立最长15分钟execution grant；数据库围栏保持到checkpoint 9精确交接，unknown/partial不得重跑SQL或自动释放writer |
| 执行合同 | PASS / FAIL CLOSED | execution intent先于授权消费；完整release artifact、精确released角色/ACL/session基线、CONNECT/read-only fence、逐文件事务/ledger及最终sealed fence完整绑定 |
| 恢复 | PASS / PRESERVATION FIRST | active fence阻断非精确恢复；恢复先emergency seal，再按operation+grant唯一label执行stop→kill→退出证明；未知结果只quarantine，不运行down SQL或删除证据 |
| 审计 | PASS / EXECUTOR STILL BLOCKED | artifact/source-manifest SHA-256为`e4aa3687…e2fc`/`53a1515a…4239`；10项SUPPORTED、5项阻断（P0=4、P1=1），6/8必需operation实现，`assert-ready`继续精确拒绝 |
| 自动验证 | PASS / SCOPED APPLICABLE | 受限Node120/120、Python57/57、inventory258/234/24、compile/syntax、生成物重放、凭据30+3文件及diff门通过；typecheck/全量/PG/Docker动态验证因资源停止线未运行并登记TASK70 |
| 资源/清理 | STOP LINE ACTIVE / CONTAINER OOM NONE | 收口前available 1.9GiB、Swap879/1024MiB、根盘12,699MiB、Load`0.33/0.26/0.16`；四服务restart0/OOM false，任务临时Node目录清零。宿主`oom_kill=2`缺同窗口起点，不作增量结论 |
| 授权/运行面 | UNCHANGED / NOT AUTHORIZED | 当前四个UAT容器仍running；未运行build、backup、Compose/PostgreSQL、Migration、镜像、部署、回滚或业务写 |
| 系统是否可用 | NO | 仓库checkpoint 8不等于actual数据库Migration；仍缺Compose/final/UAT/rollback、真实恢复、正式门、业务批准和切换 |
...
| 验证项 | 结果 | 说明 |
| --- | --- | --- |
| 当前状态 | DONE / REPOSITORY AND SYNTHETIC-ISOLATED VERIFIED / ACTUAL ACTIVATION AND CURRENT CANDIDATE BLOCKED / PRODUCTION NO-GO | TASK56已释放active slot；主智能体唯一写入，三条智能体线只读审计 |
...
| PG17编译catalog | PASS / SYNTHETIC-ISOLATED / CONTENT ADDRESSED | 固定PG17.10/libc C/UTF8、46个Migration及access intent v2；234表、211序列、394 routine、6独立type、3 extension，3132列/1709约束/957索引/285非内部trigger，31类unsupported全零。stdin集成修复后catalog文件/制品/逻辑SHA-256为`e5f3c321…c71e3`/`b8536bc1…9beb`/`e0070514…e8c`，独立refresh与完整test通过 |
...
| Git/bundle检查点 | PASS / CONTENT ADDRESSED | 最终文档源码检查点与紧随其后的唯一manifest-only直接子提交形成canonical链，精确Git/tree身份由manifest的`source_commit/source_tree`及父子拓扑自证；旧`bac5e882…cd9e`和全部更早bundle/候选均为`STALE / NOT AUTHORIZABLE` |
...
| 当前测试绑定 | PASS / FULL APPLICABLE REPOSITORY REGRESSION | inventory为`248/224/24`、SHA-256`1112404d…5ab5`，test runtime policy为`df82237b…36e7`，catalog文件/artifact为`e5f3c321…c71e3`/`b8536bc1…9beb`，operator policy文件/逻辑为`53b502fd…d163`/`85c7d2ea…7acc`。功能基线`076b840`通过Node121/1026、PG84/401加catalog、Browser6/11、POSIX7/57、typecheck38/38、release57+54、Supervisor48、Python三基线、Migration及备份/恢复；lint0 error/28 warning、credentials1665 |
| 当前候选运行策略 | BLOCKED / STALE IMAGE FAILED CLOSED | UAT/production Compose当前六服务策略均PASS；TASK51旧镜像仅作离线夹具时以`ADMIN_READ_ONLY_FIXTURE_GROUP_MISMATCH`失败且任务container/Volume/network清零，未冒充当前候选。TASK56禁止build，同源码镜像与正式19步门转交下一候选刷新任务 |
| 静态治理门 | PASS | JSON220、Shell44、Python50、Markdown395/本地链接237、官方source diff及`git diff --check`通过；未读取或纳入未跟踪状态报告 |
...
| 验收范围 | DONE / REPOSITORY SCOPE | Web锁、Backup、源码图、v1实际恢复门禁、PG17 catalog、v2角色/ACL、session、secret、container、tablespace、生产受控operator及最终bundle已闭合；在线Web/Worker不同LOGIN、实际角色/secret/Volume激活、源码匹配候选及正式门仍待外部授权/后续任务 |
...
| 当前资源/清理 | PASS / BELOW STOP LINES | 收口available约1.8GiB、Swap约723MiB/1.0GiB、根盘15GiB、Load低于4、`oom_kill=0`；Web/PostgreSQL healthy，Worker/Caddy running，四服务restart0/OOM false，Migration/恢复/runtime夹具的任务container/Volume/network及临时目录清零 |
...
| 验证项 | 结果 | 说明 |
| --- | --- | --- |
| 最终状态 | DONE / REPOSITORY BROWSER GATE VERIFIED / RUNTIME NOT DEPLOYED / PRODUCTION NO-GO | 6文件/11项Browser子门已关闭并释放active slot；系统仍不能交给真实员工 |
| 严格起点 | PASS / CONTROLLED | `main@fbbf2a5d034d11d8a50f823a55ef78d2d32d682d`、alpha.46/0045；唯一既有未跟踪状态报告保持不读不改不提交 |
| Git身份 | PASS / TWO-COMMIT BUNDLE | 源码`9a18a0f307348c974a6f341565e7d16d76df184c`/tree`8c182d38f1acbcebe10d46e3a09f73c9ec612f22`；manifest-only直接子提交`614ef7ac2aea5ec23029c81b17b8c21adc0935dd`，39文件bundle SHA-256 `e54019df…a7192` |
| 运行时合同 | PASS / CONTENT ADDRESSED | 官方Playwright 1.51.1/Chromium 134.0.6998.35、镜像Repo/config digest、平台、可执行路径/SHA和精确依赖树均固定，漂移失败关闭 |
| Browser E2E | PASS / 6 FILES 11 TESTS | 干净源码提交第十三次完整运行覆盖planning revision、purchase traceability、unit resolution、RFQ binding/traceability和supplier mapping；路径集SHA-256 `71742177…ac86`，无skip/todo |
| 隔离执行 | PASS / CLEAN | 固定PostgreSQL 17 rootfs、只读Git快照、历史Migration模板升到0045、断网单Browser容器；只写合成数据库，服务/库/进程/目录全部清理 |
| 自动验证 | PASS / SCOPED | release合同45、supervisor Python20、完整typecheck38/38、lint0 error/11既有warning、inventory235/211/24及治理/链接/凭据/控制/diff门通过 |
| 运行面边界 | UNCHANGED | UAT保持alpha.42/0040，四服务restart0/OOM false；不连接业务数据库、不挂载四卷、不build/push候选或deploy |
| 资源/清理 | PASS / THRESHOLD HONORED | Browser前available2,424,572KiB、Swap76.32%、根盘27GiB；Browser后Swap短暂80.14%即暂停，未修改Swap，回落后才继续；最终available2,481,228KiB、Swap73.47%、Load`0.98/1.91/1.62`、OOM0、restart0，任务临时资源0 |
| 下一最高风险 | OPEN | 隔离候选Web/Worker镜像、镜像级SBOM/新鲜漏洞PASS和完整18步同候选门尚未完成；G2真实异机恢复、岗位权限与员工试用仍分别阻塞 |
...
| 验证项 | 结果 | 说明 |
| --- | --- | --- |
| 最终状态 | DONE / DESIGN COMPLETE / IMPLEMENTATION NOT STARTED | 新增[执行设计包](../ai-engineering/README.md)及[任务文档](../tasks/PM-002.md)；PM-002收口时为零DOING。后续D-114接受与AGENT-R1-5启动由上方独立任务记录 |
...
| 路线 | PM-002 CLOSURE SNAPSHOT | PM-002收口时D-114仍为提案；现已由项目负责人另行接受并仅授权AGENT-R1-5，历史结论不追溯改写 |
...
| Schema基线 | PASS / HEAD 0041 | `db/schema.ts`、`0041_snapshot.json`、journal idx/tag/head 41与0041一致；0034/0035/0037/0039历史合同按自身`idx/tag`定位，不再把旧Migration误当全局head，未删除约束或降低断言 |
| 服务提交 | PASS / `218ef1b4…0eb1` | 独立types/errors/canonical/config/adapter/repository/service/handler模块并只在`selfhost-api.ts`接线；源码版本同步为alpha.44，批准身份固定`LOCAL_DETERMINISTIC/NONE/NONE`及D-111完整rule/evaluator/config/threshold/dataset/source revision，confidence/semantics均NULL |
| 四能力/ABSTAIN | FAIL CLOSED | Classification唯一ACTIVE Category；Attribute只用ACTIVE定义及严格类型/单位；Material Match唯一ACTIVE严格身份；Supplier Mapping唯一ACTIVE Supplier/part/精确Mapping/Material。缺失、歧义、冲突、失效、转换/证据失败或超限均安全`ABSTAIN`，不使用模糊匹配、总分或概率绕过 |
| 原子性/幂等/CAS | PASS / FORMAL WRITE 0 | 服务端重读group/run/rows/specs/lineage及引用，单事务写run/suggestion/items/evidence/CREATED、必要SUPERSEDED、Audit和持久幂等；同key+摘要/run digest重放，异正文409，并发单run、连续版本和事件CAS失败关闭。故障全回滚，0035决定/link/event、Material、Supplier Mapping及其他正式业务表写入0 |
| 读取有效性 | REPEATABLE READ / ZERO WRITE | GET在同一只读快照重算全部摘要并按数据库时间派生有效性；过期、终止、group不再PENDING、输入/引用/合同漂移均不可审核。创建响应不混入动态有效性，列表使用稳定UUID cursor；GET不补写INVALIDATED或Audit业务事实 |
| API/安全 | PASS | POST/list/detail GET复用`material.import.governance.run/read`及批次可见性；must-change、Origin/CSRF、Idempotency-Key、256 KiB、精确字段、request ID、no-store、中文稳定错误和去敏Audit/通用500覆盖。没有discard/approve/accept/correct/formalize或TASK04正式提交API |
| Migration验证 | PASS | 0041静态Migration/Schema/snapshot/journal合同5/5；隔离PostgreSQL空库0001→0041、0040已有数据升级、重复执行、失败回滚、旧数据保持及守卫/四item/SUGGEST/ABSTAIN/evidence/非法组合7/7；既有0035 Migration合同5/5 |
| Service/回归验证 | PASS | Suggestion Unit/Handler9/9、隔离Service5/5、专项typecheck、0035 Governance Unit/Handler/Metadata61/61、TASK02 Evaluator17/17、`npm test`3/3通过；lint 0 error/11条既有无关warning且任务新增warning0，diff与敏感扫描通过 |
| D-111/holdout | PRESERVED / REVALIDATION REQUIRED | TASK02 calibration、holdout、manifest、标签、阈值及机器报告无变化；开发测试没有使用holdout调规则，正式all-splits/holdout CLI未执行且没有新正式报告。alpha.44完整身份必须在任何release动作前单独重验 |
| 版本/运行面 | SOURCE ALPHA.44/0041 / UAT ALPHA.42/0040 | 运行Web镜像标签仍为alpha.42/source revision`569aa954…a24`，四服务运行且restart0/OOM false；0041只在隔离数据库执行。未登录/调用UAT、未build/Docker build、部署、运行UAT Migration、重启、调用模型或读取模型凭据 |
| 资源/清理 | PASS / NO THRESHOLD BREACH | 测试串行、断网、源码只读、1 CPU、受控heap且一次一个临时容器/库；起点/源码验证后available约`2.2/2.3GiB`、Swap`348/352MiB`、根盘`17/17GiB`、最终Load`0.27/0.71/0.53`。内核OOM匹配0，测试库/容器/目录清零，四个受保护Volume保持且未读取正文，未prune |
| Git/发布边界 | THREE COMMITS / PRIVATE FF ONLY | 持久层与服务两个提交已创建，九份治理文档以`docs: record AI suggestion source readiness`独立收口；全部复核后只一次普通fast-forward push到`recovery-private/main`，public origin不推送，`RELEASES.md`不变。TASK04/TASK05保持TODO |
...
| 基线 | LOCAL_DETERMINISTIC / NO MODEL | 分类、属性、物料候选匹配、供应商映射建议四项纯离线适配器；只读复用`bom-material-governance-v1`，model/prompt为NONE；不输出置信度，不产生formal action，不成为正式mapping写逻辑 |
| Calibration | MEASURED | decision exact 32/32、abstention14/32、coverage18/32、covered accuracy18/18、evidence32/32、stable32/32、safety0、失败sample_id NONE |
| Holdout | MEASURED ONCE | 功能提交后只正式运行一次all-splits；decision exact32/32、abstention13/32、coverage19/32、covered accuracy19/19、evidence32/32、stable32/32、safety0、失败sample_id NONE |
...
| 阈值/发布 | PASS / NOT_AUTHORIZED | 当前calibration/holdout逐项满足D-111，治理层`THRESHOLD_ASSESSMENT=PASS`；机器报告生成时的`threshold_status=UNAPPROVED / release_decision=NOT_AUTHORIZED`保持不可变历史。D-110不变；TASK02收口时TASK03仍TODO，后续D-112源码实施现为`SOURCE_READY / HOLDOUT_REVALIDATION_REQUIRED`，不改变TASK02发布未授权结论 |
...
| registry digest | VERIFIED / THREE-WAY EXACT | push返回、认证registry顶层manifest和带唯一tag的package version三方均为`sha256:e7761e2c61bfe77c6aab526fb0b6cbd840ad1bf6300381f4319f6e279af94964`；数值虽与本地OCI index/Image ID相同，证据来源仍分别记录 |
| OCI/config/layer | PASS / EXACT | registry index只有linux/amd64 child `sha256:36fd3118…482f`和attestation child `sha256:f4a82ba3…621b`；platform config `sha256:72452032…32c7`和9个compressed layer digest逐项匹配D-109预检基线 |
| package可见性/tag | PRIVATE / UNIQUE / NO ASSOCIATION | GitHub package owner/name/type为`3443176848/chenyida-erp-web/container`、visibility为PRIVATE且无repository association；三个预期OCI对象中仅顶层index携带唯一计划tag，没有`latest`或额外tag |
| 按digest回拉 | PASS / ONE PULL / NO RUN | 只执行一次`ghcr.io/3443176848/chenyida-erp-web@sha256:e7761e2c…f94964`拉取；Image ID、平台、config、9层、labels、User、WorkingDir、Entrypoint、Cmd和3000/tcp全部匹配，没有run、replace或deploy |
