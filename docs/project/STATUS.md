...
| 当前测试绑定 | PARTIAL / NEW INVENTORY BOUND / FINAL FULL REGRESSION RUNNING | inventory为`248/224/24`、SHA-256`f2ba95d2…6b66e`，test runtime policy为`3e0179c8…e7ea`，catalog文件/artifact为`87596b31…fb4cf`/`694daeb7…30b`；operator16/16、Supervisor定向29/29、release/catalog34/34和真实PG17 system adapter已通过。首次干净Node全套精确发现release gate测试摘要漏绑并失败关闭，已修正绑定后从头复跑；上一静态检查点不能替代最终全套 |
