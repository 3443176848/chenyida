...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE1-TASK10` 书面设计提交使用 `docs: design material draft ui`；实际哈希以 `git log -1` 为准，提交前基线为 `9bb1756` |
...
| 当前阶段 | Phase 1 Material Draft 创建、编辑与提交审核界面 V1 已完成正式书面规格和低保真线框设计；未实施前端，生产仍未迁移或部署 |
| 当前任务 | `PHASE1-TASK10` 设计已确认并完成，等待提交后人工验收 |
| 下一任务 | 未自动开始；前端实施前必须先独立批准并实现统一详情 `last_rejection` 最小只读兼容项；前端编码、生产迁移或部署均需独立任务与授权 |
...
- 统一详情尚无可靠的 `last_rejection` 投影；只内嵌最近 5 条历史且版本分页不支持按 REJECT 过滤。该最小只读兼容项是 Material Draft 前端实施前置，尚未实施
...
15. Material Draft UI 设计要求可靠展示最近一次驳回，但当前统一详情只有最近 5 条历史摘要；在独立实现 `last_rejection` 只读投影前，不得开始正式前端实施或宣称满足驳回展示验收。
...
- 下一：停止并等待人工验收；`last_rejection` 最小只读 API 兼容项必须先独立批准和实施，之后才能另行授权 Draft 前端编码。候选索引、`PENDING_APPROVAL` 收缩、生产迁移/部署和 `PHASE0-TASK03` 仍需独立任务与授权。
