...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE1-TASK11` 使用 `feat: add last rejection material projection`；实际哈希以 `git log -1` 为准，提交前基线为 `402ef9b` |
...
| 当前阶段 | Phase 1 统一详情 `last_rejection` 最小只读前置已在非生产开发代码实现；Material Draft 前端仍未实施，生产仍未迁移或部署 |
| 当前任务 | `PHASE1-TASK11` 实现、隔离测试、文档和独立提交已完成，等待人工验收 |
| 下一任务 | 未自动开始；Material Draft 前端编码必须由项目负责人另行指定，生产迁移或部署仍需独立任务与明确授权 |
...
- Material Draft 创建、编辑与提交审核界面 V1 书面设计：批准 `/materials/new`、`/materials/:materialId/edit`、布局 C、Schema 驱动完整替换表单、PATCH/GET/submit 边界、页面内存幂等状态机、乐观锁对照、Schema 漂移保护、SAVED_UNSYNCED/RESULT_UNKNOWN 和 54 项端到端验收；只新增文档，尚未实施前端
...
- Material Draft 创建、编辑和提交页面尚未编码；当前批准内容仅为书面规格与低保真线框稿
...
15. `last_rejection` 已在开发代码中解除 Draft UI 的该项 API 前置缺口，但生产仍为旧版本且前端未编码；不得把非生产实现表述为公开站点已具备该字段，前端和部署仍需独立授权。
...
- 下一：停止并等待人工验收；不自动开始 `PHASE1-TASK12`。Draft 前端编码、候选索引、`PENDING_APPROVAL` 收缩、生产迁移/部署和 `PHASE0-TASK03` 均需独立任务与授权。
