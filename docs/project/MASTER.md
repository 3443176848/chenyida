...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE1-TASK05` 功能提交 `feat: add material draft and review service`；实时哈希以 `git log -1` 为准 |
...
| 当前阶段 | Phase 1 Material Master V2 schema、分类属性模板、校验服务及草稿/审核写服务已完成 |
| 当前任务 | `PHASE1-TASK05` 已完成，当前无 DOING 任务 |
| 下一任务 | 建议另行设计受认证授权的 Draft/Review API 适配与幂等边界；`PHASE0-TASK03` 仍保留在台账 |
...
- Material Master V2 Draft/Review API、角色授权、幂等请求、应用层引用/有效期校验和生产迁移尚未实现
...
- 下一建议：另行设计受认证授权的 Draft/Review API 适配，覆盖角色权限、CSRF、请求幂等、错误码映射和隔离 API 测试；不得在该任务中加入页面、导入或生产部署。`PHASE0-TASK03` 的发布与迁移追踪基线仍需后续安排。
