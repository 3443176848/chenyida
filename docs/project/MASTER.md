...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `7cc89b8`，`feat: add material import workspace ui`；`PHASE3-TASK01` 开始时工作区干净 |
...
| 当前阶段 | Phase 3 Material Import Normalization & Staging V1 书面设计已完成；16 项决定仍为 `PROPOSED`，生产环境未改变 |
| 当前任务 | `PHASE3-TASK01` 文档交付已完成，独立提交消息为 `docs: design material import normalization` |
| 下一任务 | 停止并等待“规格确认”；不自动实施 Normalization、`0006`、API、前端、生产资源、迁移或部署 |
...
- Material Import Normalization & Staging V1 书面设计：定义独立 normalization run、版本化 JSON 行快照、独立 issue、Mapping/Metadata 绑定、确定性类型与空值语义、异步租约/Outbox、原子发布、五个 API、`0006` 迁移设计、54 项测试和 16 项 `PROPOSED` 决定；仅文档，尚未实施或接生产
...
- Material Import 的规范化仅完成书面设计，运行时代码、`0006`、API 和前端尚未实现；自动分类、候选匹配、Material Draft 创建和正式物料写入也未实现，生产 Queue/binding、生产迁移和部署仍需独立授权
...
22. Normalization 的 50,000 行、256 KiB/行、256 MiB/批、20 issue/行和 200,000 issue/批均为待确认应用建议，不是生产容量结论；必须在规格确认后用隔离 1k/10k/50k 数据验证，并在实施前重新核验 D1 限制。
...
- 下一：停止并等待“规格确认”，不开始 Normalization 实施、分类、匹配或 Material Draft。生产 Queue/binding、生产迁移/部署、`submitted_by` 候选项、`PENDING_APPROVAL` 收缩和 `PHASE0-TASK03` 均需独立授权。
