...
- `SELFHOST-PRODUCTION-READINESS-40`确认的首要风险继续开放。TASK41只关闭G1合成/隔离工具缺口：PostgreSQL及三个文件数据域仍没有真实异机当前锚点/恢复/RTO；源码alpha.46/0045与UAT alpha.42/0040仍未闭合。TASK43—TASK45已关闭导入fallback、会话绝对寿命和health/Worker/storage的仓库风险，但运行UAT尚未部署；Browser、完整typecheck、候选镜像/SBOM/漏洞证据和完整release gate仍阻止发布。详见[投产准入基线](PRODUCTION_READINESS.md)
