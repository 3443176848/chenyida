...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | `PHASE3-MATERIAL-LIBRARY-REAL-SAMPLE-IMPORT-02`，实际提交以 `git log -1` 为准 |
...
| 当前数据库 | 服务器本地 SQLite 29 张业务/迁移表，已应用本地 `0001`/`0002`；在线 D1/Drizzle 开发 schema 为 45 张表，最新 `0008` 尚未执行生产迁移 |
| 当前开发环境 | OpenCloudOS / Python 3.11 项目虚拟环境 / systemd；Site 历史工具链保留 |
| 当前阶段 | A118/V700 已进入服务器本地清洗审核队列；等待批次级单位和逐行异常确认 |
| 当前任务 | `PHASE3-MATERIAL-LIBRARY-REAL-SAMPLE-IMPORT-02` 已完成；2 Batch、766 Raw Rows、543 Cleaning Rows，未创建内部物料 |
| 下一任务 | 设计批次级 Mapping/默认单位人工确认，并处置 22 条空规格和待审核名称候选 |
...
25. A118/V700 已进入服务器本地待审核队列，但两批次均缺单位，22 行缺规格，V700 名称候选仍需确认；在批次级默认单位和逐行异常确认完成前不得批量生成内部物料。A118 XFD 完整内容只以归档原文件为权威，Raw Rows 是有界分析投影。
