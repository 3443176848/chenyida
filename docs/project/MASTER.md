...
| 当前版本 | 自托管源码为`0.1.0-alpha.47`，源码Migration为46/head `0046_runtime_lock_privilege_boundary.sql`。TASK56—TASK82已形成权限、恢复、监控、晋升/回滚控制平面与固定handler仓库闭环；TASK70的V2隔离PG17原子双rename证据保持`VERIFIED_PARTIAL_ONLY`。D-160—D-165历史修复与private锚点保持，TASK70按D-166冻结，未执行真实PG17负测、compiled catalog刷新或正式V3重跑；dump/Volume、fresh-process恢复、host激活和真实UAT仍未证明。D-189已完成最小root运维执行源码，但尚未从其最终commit/tree构建镜像。D-188及更早Web/Worker候选均为`OBSOLETE / STALE / NOT AUTHORIZABLE`；D-188候选只精确匹配旧commit `74fbeeebe95432e5f17e3313b1d14b273a91f7b9`/tree `db1edef51e21e69bd7571ef0f765e602c940fec9`，不含D-189 Worker/Migration/Compose内容，禁止启动或拼接复用。并行非生产UAT仍为Web alpha.42/source`569aa954d764309e239d1f6c174e582596d33a24`与PostgreSQL 40/head`0040_warehouse_receipt_readiness.sql`，0041—0046未应用 |
| 当前 Branch | 根仓库 `main` |
...
| 当前根仓库运维基线 | TASK92早先仅清理未使用BuildKit cache，三次受控清理由174项/10.31GB降为0。D-189测试收口仍为6容器、77镜像、277 Volume和6网络；14:06 CST根盘available `14,833,958,912`B、MemAvailable `1,968,537,600`B、Swap `388,296,704`B/1GiB、Load`0.18/0.17/0.21`，Memory PSI/kernel OOM0。四服务running、restart0/OOM false，Web/PostgreSQL healthy，四保护卷存在；`chenyida-erp-uat-*`任务容器、Compose临时目录和D-189 pyc残留0。没有运行prune、常驻服务重启、数据库、Migration或业务动作 |
| 当前同机UAT隔离基线 | D-177—D-187的历史合同与简化信任边界保持。D-189已在仓库内实现并测试第四层operations Compose、root orchestrator/system port、数据库operation CLI/operator和Migration执行合同，固定授权/单一受信package摘要门、PostgreSQL-only、技术角色、短期grant、`EMPTY→0046`、live fence/ledger、事务内unfence/最终ACL及失败收容。按D-187比例原则删除重复全包校验和内存`compile/exec`，固定路径装载不写pyc；Worker只复制实际需要的3份operations JSON。六Secret、实际Docker身份和现有四服务/四保护卷在关键边界精确重验。D-188 Web/Worker manifest/config及resolved Compose `f9ec23b4…68e99`因不含这些变更而废止；Secret、动态数据库/manifest、异故障域恢复、新候选和部署授权均未建立。新UAT未创建，L2a部署NO-GO |
...
| 当前阶段 | `TASK92 SAME-HOST B / D-189 ROOT OPERATIONS SOURCE COMPLETE / D-188 CANDIDATES OBSOLETE / NEW BUILD + DEPLOYMENT PENDING / NEW UAT NOT CREATED / PRODUCTION NO-GO`。执行源码已完成，但还没有与最终源码匹配的候选、ELIGIBLE release或部署授权；现有运行面未改变 |
| 当前任务 | 唯一`DOING`为`SELFHOST-SMALL-TEAM-UAT-ISOLATION-PREREQUISITES-92`；D-189关闭了PostgreSQL-only、技术角色、Migration grant、`EMPTY→0046`、最终ACL及失败收容的仓库源码缺口。当前仍缺从D-189最终commit/tree重新构建匹配候选/部署包、Secret实物、动态数据库身份、现有UAT异故障域恢复、资源门和部署授权 |
| 下一任务 | 仍在TASK92：在新的明确授权下，从D-189最终独立提交串行重建Web/Worker候选，冻结同commit/tree的四层resolved Compose和ELIGIBLE部署包；不自动创建Secret、数据库或UAT，不运行Migration/deploy。构建后仍须补齐恢复/动态门并另获部署执行授权 |

## 当前完成模块

以下模块已有可运行代码或已完成治理交付，但“已实现/已完成”不代表已达到 V2、审计或生产成熟度标准：

- `SELFHOST-SMALL-TEAM-UAT-ENVIRONMENT-READINESS-91`已按D-172完成[新建隔离UAT L1只读核对](../uat/small-team-v1/environment-readiness.md)：源码alpha.47/46项Migration/233表snapshot和Compose静态渲染通过，新UAT固定从`EMPTY → 0046`且不接触现有UAT。TASK92后续恢复磁盘、完成D-173—D-187历史准备/简化、D-188历史候选构建及D-189最小root运维执行源码。D-188候选现已废止；当前由D-189最终源码的新候选/部署包、Secret、动态数据库/manifest、现有UAT异故障域恢复、资源门及明确部署授权阻断，新UAT未创建
...
56. TASK92/D-187按小团队第一性原理冻结同机高级attestation并接受非生产root运维边界；D-188历史构建证明精确候选/静态Compose可行；D-189随后实现最小root运维执行源码，固定PostgreSQL-only、技术角色、短期grant、`EMPTY→0046`、最终ACL及失败收容。D-188候选因不含D-189变更而废止，必须从最终提交重建；Secret、异故障域恢复、动态数据库门和部署授权未建立，新UAT仍未创建。

## 当前任务与下一任务

- 当前唯一`DOING`为`SELFHOST-SMALL-TEAM-UAT-ISOLATION-PREREQUISITES-92`；D-189最小root运维执行源码和测试已完成，D-188候选已废止。新UAT、Secret、数据库及账号均未创建，现有运行面不变。TASK70保持冻结，AI和高级控制面不恢复。
- 下一步仍在TASK92：另获授权后从D-189最终commit/tree串行重建Web/Worker候选及四层部署包。该构建准备不自动部署；后续仍须Secret、异故障域恢复、动态数据库门、新鲜资源门和新的明确部署授权。
