...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE2-TASK05` 计划使用 `docs: design material import workspace ui`；实际哈希以 `git log -1` 为准，提交前基线为 `73435a3` |
...
| 当前阶段 | Phase 2 Material Import Workspace UI V1 书面规格已完成，16 项决定仍为 `PROPOSED`；生产环境未改变 |
| 当前任务 | `PHASE2-TASK05` 设计 Material Import Workspace UI V1，状态 `DONE / AWAITING SPECIFICATION CONFIRMATION` |
| 下一任务 | 停止并等待项目负责人回复“规格确认”；之后先独立设计/实现 Mapping Target Catalog 只读兼容 API，完整 UI 实施仍受 catalog 与 50×256 验收门禁限制 |
...
- Material Import Workspace UI V1 书面设计：三条路由、状态驱动 Stepper、文件预检/Worker SHA/受控 XHR、解析轮询/取消、Sheet/Rows/Header、三列 Mapping 保存/预览/确认、22 个线框状态、集中状态矩阵和 100 项未来实施测试已形成；16 项决定仍为 `PROPOSED`，未实施运行时代码
...
- Material Import Workspace 完整 Mapping 编辑器缺少权威只读 target catalog，且 50×256 Rows 尚未完成性能与可访问性验收；两项均为后续实施硬门禁
...
21. 当前 Mapping API 不返回完整动态目标 catalog，且没有独立权威 metadata 读取接口；禁止用 seed、测试数据、历史 Mapping 或前端硬编码绕过。50×256 表格也尚未通过性能和可访问性门禁，完整 UI 实施必须先完成两项独立验收。
...
- 已完成：`PHASE2-TASK05` Material Import Workspace UI V1 正式规格、22 状态线框、状态矩阵、100 项未来实施测试和 16 项 `PROPOSED` 决定；仅文档，未修改运行时、API、Schema、Migration 或生产配置。
- 下一：停止并等待“规格确认”；之后必须先独立完成 Mapping Target Catalog 只读兼容 API 的设计/实现/测试/OpenAPI，再按 50×256 性能与可访问性门禁实施完整 UI。生产 Queue/binding、生产迁移/部署、清洗/匹配/Material Draft、`submitted_by` 候选项、`PENDING_APPROVAL` 收缩和 `PHASE0-TASK03` 均需独立授权。
