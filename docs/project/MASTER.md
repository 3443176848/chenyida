...
| 当前版本 | 自托管源码为`0.1.0-alpha.47`，源码Migration为46/head `0046_runtime_lock_privilege_boundary.sql`。TASK56已按D-133/D-134完成权限/恢复/受控operator仓库闭环，TASK59—TASK60按D-135/D-136完成独立detached候选和创建前reservation，TASK61—TASK65按D-137—D-141完成monitor交付、权威投影、V2恢复政策/激活及target-bound egress，TASK66—TASK79再按D-142—D-154固定授权矩阵、跨岗UAT证据、晋升审计、内容寻址事务BEGIN/RECOVER、promotion-bound四域snapshot、精确Compose writer持续静默、Migration批准/提交、Compose部署、postdeploy checkpoint 10/11、cross-role checkpoint 12、promotion final checkpoint 13及rollback checkpoint 14/15控制平面。真实rollback runtime adapter仍故意缺失并在授权消费前失败。TASK57的Web/Worker本机候选以及当前安全仓库变化前的镜像均为`STALE / NOT AUTHORIZABLE`；当前没有与最终源码匹配的可授权镜像或正式19步门。并行非生产UAT仍为Web alpha.42/source`569aa954d764309e239d1f6c174e582596d33a24`与PostgreSQL 40/head`0040_warehouse_receipt_readiness.sql`；0041—0046未应用，真实角色/secret/Volume、外部锚点、UAT晋升和正式门均未执行 |
| 当前 Branch | 根仓库 `main` |
...
| Git 同步与工作区 | D-108的`GIT PRIVATE RECOVERY ANCHOR ESTABLISHED`保持。TASK79 feature source`1015b53ec1e0c90cc1ed4e9761255c204ad866f4`/tree`d8dc52cb0b88a1c4f3cdad505a3131924b99afa1`→Supervisor manifest-only`cd9c9dee3bcf6aa859f177c699b754a129e2c54f`/tree`e6f035b180ab4be8f1613268b3f5e745ced05cac`形成当前141文件canonical链，manifest raw SHA-256为`e635792db65107d165d443325b1b70c15b325a499fb145dc404df07e2ce4645d`；TASK78及更早清单/镜像只保留历史审计价值并为`STALE / NOT AUTHORIZABLE`。项目负责人既有未跟踪`docs/ERP_CURRENT_STATUS_REPORT.md`保持不读、不改、不提交；未fetch/push或改动远端，公开origin继续禁止接收内部历史 |
...
| 当前阶段 | `PRODUCTION READINESS CONTINUOUS DELIVERY / CHECKPOINT 4—15 CONTROL PLANE COMPLETE / TRUSTED ROLLBACK RUNTIME ADAPTER OPEN / ACTUAL HUMAN UAT ABSENT / EXECUTOR BLOCKED FAIL-CLOSED / BUSINESS APPROVAL PENDING / LOCAL IMAGE CANDIDATE STALE / RESOURCE STOP LINE ACTIVE / PRODUCTION NO-GO`。TASK79/D-154已闭合checkpoint 14/15执行包、独立授权、逐阶段/逐检查结果、ROLLED_BACK终态和保全恢复；机器审计为15项SUPPORTED，但真实runtime adapter、隔离回退演练和人工UAT三项动态条件仍阻断 |
| 当前任务 | 唯一`DOING`为`SELFHOST-UAT-PROMOTION-ROLLBACK-RUNTIME-ADAPTER-80`：从TASK79最终`cd9c9de`链核对现有恢复/部署/probe工具并实现受信、最小权限、无shell、unknown不重跑的runtime adapter协议。只做仓库/fake-root，不访问或修改真实UAT/数据库/Volume |
| 下一任务 | TASK80完成受信runtime adapter后，合成Compose/隔离PostgreSQL动态验收TASK70仍须等待Swap≤80%；最终安全仓库变化收口后统一重建alpha.47镜像和授权包。真实target/凭据、host安装/政策激活、账号/systemd/网络/告警、A1/A3及真实异机恢复/UAT/数据/员工/切换仍须专项授权 |
...
- 当前唯一`DOING`为`SELFHOST-UAT-PROMOTION-ROLLBACK-RUNTIME-ADAPTER-80`：TASK79/D-154已完成checkpoint 14/15控制平面并故意让缺失生产adapter在授权消费前失败。当前从`cd9c9de`严格核对cluster recovery、四域restore、Compose deployment、postdeploy probe及identity工具，建立固定binary/argv、最小权限、阶段化PREPARE/EXECUTE/CONTAIN/PROBE和unknown不重跑的受信adapter；只做仓库/fake-root。Swap停止线有效，TASK70动态Compose/隔离PostgreSQL验收保持BLOCKED，不修改UAT，系统继续`PRODUCTION NO-GO`。
