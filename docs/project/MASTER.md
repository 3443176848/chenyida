...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | `PHASE3-MATERIAL-LIBRARY-STRUCTURED-SPEC-MATCH-01`，实际提交以 `git log -1` 为准 |
...
| 当前数据库 | 服务器本地 SQLite 29 张业务/迁移表，已应用本地 `0001`/`0002`/`0003`；在线 D1/Drizzle 开发 schema 为 45 张表，最新 `0008` 尚未执行生产迁移 |
| 当前开发环境 | OpenCloudOS / Python 3.11 项目虚拟环境 / systemd；Site 历史工具链保留 |
| 当前阶段 | 1928C 已改为分项规格提取和逐属性匹配；现有清洗数据未重写 |
| 当前任务 | `PHASE3-MATERIAL-LIBRARY-STRUCTURED-SPEC-MATCH-01` 已完成并部署 |
| 下一任务 | 项目负责人清空当前旧结果后重新导入 1928C；对内部库未覆盖的 10PF 等规格人工建档生成编号 |
