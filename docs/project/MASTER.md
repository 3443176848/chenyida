...
快照时间：2026-07-26（Asia/Shanghai）

| 项目 | 当前值 |
| --- | --- |
| 当前版本 | 源码与并行环境均为 `0.1.0-alpha.22`；PostgreSQL migration head 为 `0022_production_quality_release.sql`；始终仅限回环非生产环境 |
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | TASK08 功能提交 `4a638522b7ca295b41d2f35adbc464b23762b007`，严格 Parent 为 `7d9c2dbaf62664e46c4f984822bb43903999f5fd`；独立 ops 验收提交以 Git log 为准 |
| Git 同步与工作区 | 任务起点 `origin/main...HEAD` 为 behind 0/ahead 32；ops 文档提交后仍不 push、不创建 PR、不改写历史 |
...
| 当前数据库 | 源码和并行 PostgreSQL 均为 `0001`—`0022`；最终唯一启用管理员、所有合成业务表 0、uploads/attachments 0。SQLite/D1 未向 PostgreSQL 迁移真实数据 |
...
| 当前阶段 | 第一阶段 TASK01—TASK08 已完成并行验收；生产过程/成品品质关闭后仅形成订单级可发货额度 |
| 当前任务 | `SELFHOST-PHASE4-TASK08` 已 `DONE`。`PHASE0-TASK03`、TASK05、TASK06 和 TASK07 保持历史 `DONE`，不得重复 |
| 下一任务 | 停止；Shipment、销售金额来源、AR、收款、真实迁移、HTTPS、生产恢复和切换均未授权 |
...
- 当前无 `DOING`；`SELFHOST-PHASE4-TASK08` 已完成 Report→IPQC、Completion→Sales Order Allocation→FQC→订单级放行额度的并行验收。
- `PHASE0-TASK03`、`SELFHOST-PHASE4-TASK05`、TASK06 和 TASK07 保持历史 `DONE`；TASK08 功能提交严格基于授权起点 `7d9c2dba`。
- 本轮完成后停止。Shipment、销售金额来源、AR、收款、IQC 库存隔离、批次/序列、返工/报废过账、真实数据迁移和生产部署均未授权。
