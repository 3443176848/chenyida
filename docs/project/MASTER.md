...
| 当前版本 | 自托管源码为`0.1.0-alpha.47`，源码Migration为46/head `0046_runtime_lock_privilege_boundary.sql`。TASK56—TASK82已形成权限、恢复、监控、晋升/回滚控制平面与固定handler仓库闭环；TASK70的V2隔离PG17原子双rename证据保持`VERIFIED_PARTIAL_ONLY`。D-160—D-165历史修复与private锚点保持，TASK70按D-166冻结，未执行真实PG17负测、compiled catalog刷新或正式V3重跑；dump/Volume、fresh-process恢复、host激活和真实UAT仍未证明。TASK57及D-188之前的Web/Worker候选均为`STALE / NOT AUTHORIZABLE`；D-188新候选精确匹配其冻结源码commit `74fbeeebe95432e5f17e3313b1d14b273a91f7b9`/tree `db1edef51e21e69bd7571ef0f765e602c940fec9`，后续治理文档提交不属于该镜像build context。候选仅为本机build-preparation制品，尚无外部恢复锚点、ELIGIBLE manifest或部署授权。并行非生产UAT仍为Web alpha.42/source`569aa954d764309e239d1f6c174e582596d33a24`与PostgreSQL 40/head`0040_warehouse_receipt_readiness.sql`，0041—0046未应用 |
| 当前 Branch | 根仓库 `main` |
...
| 当前根仓库运维基线 | TASK92早先仅清理未使用BuildKit cache，三次受控清理由174项/10.31GB降为0。D-188精确构建后12:12 CST收口为6容器、77镜像、277 Volume、6网络和46项/2.431GB Build Cache（active 0），新增对象恰为Web/Worker两候选；根盘available `14,991,380,480`B，MemAvailable `2,112,208,896`B，Swap `327,303,168`B，Load`0.44/0.57/0.56`，PSI/OOM0，均高于停止线。四服务ID、restart0/OOM、Web/PostgreSQL health及四保护卷不变；构建专用临时registry/provenance容器和detached worktree已清零。未运行任何prune、服务重启或业务动作 |
| 当前同机UAT隔离基线 | D-177—D-187的历史合同与简化信任边界保持。D-188已从固定commit `74fbeeebe95432e5f17e3313b1d14b273a91f7b9`/tree `db1edef51e21e69bd7571ef0f765e602c940fec9`串行构建精确alpha.47 Web/Worker本机候选；manifest/config分别为`42b41540…40ffd`/`d4da6cba…c8dd3`和`861d71ae…74b9b`/`bd34dfd2…227c1`。root-only规范化resolved Compose `f9ec23b4…68e99`、非Secret render env、实际`uat-edge`服务集和第一阶段无Volume删除回退输入已冻结。静态等值门通过，但现有Compose仍缺Migration grant挂载/环境、技术角色bootstrap和Migration后ACL最小执行接线；Secret、动态数据库/manifest、异故障域恢复和部署授权也未建立。新UAT未创建，L2a部署NO-GO |
...
| 当前阶段 | `TASK92 SAME-HOST B / D-188 BUILD PREPARATION COMPLETE / DEPLOYMENT PACKAGE INCOMPLETE / NEW UAT NOT CREATED / PRODUCTION NO-GO`。精确本机候选和静态Compose/回退输入已冻结；这不是外部镜像恢复锚点、ELIGIBLE release或部署授权，现有运行面未改变 |
| 当前任务 | 唯一`DOING`为`SELFHOST-SMALL-TEAM-UAT-ISOLATION-PREREQUISITES-92`；D-188关闭了“缺D-188冻结候选源码的精确镜像/静态Compose”阻断，但确认PostgreSQL-only启动、技术角色bootstrap、Migration grant/manifest接线和Migration后ACL reconcile仍缺最小root运维执行包。另缺Secret实物、动态数据库身份、现有UAT异故障域恢复、资源门和部署授权 |
| 下一任务 | 仍在TASK92：下一独立切片只实现并测试最小root运维执行包，连接PostgreSQL-only启动、技术角色初始化、受控Migration grant/manifest、`EMPTY→0046`和最终ACL；不自动创建Secret、数据库或UAT，不运行Migration/deploy。执行包完成后仍须另获部署授权并补齐恢复/动态门 |

## 当前完成模块

以下模块已有可运行代码或已完成治理交付，但“已实现/已完成”不代表已达到 V2、审计或生产成熟度标准：

- `SELFHOST-SMALL-TEAM-UAT-ENVIRONMENT-READINESS-91`已按D-172完成[新建隔离UAT L1只读核对](../uat/small-team-v1/environment-readiness.md)：源码alpha.47/46项Migration/233表snapshot和Compose静态渲染通过，新UAT固定从`EMPTY → 0046`且不接触现有UAT。TASK92后续恢复磁盘、完成D-173—D-187历史准备/简化，并由D-188构建精确本机候选及冻结静态Compose/回退输入。当前由最小Migration/角色/ACL执行包、Secret、动态数据库/manifest、现有UAT异故障域恢复、资源门及明确部署授权阻断；新UAT未创建
...
56. TASK92/D-187按小团队第一性原理冻结同机高级attestation并接受非生产root运维边界；D-188随后按明确构建准备授权，从固定commit/tree构建精确Web/Worker本机候选并冻结root-only resolved Compose和第一阶段回退输入。静态候选/隔离等值门通过，但Migration grant挂载/环境、技术角色bootstrap和Migration后ACL最小执行接线仍缺，且Secret、异故障域恢复、动态数据库门和部署授权未建立。新UAT仍未创建。

## 当前任务与下一任务

- 当前唯一`DOING`为`SELFHOST-SMALL-TEAM-UAT-ISOLATION-PREREQUISITES-92`；D-188精确候选与静态Compose/回退输入已完成，但只属于构建准备。新UAT、Secret、数据库及账号均未创建，现有运行面不变。TASK70保持冻结，AI和高级控制面不恢复。
- 下一步仍在TASK92：另行实现并测试PostgreSQL-only启动、技术角色bootstrap、ELIGIBLE manifest/Migration grant和Migration后ACL的最小root运维执行包。该代码/运维准备不自动部署；后续仍须Secret、异故障域恢复、动态数据库门、新鲜资源门和新的明确部署授权。
