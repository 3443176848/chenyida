...
| 当前版本 | 自托管源码为`0.1.0-alpha.47`，源码Migration为46/head `0046_runtime_lock_privilege_boundary.sql`。TASK56已闭合Web行锁及Backup control/capture/零large-object边界，并把legacy D1 Parser、Worker lease writer、完整后台队列、初始Mapping发布和normalization暂存写入移出Web源码图；文件图候选与唯一reviewed exclusion又分别内容寻址，职责组统一为`*_priv`。Web最终权限相较复核前精确减少25个表操作和9个sequence USAGE，access intent现为`b2defe95…3aa1`。D-132 v1实际恢复readiness也已在V4所有入口及Dashboard失败关闭，v1 synthetic只保留历史解析且永不ready。固定PG17.10/libc C/UTF8的精确编译catalog现已覆盖234表、211序列、394 routine、6独立type、3 extension以及3132列、1709约束、957索引和285个非内部trigger，31类unsupported计数全零并拒绝同owner越界对象；catalog文件/制品/逻辑摘要分别为`4ca22dfa…1162`、`93af15b7…7674`、`40c8c620…7f8f`。catalog源码`8675efd28ed8b61900fb49f7644541103f5f60b0`与manifest-only直接子提交`633b42dca48393d7f24d48808c9046e0d2bd8fc4`已形成50文件Supervisor检查点，bundle SHA-256为`baf820f4…ddc1`；它只解除结构证据前置条件，v2角色/ACL policy与reconciler、角色实施后的TASK56最终bundle及候选镜像仍未完成。TASK55及更早bundle、TASK51本机候选/诊断均为`STALE / NOT AUTHORIZABLE`。并行非生产UAT Web仍为`0.1.0-alpha.42`原镜像`sha256:e7761e2c61bfe77c6aab526fb0b6cbd840ad1bf6300381f4319f6e279af94964`，source revision仍为`569aa954d764309e239d1f6c174e582596d33a24`，UAT PostgreSQL仍为40/head `0040_warehouse_receipt_readiness.sql`；0041—0046未应用到UAT。当前运行面仍是受控非生产UAT，不是生产发布、真实公司数据迁移或切流 |
| 当前 Branch | 根仓库 `main` |
...
| Git 同步与工作区 | D-108的`GIT PRIVATE RECOVERY ANCHOR ESTABLISHED`保持。TASK56 catalog源码`8675efd28ed8b61900fb49f7644541103f5f60b0`/tree`21556c6695b5b49a62959797b1adcb3b116387ef`与只改canonical manifest的直接子提交`633b42dca48393d7f24d48808c9046e0d2bd8fc4`/tree`241f808e73464275fc8472a92f35e9254ef9522b`形成50文件bundle，manifest SHA-256为`baf820f4d1647e427cae1409c5a3797edc4b38fa8eefa2d56c669c4c2094ddc1`；角色/ACL后续源码会使该检查点过期并要求新的TASK56最终bundle，TASK55及更早链仅保留历史审计价值。项目负责人既有未跟踪`docs/ERP_CURRENT_STATUS_REPORT.md`保持不读、不改、不提交；未fetch/push或改动远端，公开origin继续禁止接收内部历史 |
...
| 当前阶段 | `PRODUCTION READINESS CONTINUOUS DELIVERY / RUNTIME DATABASE PRIVILEGE CLOSURE IN PROGRESS / ISOLATED-ONLY / PRODUCTION NO-GO`。TASK56只在仓库和隔离环境收紧未来运行候选，不证明当前UAT权限或秘密已改变 |
| 当前任务 | 唯一`DOING`为`SELFHOST-OPS-POSTGRES-RUNTIME-PRIVILEGE-56`，严格起点`fb1f7e8893b2affba0ca07ecd9629ae2726adca9`/tree`13fe6ce3d04b60bbc724f63b9fa7b5bdc5d16d3e`；D-132 v1 actual readiness P0及PG17精确编译catalog已闭合，源码/manifest检查点为`8675efd…`→`633b42d…`，干净源码PG17门84/401及独立catalog重建通过，当前执行v2角色/ACL policy与reconciler；主智能体唯一写入，三条智能体线只读审计 |
| 下一任务 | 使用已冻结PG17 catalog完成owner/migration、Web、Worker、Backup及Admin完整角色/ACL reconcile与五个物理身份正负探针，再闭合secret-file delivery、连接身份断言、受控operator和custom tablespace持久mount仓库合同，最后重建release bundle；真实角色/凭据/Volume、host和UAT部署仍按A4/A1/A6专项授权 |
...
- `SELFHOST-PRODUCTION-READINESS-40`确认的首要风险继续开放。TASK41/TASK54/TASK55已关闭四域内层、签名密文外层及cluster security/tablespace恢复的仓库/合成隔离缺口，但PostgreSQL及三个文件数据域仍没有真实异机当前锚点、真实恢复或RTO；当前Compose仍以共享初始化superuser、环境变量秘密和superuser备份/恢复operator运行，且没有获批custom tablespace持久mount。源码alpha.46/0045与UAT alpha.42/0040仍未闭合。TASK43—TASK55虽已关闭多项仓库合同缺口，当前源码匹配候选镜像、正式Supervisor镜像证据/19步同候选release gate、host告警投递、UAT部署和真实数据门仍阻止发布。详见[投产准入基线](PRODUCTION_READINESS.md)
...
- `SELFHOST-OPS-POSTGRES-RUNTIME-PRIVILEGE-56`是唯一`DOING`。源码已推进到alpha.47/0046：16个owner控制窄函数覆盖Web原有19个锁目标，Web保持零table/column UPDATE，20个locking trigger固定owner安全执行路径；PG17完整回归84文件/401项及专项5/5通过。Backup又以独立control/capture、精确CONNECT围栏、v3中断恢复和零large-object dump闭合。后续审计把共享Parser错误合同与四类Worker-only写实现从Web模块结构中拆出；Web raw candidate自然收敛为`9/191/205/79`，只排除既有`app_meta INSERT`并叠加5项routine依赖后最终为`9/190/209/79`，sequence USAGE `182→173`，同时统一`*_priv`职责组。D-132 v1实际readiness入口已失败关闭。固定PG17.10的精确结构编译器现以双次相同结果冻结234表、211序列、394 routine、6独立type、3 extension及3132列/1709约束/957索引/285非内部trigger，31类unsupported计数全零；同owner rogue table、真实用户rule、view `_RETURN`语义、TOAST reloptions、extension同地址owner漂移、未知extension成员class及不安全`search_path`等负测均失败关闭。目录文件/制品/逻辑SHA-256分别为`4ca22dfa…1162`、`93af15b7…7674`、`40c8c620…7f8f`。该证据不含角色/ACL授权，下一步使用目录实现v2 policy/reconciler与default/extension ACL精确收敛。只读UAT仍是共享superuser、alpha.42/0040与环境变量秘密，任务未修改当前UAT/生产。
...
- `SELFHOST-RELEASE-CANDIDATE-REFRESH-51`已完成并释放active slot：当前本机候选精确绑定`8084d6c3`/tree`a54473f6`，六服务runtime policy与固定Trivy零发现诊断通过；正式镜像证据和19步门因installed supervisor缺失在写入前失败关闭，UAT未改变。岗位权限矩阵须业务负责人批准后另立任务，host supervisor安装、真实异机备份/恢复、真实告警、账号变化、员工试用和切换仍须专项明确授权。


- `SELFHOST-OPS-BACKUP-RECOVERY-V2-41`内层四域工具由TASK54/TASK55继续复用；TASK56已替代其单一superuser/connection-limit guard为双身份零large-object与CONNECT围栏合同。D-115/D-131/D-132的合成隔离链保持，G2真实异机备份/恢复仍因目标、密钥、RPO/RTO与专项授权阻塞；当前TASK56继续未来Compose独立最小数据库角色、secret-file delivery、受控operator与custom tablespace持久mount合同，仍不build/deploy或读取当前卷。
- 真实异机备份与恢复、UAT build/Migration/deploy、旧数据读取、员工试用、账号/权限、网络/systemd和正式切换仍须专项明确授权；持续交付授权不改变这些边界。

- 项目负责人已接受D-113与D-114；`AGENT-R1`只读控制器、`PM-002`执行设计和`AGENT-R1-5`合成docs/test协议MVP均已完成。该治理任务链收口时回到`IDLE`，未自动启动R2或恢复TASK03；当前正式active slot仅为TASK56。
