...
| 当前版本 | 自托管源码为`0.1.0-alpha.47`，源码Migration为46/head `0046_runtime_lock_privilege_boundary.sql`。TASK56已按D-133/D-134完成权限/恢复/受控operator仓库闭环，TASK59—TASK60按D-135/D-136完成独立detached候选和创建前reservation，TASK61—TASK65按D-137—D-141完成monitor交付、权威投影、V2恢复政策/激活及target-bound egress，TASK66—TASK73再按D-142—D-148固定授权矩阵、跨岗UAT证据、晋升审计、内容寻址事务BEGIN/RECOVER、promotion-bound四域snapshot、精确Compose writer持续静默及一次性Migration批准回执。TASK57的Web/Worker本机候选以及TASK73及更早Supervisor bundle都会因后续Site输入变化成为`STALE / NOT AUTHORIZABLE`；当前没有与最终源码匹配的可授权镜像或正式19步门。并行非生产UAT仍为Web alpha.42/source`569aa954d764309e239d1f6c174e582596d33a24`与PostgreSQL 40/head`0040_warehouse_receipt_readiness.sql`；0041—0046未应用，真实角色/secret/Volume、外部锚点、UAT晋升和正式门均未执行 |
| 当前 Branch | 根仓库 `main` |
...
| Git 同步与工作区 | D-108的`GIT PRIVATE RECOVERY ANCHOR ESTABLISHED`保持。TASK73最终source`32860b86be13cab880b5cf0cd8e9cfb255956809`/tree`b950a29944c48a73be78bab730f252b6f5ccf9c4`→monitor manifest-only`18b93e90ecd8f90b084d82596f847e7651aec6ee`/tree`a5967c5bfbe853bb322bb77a11eadecd6a495f33`→Supervisor manifest-only`302661c5d49722c6c4b4bcfe18749417e3688e52`/tree`0a05618b217878c9dd71bb0226bebfb16f5e4a78`形成当前30/128文件canonical链，manifest raw SHA-256为`59ea1084…7c0`/`090c3a23…800`；TASK72及更早清单/镜像只保留历史审计价值并为`STALE / NOT AUTHORIZABLE`。项目负责人既有未跟踪`docs/ERP_CURRENT_STATUS_REPORT.md`保持不读、不改、不提交；未fetch/push或改动远端，公开origin继续禁止接收内部历史 |
...
| 当前阶段 | `PRODUCTION READINESS CONTINUOUS DELIVERY / MIGRATION COMMIT RECEIPT ADAPTER / EXECUTOR BLOCKED FAIL-CLOSED / BUSINESS APPROVAL PENDING / LOCAL IMAGE CANDIDATE STALE / RESOURCE STOP LINE ACTIVE / PRODUCTION NO-GO`。TASK73/D-148已闭合checkpoint 7一次性Migration批准且在SQL前失败关闭，机器审计当前为9项SUPPORTED、6项阻断；正在接入checkpoint 8数据库围栏、逐文件执行结果与提交回执 |
| 当前任务 | 唯一`DOING`为`SELFHOST-UAT-MIGRATION-COMMIT-74`：从TASK73最终`302661c`链核对独立执行授权、数据库CONNECT/read-only围栏、角色/session、`schema_migrations`、逐文件结果与journal，关闭checkpoint 8。只做仓库/fake-root/可注入client，不连接真实数据库或运行Migration |
| 下一任务 | TASK74后按机器依赖接入一次性Compose deploy/postdeploy和rollback adapter；合成Compose/隔离PostgreSQL动态验收TASK70在Swap≤80%且执行器完整前保持BLOCKED。最终安全仓库变化收口后统一重建alpha.47镜像和授权包。真实target/凭据、host安装/政策激活、账号/systemd/网络/告警、A1/A3及真实异机恢复/UAT/数据/员工/切换仍须专项授权 |
...
- 当前唯一`DOING`为`SELFHOST-UAT-MIGRATION-COMMIT-74`：TASK73/D-148已完成`ONE_TIME_MIGRATION_AUTHORIZATION`并把审计收敛到6项阻断；当前从`302661c`严格链接入独立`RUN_UAT_PROMOTION_MIGRATION`、数据库连接/角色/session围栏、逐文件结果、`MIGRATION_COMMIT_RECEIPT`和partial恢复。Swap停止线有效，只做仓库静态、Python、受限Node及可注入client验证；TASK70动态Compose/隔离PostgreSQL验收保持BLOCKED，不修改UAT，系统继续`PRODUCTION NO-GO`。
