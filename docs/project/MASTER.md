...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | `PHASE3-MATERIAL-LIBRARY-EXCEL-COMPAT`，实际提交以 `git log -1` 为准 |
...
| 当前数据库 | 服务器本地 SQLite 29 张业务/迁移表，已应用本地 `0001_material_import_source_lineage`；在线 D1/Drizzle 开发 schema 为 45 张表，最新 `0008` 尚未执行生产迁移 |
| 当前开发环境 | OpenCloudOS / Python 3.11 项目虚拟环境 / systemd；Site 历史工具链保留 |
| 当前阶段 | 服务器本地 CSV/XLSX/XLS 自适应导入已部署；等待用户网页实测 |
| 当前任务 | `PHASE3-MATERIAL-LIBRARY-EXCEL-COMPAT` 已完成；真实数据任务仍因 A118 超宽异常块和 V700 缺少名称/单位语义而 `BLOCKED` |
| 下一任务 | 用户刷新公网网页并用具备明确名称/规格列的 XLSX/XLS 验收；同时由数据所有人修复 A118、确认 V700 名称/单位来源 |
