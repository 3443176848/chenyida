...
| 当前根仓库运维基线 | `SELFHOST-OPS-TRUSTED-ORIGIN-05` 起点为 `f3092ee`；独立提交消息为 `fix: trust configured public origin behind TLS`，实际提交 SHA 以 Git log 为准。此前管理员、数据库凭据轮换和资源保护基线保持不变 |
| Git 同步与工作区 | TRUSTED-ORIGIN-05 起点为本地 `main`/`f3092ee`、领先 origin 86；只提交来源校验代码、测试和脱敏文档。未 push/PR/改写历史；受保护未跟踪 `shujvbiao/` 未修改、暂存、打开或提交，密码、摘要、Cookie、Token、凭据和业务数据未进入 Git |
...
| 当前数据库 | 源码 migration 为 `0001`—`0035`，并行常驻 PostgreSQL 仍为 `0001`—`0034`；34 条 checksum 与基线 manifest `b2ff69f7...13b8b` 一致。当前用户/active admin 为 `2/2`；`admin2` active、version 2、已成功登录但仍为首次改密状态。Session/有效 `3/1`，Audit/Identity `887/15`，身份幂等 3；本任务两次无凭据公网验收各新增一条合法 `AUTH_REQUIRED` Identity Audit。Material/Product/BOM/Line 仍为 `532/6/6/316`，交易事实 0 |
| 当前运行状态 | `https://43.135.157.211.nip.io:18888` 经 Caddy 可信 TLS 到 Web；唯一允许的浏览器来源由 `ERP_PUBLIC_ORIGIN` 精确固定。Web 为基于 alpha.34 基线的来源校验 hotfix 镜像 `sha256:f9c34a11b900...`，仍只绑定 `127.0.0.1:3000`；PostgreSQL 无宿主端口。PostgreSQL/Web/Worker/Caddy 为 healthy/healthy/running/running、restart 0/OOM false；PostgreSQL、Worker、Caddy 容器未重建，旧 Python 保留在 `127.0.0.1:18889` |
...
| 当前阶段 | `SELFHOST-OPS-TRUSTED-ORIGIN-05` 已完成 Caddy TLS 终止场景的显式公网 Origin 修复与 alpha.34 Web hotfix 更新；常驻数据库/Worker 保持 alpha.34/0034，源码仍为 alpha.35/0035 |
| 当前任务 | `SELFHOST-OPS-TRUSTED-ORIGIN-05` 已 `DONE / DEPLOYED / USER FIRST-CHANGE RETRY REQUIRED`；合法公网 Origin 已越过来源门禁，错误来源、缺失来源和错误 CSRF 继续 fail closed；Web/数据/Migration/资源与临时资源均已核对 |
| 下一任务 | 停止，不自动执行其他任务。项目负责人应刷新页面，用 `admin2` 当前会话重新完成首次改密；V9 单位/BOM 契约、0035 部署、历史物料修订、治理 UI、公司域名和备份异机复制仍需独立任务与授权 |
...
- `admin2` 已创建但仍处于首次改密门禁；在项目负责人完成首次改密前，只能访问 Session/Logout/本人改密。创建时曾输出的旧摘要已通过正式重置生成新盐而失效，但该安全事件必须保留在审计与任务记录中，不得删除证据或把临时密码当长期密码。
...
- 常驻 18888 Web 已运行 alpha.34 基线+Origin hotfix，Worker/PostgreSQL 保持 alpha.34/0034，Caddy 未重建。0035 未应用，未读取、回填或重算真实 BOM；Origin 隔离测试库和 runner 已删除，四个受保护卷均保留。
...
- 本轮完成后停止，不自动启动任何后续任务。项目负责人下一步只需刷新当前页面，用 `admin2` 重新提交首次改密；`0035` 常驻/生产迁移与部署、真实 BOM 治理、历史异常物料修订、正式替代料审批、生产领料 Lot、FIFO/FEFO、序列号、设备/OEE、外协、产能、工时/成本均未授权。
