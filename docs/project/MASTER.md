...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE2-TASK03` 文档任务计划使用 `docs: design material import parser and mapping`；实际哈希以 `git log -1` 为准，提交前基线为 `63e0483` |
...
| 当前数据库 | 本地 SQLite 26 张业务表；在线 D1 现有 8 张表，开发迁移另新增 12 张 V2 表、2 张 Material API 安全表及 5 张 Material Import 表；尚未执行生产迁移 |
| 当前开发环境 | Windows / PowerShell 5.1 / Git 2.52 / Node 26.3 / npm 11.16 / 项目绑定 Python 3.12.13 |
| 当前阶段 | Phase 2 Excel/CSV Parser 与 Mapping V1 书面规格已完成；运行时和生产环境未改变 |
| 当前任务 | `PHASE2-TASK03` 设计 Excel/CSV Parser 与字段 Mapping V1，状态 `DONE / AWAITING SPECIFICATION CONFIRMATION` |
| 下一任务 | 停止并等待 `PHASE2-TASK03` “规格确认”；任何 `0005`、Parser、Queue、Schema、API 或部署实施均需独立授权 |
...
- Material Import Parser 与 Mapping V1 书面设计：定义 `FILE_READY -> MAPPING_CONFIRMED`、`PARSED` 原子发布恢复点、run 隔离、Outbox 可靠投递、Sheet 级恢复、Shared Strings 有界策略、raw cell/row 契约、Mapping 目标/CAS、七个 API 草案、`0005` 设计、兼容性矩阵和 16 项 `PROPOSED` 决策；未修改运行时代码、Schema、Migration 或部署配置
...
- Material Import Excel/CSV 行解析、Outbox/Queue、表头建议、原始行写入、字段 Mapping API、匹配和 Material Draft 创建尚未实现；`PHASE2-TASK03` 只完成书面规格，实施必须在规格确认后另行授权
...
20. Parser 推荐栈 `zip.js + sax-wasm + 受限 OOXML` 与 `csv-parse` browser ESM 尚未通过 Vinext、Miniflare 或 Workers 兼容测试；Queue/Outbox、Shared Strings 策略、`0005` 和全部资源上限仍为 `PROPOSED`，不得据书面设计宣称可用于生产。
...
- 已完成：`PHASE2-TASK03` 正式书面规格、OpenAPI 草案、流程图、Mapping 规格和 16 项 `PROPOSED` 决策表；定义 Outbox、Sheet 级恢复、原子发布、Shared Strings/总字节预算、Mapping 准备恢复与 `0005` 设计，未实施任何运行时或生产变更。
- 下一：停止并等待项目负责人回复“规格确认”；`0005`、Parser/Mapping API、Queue/R2、生产迁移/部署、`submitted_by` 候选项、`PENDING_APPROVAL` 收缩和 `PHASE0-TASK03` 均需独立任务与授权。
