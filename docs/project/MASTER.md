...
| 项目 | 当前值 |
| --- | --- |
| 当前版本 | 源码与并行环境均为 `0.1.0-alpha.21`；PostgreSQL migration head 为 `0021_production_reporting_completions.sql`；始终仅限回环非生产环境 |
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | TASK07 功能提交 `323e85d44a2a4202811944591d0a4f6b96ae6751`，严格 Parent 为 `26ccb95782478645720c8284c59b0afadca68649`；独立 ops 验收提交以 Git log 为准 |
| Git 同步与工作区 | 功能提交后 `origin/main...HEAD` 为 behind 0/ahead 31；ops 文档提交后仍不 push、不创建 PR、不改写历史 |
...
| 当前数据库 | 源码和并行 PostgreSQL 均为 `0001`—`0021`；最终唯一启用管理员、所有合成业务表 0、uploads/attachments 0。SQLite/D1 未向 PostgreSQL 迁移真实数据 |
...
| 当前阶段 | 第一阶段 TASK01—TASK07 已完成并行验收；生产报工到成品入库交接后停止 |
| 当前任务 | `SELFHOST-PHASE4-TASK07` 已 `DONE`。`PHASE0-TASK03`、TASK05 和 TASK06 保持历史 `DONE`，不得重复 |
...
- 当前无 `DOING`；`SELFHOST-PHASE4-TASK06` 已完成 ACCEPTED Planning Package→版本化生产交接→唯一 DRAFT 工单→显式释放/BOM 快照/需求→齐套预留→仓库分批领退料并行验收。
