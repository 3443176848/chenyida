...
| 项目 | 当前值 |
| --- | --- |
| 当前版本 | 源码与并行环境均为 `0.1.0-alpha.18`；仅部署到回环 `PARALLEL HTTP ACCEPTANCE ONLY`，明确为非生产、尚未正式发布 |
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | TASK04 功能提交 `4506db2579c07080afe27b33bb2e50623c3d1366`；验收提交消息 `ops: accept procurement sourcing workflow in parallel environment` |
...
| 当前数据库 | 自托管源码与并行 PostgreSQL 均为 `0001`—`0018`；TASK04 验收业务已整体恢复清空，只保留 Schema 和唯一管理员。SQLite/D1 未向 PostgreSQL 迁移真实数据 |
