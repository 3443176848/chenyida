...
| 当前任务 | `PHASE0-TASK01-B` 已解除 Site gitlink 并把完整源码纳入根仓库管理 |
| 下一任务 | `PHASE0-TASK02` 建立隔离测试与安全基线 |
...
7. 部分测试可能写入数据或受本地构建缓存影响，隔离与清理机制尚未完成。

## 当前任务与下一任务

- 已完成：`PHASE0-TASK01-B`，把 Site gitlink 转为根仓库直接跟踪的普通目录，保留生产版本、开发基线和提交历史关系；未修改业务代码或生产环境。
- 下一：`PHASE0-TASK02`，建立隔离测试与安全基线，确保测试拒绝生产地址并完成凭证、备份和恢复风险收敛。
- 后续：`PHASE0-TASK03`，统一版本号、生产提交、迁移版本、发布验收和回退记录。
