...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | `41e293f`，`feat: adapt supplier material imports` |
...
| 当前数据库 | 本地 SQLite 26 张业务表；在线 D1/Drizzle 开发 schema 为 45 张表，最新 `0008` 仅在一次性隔离 D1 验证，尚未执行生产迁移 |
| 当前开发环境 | Windows / PowerShell 5.1 / Git 2.52 / Node 26.3 / npm 11.16 / 项目绑定 Python 3.12.13 |
| 当前阶段 | 多供应商自适应导入已完成非生产实现；`PHASE3-MATERIAL-LIBRARY-02` 仍处于 `NO_REAL_DATA_MODE` |
| 当前任务 | 当前无 `DOING`；自适应导入功能提交 `41e293f` 已完成，真实数据任务因未发现真实企业物料文件而 `BLOCKED` |
| 下一任务 | 用户提供真实 `.xlsx/.csv` 和隔离上传目录后，只读 inspect、逐文件脱敏结构报告、人工 Mapping/治理及 dry-run；生产迁移或部署仍需另行授权 |
...
- Material Import 多供应商自适应识别 V1：`0008` 在既有 Parser/Mapping/Normalization 上增加全部可见 Sheet、前 50 行、1～3 行及合并表头评分、集中别名、样本/Profile 加权、多来源规格、Canonical Row、可解释非数据行分类和空规格 Draft 阻断；全量 Node 589/589 及隔离基线通过，未发现真实供应商样本，未接生产
...
25. `0008` 和自适应识别只在一次性 Miniflare D1、合成脱敏 fixture 验证；受控目录没有真实供应商样本。兼容 Down 会保留新增可空列，完整结构回退依赖迁移前快照；真实 Profile 初始化、结构识别召回率、规格误判率和逐行人工修订仍未验收。
...
- 当前受阻：`PHASE3-MATERIAL-LIBRARY-02` 已进入 `NO_REAL_DATA_MODE` 并完成本地只读 inspect、安全 dry-run 汇总、分类/单位/品牌治理状态及 EXACT/HIGH_CONFIDENCE 重复阻断；Node 575/575、build、lint、隔离 API smoke、凭证和临时 SQLite 基线通过。`/opt/erp` 只有已跟踪模板镜像，`/home` 无候选，故未创建首批真实 DRAFT；解除条件为用户提供真实文件和隔离上传目录。
