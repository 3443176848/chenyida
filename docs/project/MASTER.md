...
快照时间：2026-07-15（Asia/Shanghai）

| 项目 | 当前值 |
| --- | --- |
| 当前版本 | 尚未建立统一 ERP 发布版本；在线 Site `package.json` 为 `0.1.0` |
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE2-TASK01` 计划使用 `docs: design material import batch foundation`；实际哈希以 `git log -1` 为准，提交前基线为 `353c6d9` |
...
| 当前阶段 | Phase 2 Material Import Batch Foundation V1 正式书面规格完成，12 项决定全部保持 `PROPOSED`；生产环境未改变 |
| 当前任务 | `PHASE2-TASK01` 设计 Material Import Batch Foundation V1，状态 `DONE / AWAITING SPECIFICATION CONFIRMATION` |
| 下一任务 | 等待项目负责人逐项选择并统一回复“规格确认”；此前不得创建 Schema、Migration、R2、Binding、API、前端或部署配置 |
...
- Material Import Batch Foundation V1 书面设计：定义私有 R2 + D1 元数据边界、单文件 Worker 代理上传、可恢复 Saga、存储/安全双状态机、批次级协调、原始行类型契约、权限、幂等、清理、Migration 方案、OpenAPI 草案和数据流图；12 项决定仍为 `PROPOSED`，未实施任何代码或基础设施
...
- Material Import Batch Foundation 的 12 项决定尚未统一确认；当前 `hosting.json` 的 `r2` 为 `null`，仓库没有可用 R2 binding，任何 Schema、Migration、bucket、binding、上传 API 或解析器均未创建
...
19. Material Import Batch Foundation 目前仅有 `PROPOSED` 规格；10 MiB 上限、30/1095 天保留期、重复导入、跨用户可见性、取消范围、安全扫描与 R2 基础设施均未批准。仓库也无历史文件样本容量证据和 R2 binding，不得把官方平台上限或规格推荐写成现有能力。
...
- 下一：停止并等待项目负责人指定新任务；`submitted_by` 只读筛选 API 候选项、候选索引、`PENDING_APPROVAL` 收缩、生产迁移/部署和 `PHASE0-TASK03` 均需独立任务与授权。
