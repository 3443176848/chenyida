...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE1-TASK04` 功能提交 `feat: add material validation service`；实时哈希以 `git log -1` 为准 |
...
| 当前阶段 | Phase 1 Material Master V2 schema、分类属性模板和独立物料校验服务已完成 |
| 当前任务 | `PHASE1-TASK04` 已完成，当前无 DOING 任务 |
| 下一任务 | 建议另行启动物料草稿/审核写服务，接入事务、审计和乐观锁；`PHASE0-TASK03` 仍保留在台账 |
...
- Material Master V2 独立物料校验：Repository + Rules + Service 三层按 D1 metadata 校验基础字段、四级叶子、必填、类型、单位和枚举；25 个结构化 code、28 个新增校验测试；尚未接入 API 或写服务

## 当前未完成模块

- Material Master V2 创建/审核写事务、应用层引用/有效期校验和生产迁移尚未实现
...
8. V2 schema 和独立物料校验服务已验证但尚未接入 API 或写事务；供应商历史有效期重叠、正式编码、状态转换和审计仍必须由后续应用层事务保证，且不得据本地测试直接迁移生产。
...
- 下一建议：另行设计物料草稿创建与审核写服务，在单一事务中处理属性值、版本、审计、乐观锁和审核阻断；`PHASE0-TASK03` 的发布与迁移追踪基线仍需后续安排。
