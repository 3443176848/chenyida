...
| 项目 | 当前值 |
| --- | --- |
| 当前版本 | 源码与并行环境均为 `0.1.0-alpha.23`；PostgreSQL migration head 为 `0023_sales_delivery_receivable.sql`；始终仅限回环非生产环境 |
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | TASK09 功能提交 `dfda1c5597cc576cd96f495e272e9fc59c851fa4`，严格 Parent 为 `d9ebfb4644bb9e0d07bfbf81d168d7babcd4bdea`；独立 ops 验收提交以 Git log 为准 |
| Git 同步与工作区 | 任务起点 `origin/main...HEAD` 为 behind 0/ahead 34；两次独立提交后为 behind 0/ahead 36，仍不 push、不创建 PR、不改写历史 |
...
- 当前无 `DOING`；`SELFHOST-PHASE4-TASK09` 已完成 CLOSED/RELEASED FQC→Delivery Instruction→Shipment/FQC/成品出库→Sales Source→显式 AR 的并行验收。
- `PHASE0-TASK03`、`SELFHOST-PHASE4-TASK05`—`TASK08` 保持历史 `DONE`；TASK09 功能提交严格基于授权起点 `d9ebfb46`。
- 本轮完成后停止。客户收款、Finance Settlement、银行、总账、税票、收入确认、真实数据迁移和生产部署均未授权。
