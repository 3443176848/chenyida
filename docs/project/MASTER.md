...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库基线提交 | `186c80a`，`feat: add material import normalization`；`PHASE3-TASK03` 开始时仅有用户既有未跟踪 `.obsidian/`，`bub.md` 不存在且本任务未创建或修改二者 |
...
| 当前阶段 | Phase 3 Normalization 后端非生产实现完成；Normalization Review UI V1 正式规格完成，14 项 UI 决定仍为 `PROPOSED`，生产环境未改变 |
| 当前任务 | `PHASE3-TASK03` 已完成 docs-only 设计；独立提交消息为 `docs: design import normalization review ui` |
| 下一任务 | 停止并等待用户回复“规格确认”；未确认前不得实施 Review UI，分类、匹配、Draft、生产迁移或部署仍需另行授权 |
...
- Material Import Normalization Review UI V1 正式规格：统一 Batch 工作区、七步 Stepper、`current_run/latest_attempt` 双轨状态、启动/重试/重跑/取消、Rows/Issues opaque cursor、批次作用域 Row Drawer、37 个线框、状态矩阵和 104 项未来测试；仅文档，14 项决定待“规格确认”，未实施前端或改变 API/Schema/Migration/生产环境
...
- Material Import Normalization Review UI 尚未实施；Row Drawer 无法精确查询“该行全部 Issues”的局部门禁、完整 Run 历史/部分筛选/Batch Pointer 等非阻塞限制及性能与可访问性门禁必须在未来实施任务中保持，不得由前端猜测或无界扫描绕过
...
23. Normalization Review UI 目前只有 docs-only 规格；Issue API 无 `normalized_row_id`/Sheet 精确筛选，Drawer 内完整行 Issue 集合属于局部门禁。完整 Run 历史、Batch Current Pointer、部分筛选和列表候选摘要也未暴露；这些均不得被前端推断。性能与可访问性仍需未来实现后的 1366×768、700px、50 Rows、100 Issues、200 Attributes 与最大 Payload 实测。
...
- 已完成：`PHASE3-TASK03` Material Import Normalization Review UI V1 docs-only 设计；四份正式文档覆盖统一路由、七步 Stepper、启动/轮询/取消、Current/Latest、Rows/Drawer/Issues、37 个线框、104 项测试、局部门禁、性能门禁和 14 项 `PROPOSED` 决定；未实施运行时代码或改变生产环境。
- 下一：停止并等待“规格确认”；确认前不得开始 Review UI 实施。分类、匹配、Material Draft、生产 Queue/binding、生产迁移/部署、`submitted_by` 候选项、`PENDING_APPROVAL` 收缩和 `PHASE0-TASK03` 均需独立授权。
