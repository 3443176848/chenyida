...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库功能基线提交 | `cea940a`，`fix: adapt imports to real supplier BOMs` |
...
| 当前阶段 | A118/V700 真实 BOM 只读验证完成；`PHASE3-MATERIAL-LIBRARY-02` 进入 `REAL_SAMPLE_REVIEW` |
| 当前任务 | 当前无 `DOING`；真实数据任务因 A118 超宽异常块和 V700 缺少必填 Mapping 语义而 `BLOCKED` |
| 下一任务 | 数据所有人重新导出 A118，并确认 V700 标准名称/单位来源后执行人工 Mapping 与隔离 dry-run；生产迁移或部署仍需另行授权 |
...
25. `0008` 已用 A118/V700 两份真实 BOM 做只读结构验证，但样本量仍不足以代表全部供应商。A118 的 XFD 异常块保持阻断，V700 缺少可确认的标准名称/单位；真实 Profile 初始化、更多模板召回率、规格误判率和逐行人工修订仍未验收。兼容 Down 会保留新增可空列，完整结构回退依赖迁移前快照。
...
- 方向变更：根据 D-028，后续默认交付运行面改为服务器本地 `chenyida_erp_app`，默认监听 `127.0.0.1:18888`；`chenyida_erp_site` 不再作为后续新功能的默认整合和部署目标。现有 Site 代码保留，不自动回写本地运行面。
