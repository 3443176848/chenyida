...
| 项目 | 当前值 |
| --- | --- |
| 当前版本 | 源码与并行非生产UAT Web均为`0.1.0-alpha.42`；当前Web完整Image ID为`sha256:e7761e2c61bfe77c6aab526fb0b6cbd840ad1bf6300381f4319f6e279af94964`，source revision为`569aa954d764309e239d1f6c174e582596d33a24`。PostgreSQL仍为40/head `0040_warehouse_receipt_readiness.sql`。这是受控非生产UAT Web-only部署，不是生产发布、真实公司数据迁移或切流 |
| 当前 Branch | 根仓库 `main` |
...
| Git 同步与工作区 | D-108的`GIT PRIVATE RECOVERY ANCHOR ESTABLISHED`已在镜像预检前复核：唯一worktree、clean`main@e1eff533eb7cb38d169f266bdf3a97b0d3dc7e71`、Parent`acdf1de0364e04aef2a860b3ff1148469d978db7`，`recovery-private/main`逐字节同为`e1eff533…e71`且behind0/ahead0；private仓库`3443176848/chenyida-erp-recovery-private`为PRIVATE/ADMIN/main。公开`origin/main=39946f6b854a985b5c19106eaa6c938bddaf9c7c`、behind0/ahead187，HTTPS fetch、SSH push、upstream、remote HEAD和public visibility不变。本阶段六文档提交增量扫描后只普通推送精确最终HEAD到private main，公开origin仍禁止接收内部历史 |
| 镜像恢复预检 | 精确alpha.42 Web镜像`sha256:e7761e2c…f94964`的身份、config/Env/history、OCI provenance、9层及8,112个文件/元数据记录已离线审计；`CONFIRMED_SECRET=0 / POSSIBLE_SECRET=0`，路径穿越、逃逸link和world-writable regular file均0。一次性archive SHA-256为`d7c78654…bea2`且已随唯一任务目录精确删除，不是异机锚点。候选private目标为`ghcr.io/3443176848/chenyida-erp-web`，目标存在性`TARGET EXISTENCE UNRESOLVED — CREDENTIAL REQUIRED`；未login、tag或push镜像 |
...
| 当前阶段 | `DOING / IMAGE_PREFLIGHT / GHCR_CREDENTIAL_REQUIRED`。阶段结论为`ALPHA.42 IMAGE OUTBOUND REVIEW PASSED — GHCR CREDENTIAL REQUIRED / NO IMAGE PUSH`；FIX38仍为`DONE / WEB_ONLY_UAT_DEPLOYED / ZERO_WRITE_REVALIDATED`和`NO UAT RECEIPT`，恢复治理不改变非生产边界 |
| 当前任务 | `SELFHOST-OPS-RECOVERY-FOUNDATION-39`为唯一`DOING`。D-108的Git private锚点已建立；D-109固定候选`ghcr.io/3443176848/chenyida-erp-web`、唯一完整revision tag、private/no-latest/no-overwrite/最小`write:packages`凭据及push后按registry digest回拉合同。当前只完成审计和准入决策，没有GHCR凭据或镜像push，PostgreSQL/文件卷异机锚点未开始 |
| 下一任务 | 只有项目负责人在任务外安全准备仅含`write:packages`的classic PAT并完成不进入聊天、日志、Git或命令参数的GHCR登录后，才可另启独立镜像push阶段；届时先用认证元数据确认package/tag不冲突且visibility为private，push后按registry digest回拉验证。PostgreSQL dump+文件卷异机恢复仍须再立独立阶段；真实Receipt、IQC、AP、Payment、生产、真实迁移或切流继续要求单独明确授权，alpha.41回退锚点和被拒镜像继续保留且不得上传 |
