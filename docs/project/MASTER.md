...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE1-TASK09` 书面设计提交使用 `docs: design material master read ui`；提交前基线为 `4d2f54b`，实际哈希以 `git log -1` 为准 |
...
| 当前阶段 | Phase 1 Material Master 只读管理界面 V1 书面规格已完成；未实施前端，生产仍未迁移或部署 |
| 当前任务 | `PHASE1-TASK09` 设计完成，等待项目负责人回复“规格确认” |
| 下一任务 | 等待规格确认；任何前端实施必须另建任务，候选索引、生产迁移或部署仍需独立授权 |
...
- Material Master 只读页面尚未实施；当前只有 `material-read-ui-v1.md` 和文字线框稿，未经规格确认不得修改前端
...
13. 当前 Site 仍是根页面 iframe + 静态 tab，缺少真正的 `/materials/...` 页面路由、通用分页和可复用组件系统；实施必须先验证 Vinext 深链接和刷新行为，不能退化为 hash。
14. 现有 legacy `api()` 不能正确解析 Material 嵌套错误外壳；后续实施必须形成唯一共享请求边界，不能让列表、详情和历史各自建立 HTTP Client。
...
- 已完成：`PHASE1-TASK09` 书面设计；确认高密度企业表格列表、分区卡片详情和独立 URL 历史页签，形成主规格和文字线框稿；未修改前端、API、schema、migration 或业务服务。
- 下一：等待项目负责人回复“规格确认”。未经确认不得实施页面；查询报告中的候选索引、`PENDING_APPROVAL` 收缩、生产迁移/部署和 `PHASE0-TASK03` 均需独立任务与授权。
