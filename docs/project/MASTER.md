...
| 当前 Branch | 根仓库 `main` |
| 当前根仓库提交 | `PHASE2-TASK06` 计划使用 `docs: design import mapping target catalog`；实际哈希以 `git log -1` 为准，提交前基线为 `d1c6763` |
...
| 当前数据库 | 本地 SQLite 26 张业务表；在线 D1 开发 schema 34 张表，其中 Material Import 为 12 张；`0005` 仅在一次性隔离 D1 验证，尚未执行生产迁移 |
| 当前开发环境 | Windows / PowerShell 5.1 / Git 2.52 / Node 26.3 / npm 11.16 / 项目绑定 Python 3.12.13 |
| 当前阶段 | Phase 2 Mapping Target Catalog V1 只读 API 规格已完成，12 项决定保持 `PROPOSED`；生产环境未改变 |
| 当前任务 | `PHASE2-TASK06` 设计 Material Import Mapping Target Catalog V1，状态 `DONE / AWAITING SPECIFICATION CONFIRMATION` |
| 下一任务 | 停止等待“规格确认”；确认后才能另立 Catalog 非生产实现任务，完整 UI 仍受 Catalog 实现与 50×256 验收门禁限制 |
...
- Material Import Workspace UI V1 书面设计：三条路由、状态驱动 Stepper、文件预检/Worker SHA/受控 XHR、解析轮询/取消、Sheet/Rows/Header、三列 Mapping 保存/预览/确认、22 个线框状态、集中状态矩阵和 100 项未来实施测试已形成；完整规格与 16 项决定已确认，未实施运行时代码
- Material Import Mapping Target Catalog V1 书面设计：推荐批次作用域只读 GET，定义 BASIC/ATTRIBUTE/SPECIAL、运行时 D1 属性、完整 target DTO、共享 digest、统一 cursor、双 capability/行级可见性、no-store、失效目标和 43 项未来实施测试；12 项决定保持 `PROPOSED`，未实施运行时代码
...
- Material Import 的清洗、标准化、自动分类、候选匹配、Material Draft 创建和正式物料写入尚未实现；生产 Queue/binding、生产迁移和部署也仍需独立授权
- Material Import Workspace 完整 Mapping 编辑器的权威只读 target catalog 已完成书面规格但尚未实现；现有 digest 还未抽为 Catalog/save/preview/confirm 共用 Snapshot。50×256 Rows 也尚未完成性能与可访问性验收；两项均为后续实施硬门禁
...
21. 当前 Mapping API 仍不返回完整动态目标 catalog，独立只读 Catalog 仅完成 `PROPOSED` 书面规格。实现审计确认当前 digest 只含基础/供应商 code 与属性 code/type/status，缺少特殊目标、单位/default/必填等共享语义；实施前必须抽取共享 Target Registry + Metadata Snapshot，禁止以第二套 digest、seed、测试数据、历史 Mapping 或前端硬编码绕过。50×256 表格也尚未通过性能和可访问性门禁。
...
- 已完成：`PHASE2-TASK06` Mapping Target Catalog V1 正式规格与 OpenAPI；比较批次/全局/混入 Mapping 三种路由，推荐批次作用域，定义共享 Registry/digest、BASIC/ATTRIBUTE/SPECIAL DTO、统一 cursor、权限/缓存/失效目标边界和 43 项未来测试；12 项决定保持 `PROPOSED`，仅文档。
- 下一：停止等待“规格确认”；确认后才能另立 Catalog 非生产实现任务，再按 50×256 性能与可访问性门禁实施完整 UI。生产 Queue/binding、生产迁移/部署、清洗/匹配/Material Draft、`submitted_by` 候选项、`PENDING_APPROVAL` 收缩和 `PHASE0-TASK03` 均需独立授权。
