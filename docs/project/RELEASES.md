...
| Node.js / PostgreSQL 自托管开发基线 | `0.1.0-alpha.3`；包名 `chenyida-erp-selfhosted` | TASK03 起始基线 `2784a9a064838ebbb76f2bce8c97ebeb1eb8befb`；功能提交通过 `git log -1 -- docs/tasks/SELFHOST-PHASE2-TASK03-completion.md` 解析 | PostgreSQL 17 migration `0001`—`0007` | TASK03 单元/UI/PostgreSQL/migration/Compose restart 与 Identity/Material/Mapping/Normalization/Review/Phase0/Python 回归通过 | `NOT_RELEASED`；没有运行中的 Compose 项目，未生产部署 | SQLite/D1 真实数据、真实用户与主数据均未迁入 PostgreSQL | 未部署版本无线上回退动作；未来部署前必须以现运行面、Git commit、数据库快照和文件快照建立恢复点 | 非生产开发版本，未获发布批准 |
| 自托管生产版本 | 尚不存在 | `N/A` | `N/A` | `N/A` | `NOT_RELEASED` | `NOT_MIGRATED` | `NOT_ESTABLISHED` | `NOT_APPROVED` |

`0.1.0-alpha.3` 只表示加入非生产 Customer/Supplier/Product/BOM/Supplier Mapping 的自托管开发基线，不表示生产候选、生产上线、真实数据迁移完成或业务迁移完成。当前自托管 API 已覆盖身份/用户/系统审计、Material Draft/Review/Active、Import Mapping、Normalization、人工复核和 TASK03 主数据/BOM；采购、库存、生产、销售、品质、财务以及相关旧业务流程仍依赖 Python/SQLite 运行面。
...
当前 PostgreSQL 基线是空库 `0001 -> 0007`。它只在隔离 PostgreSQL 17/Compose 中执行过；没有生产 PostgreSQL 部署，也没有真实数据迁移。
