...
适用任务：最新本机隔离候选证据为`SELFHOST-RELEASE-CANDIDATE-REFRESH-51`；最新并行非生产部署验收仍为`SELFHOST-UAT-FIX-38`，历史发布/恢复记录保留下文
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Node.js / PostgreSQL alpha.46当前本机隔离候选 | `0.1.0-alpha.46`；Web`sha256:249d0ce44a595f306e6423219d530bcf3d017e5c87bcd4da13d103def8b75b7f`；Worker`sha256:0e07fded39ce07122246752a29a2960493811b55f9aa9059e7d674bf90eb8370` | 精确commit`8084d6c3e38e4246b79791414e84bfe2da4ea8f8`/tree`a54473f6b05cdfaa014f286149a740b90d5067fe`；bundle绑定父源码`12beccf0…390e`，SHA-256`f4481316…5ce6` | 源码/镜像含45/head`0045_runtime_worker_readiness.sql`与allowlist`09d34ac7…504a1`；未应用任何运行数据库 | 构建回执`f490b969…c1b2`；Compose/六服务runtime通过；固定Trivy/11.8小时内DB断网无socket扫描Web25+63、Worker25+60包且全部severity0；release48/48、直接45/45、supervisor31/31、lint/credentials通过。正式证据/19步门因supervisor未安装退出1 | `LOCAL_ENGINE_ONLY / NOT_RELEASED / NOT_DEPLOYED`；loopback registry已删除，无外部registry锚点；UAT未变 | `NOT_MIGRATED`；未读取或迁移真实数据 | 现行alpha.42 UAT及其既有回退未改变；该候选若本机镜像删除则构建回执不能自行恢复 | `CURRENT DIAGNOSTIC ZERO / NOT_ELIGIBLE / NOT_APPROVED`；无正式scan provenance/SBOM/security evidence、19步PASS或release manifest |
...
`0.1.0-alpha.1` 是 PHASE0-TASK03 建立的原始非生产发布基线，不是当前包版本。当前源码和并行非生产 UAT 已演进到 `0.1.0-alpha.42`/`0040`；既有部门交接、Manufacturing Batch、Finished Goods Lot/FQC/Shipment、Supplier Receipt Lot→IQC、Project Unit Resolution、Revision Response、Supplier Mapping、RFQ→PO追溯与warehouse readiness事实保持，并新增服务端日期驱动的收货preview失败关闭、NORMAL实际模式投影、返回修改草稿合同及运行时版本health。FIX38只完成零业务写黑盒，没有真实Receipt。这只证明非生产链路和升级恢复门禁成立，不表示真实公司数据已迁移或已批准生产上线。

Git 同步状态以2026-08-09 FIX38部署严格起点计：唯一worktree、clean`main@fc551c6571b57593a3232a14617935b3e3c3171f`、Parent`569aa954d764309e239d1f6c174e582596d33a24`、behind0/ahead185；独立运维/文档收口后预期ahead186，未推送Git或镜像。最终状态以`git status --short --branch`为准。
