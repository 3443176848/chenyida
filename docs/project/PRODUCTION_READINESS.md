...
| 证据项 | 当前事实 | 判定 |
| --- | --- | --- |
| 根仓库 | TASK53源码`08608eb1…2125`、tree`1a750f85…81c8`与manifest-only直接子提交`d246cbde…a938`形成47文件可复核链；manifest SHA-256`94027198…1c87`，未fetch/push | 本地可追踪；当前完整历史的异机锚点待更新 |
| 私有源码锚点 | 本机最后已知跟踪引用为`recovery-private/main@1dcbf8d`；本任务未fetch、调用远端API或push，因此未声称其为远端实时状态，也未用本地提交差值冒充异机证明 | `FAIL`，当前完整历史未证明异机存在 |
| 源码 | `0.1.0-alpha.46`，Migration45/head`0045_runtime_worker_readiness.sql`；0045 SHA-256为`cc4685a0…80fc`，0001—0044未修改；当前内容寻址源码提交为`08608eb1`，bundle提交为`d246cbde` | 源码/supervisor输入闭合；尚无同源码候选镜像 |
...
| 隔离候选 | TASK51的`8084d6c3` Web/Worker镜像、六服务runtime及零发现诊断仍可作历史审计证据，但TASK53改变了发布源码与合同输入 | `STALE / NOT AUTHORIZABLE`；须从新精确链重建并取得外部锚点 |
...
| 当前资源 | available memory约 2.2 GiB，Swap约 719 MiB/1.0 GiB，根分区可用约 16 GiB，Load约`0.91/1.23/1.25` | 未触发停止线；Swap与磁盘余量需持续观察，不代表容量验收 |
...
- TASK50使未来Compose六服务全面使用`read_only`、`no-new-privileges`和`cap_drop`，TASK51历史候选曾完成digest绑定与零发现诊断；TASK53源码变化后该候选已失效。现行UAT未部署，也没有当前镜像外部锚点、签名/attestation或正式supervisor SBOM/安全证据。
...
6. TASK50已完成容器运行时最小权限加固；TASK53已完成三阶段发布生命周期修复和bundle重建。持续交付下一安全任务优先补齐PR-001的异机传输provenance、加密/保留/调度及失败恢复仓库合同；候选重建、A3外部锚点及A1/A2另按资源和授权依赖推进。岗位权限矩阵仍等待业务负责人确认。
