...
- `SELFHOST-PHASE4-TASK01`：市场部门 → 项目部门需求交接（DOING；源码 `0.1.0-alpha.15` / PostgreSQL `0015` 已实现并通过隔离测试，等待并行环境实际验收）。
- TASK02：项目部 → 计划部，产品/BOM/规格交接（PLANNED，禁止自动启动）。
- TASK03：计划物料需求与采购申请（PLANNED）。
