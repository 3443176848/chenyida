...
| Phase 3 | 导入规范化与暂存 | 2 | DOING（设计完成，待确认） |
...
- `PHASE3-TASK01`：设计 Material Import Normalization & Staging V1（DONE；16 项决定为 `PROPOSED`，待规格确认）。
- `PHASE3-TASK02`：实施 Normalization、`0006`、API 和隔离测试（PLANNED；依赖 TASK01 规格确认，不含生产迁移/部署）。

**当前状态**：DOING（书面设计完成，实施未开始）。
