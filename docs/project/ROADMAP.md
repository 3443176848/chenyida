...
| Phase 4 | AI治理 | 5 | PLANNED（TASK01 DONE；TASK02 DOING / HOLDOUT MEASURED；TASK03—TASK05 TODO） |
...
**预计任务数**：5。**当前状态**：PLANNED（TASK01治理基线DONE；TASK02为`DOING / SOURCE_READY / HOLDOUT_MEASURED / THRESHOLD_DECISION_REQUIRED`；TASK03—TASK05未开始）。

**执行顺序**：

- `PHASE4-TASK01`：建立AI治理评估与审批边界（DONE；D-110及V1合同已批准，AI仅建议、确定性门禁优先、关键安全违规允许值0；`IMPLEMENTATION NOT STARTED`）。
- `PHASE4-TASK02`：建立版本化去敏评估集、固定holdout、确定性基线和离线Evaluator（DOING；`material-v1@1.0.0`共64条，固定holdout已从冻结提交`d69f6dff…194ec`测量，关键安全违规0；准确率阈值和最低coverage仍为`UNAPPROVED`，等待项目负责人决定）。
...
D-110不授权模型调用、外部供应商、真实数据外发、Schema/Migration、生产试点或部署；TASK02的离线测量不自动批准阈值，也不自动启动TASK03。
