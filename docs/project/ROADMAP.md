...
- `PHASE3-TASK03`：设计 Material Import Normalization Review UI V1（DONE / WAITING FOR SPEC CONFIRMATION；四份 docs-only 交付、37 线框、104 项测试和 14 项 `PROPOSED` 决定）。

**当前状态**：Normalization 后端非生产实现完成；Review UI 正式规格已形成并停止等待“规格确认”，前端实施尚未开始。行完整 Issue 查询局部门禁及性能/可访问性门禁必须在未来实施任务中保留。
