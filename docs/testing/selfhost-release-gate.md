...
- Node源码门：`npm run test:release:node-source`，先 build，再按冻结清单串行运行107个纯Node测试文件；数据库、浏览器和专用POSIX文件由各自独立门负责。
- PostgreSQL清单门：`npm run test:release:postgres-regression`，在一个断网、只读、资源受限的PostgreSQL容器内，按冻结清单串行执行80个文件并为每个文件创建/销毁隔离数据库。
...
4. 生成新的`RUN_RELEASE_GATE`授权并调用 launcher。18步依次覆盖 release 合同、supervisor Python合同、凭证扫描、build+全部Node测试、80文件PostgreSQL回归、Browser E2E、POSIX专用测试、全部tsconfig、ESLint、隔离Migration、备份恢复、Python三基线、Compose config、`git diff --check`、镜像SBOM和漏洞证据。
...
- 仓库实现已通过43/43发布合同、80文件/367测试PostgreSQL清单门、4文件/28测试POSIX专用门、46/46备份与Dashboard相关合同、15/15 supervisor Python合同、TASK42定向TypeScript和隔离双集群备份恢复；Migration隔离PostgreSQL最终结果记录在任务文档。正式候选门没有运行。
- 当前没有安装 host supervisor，也没有修改 systemd、权限、网络、Docker daemon 或运行中的 Compose。
- alpha.44/0041尚未 build；不存在同候选 Web/Worker 镜像、真实 Trivy image evidence、完整 gate `PASS`或`ELIGIBLE`manifest。
