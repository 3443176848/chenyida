...
}, (t) => [uniqueIndex("production_nonconformances_code_uq").on(t.ncrCode), uniqueIndex("production_nonconformances_inspection_uq").on(t.inspectionId), uniqueIndex("production_nonconformances_operation_uq").on(t.operationId), index("production_nonconformances_queue_idx").on(t.status, t.createdAt, t.id), index("production_nonconformances_work_order_idx").on(t.workOrderId, t.snapshotOperationId, t.id), check("production_nonconformances_status_ck", sql`${t.status} in ('OPEN','REWORK_PENDING','REWORK_ACCEPTED','DISPOSED','CANCELLED')`), check("production_nonconformances_quantity_ck", sql`${t.inspectedQty}>0 and ${t.passedQty}>=0 and ${t.failedQty}>0 and ${t.passedQty}+${t.failedQty}=${t.inspectedQty} and ${t.activeReworkQty}>=0 and ${t.finalScrapQty}>=0 and ${t.unresolvedQty}>=0 and ${t.activeReworkQty}+${t.finalScrapQty}+${t.unresolvedQty}=${t.failedQty}`), check("production_nonconformances_version_ck", sql`${t.version}>0`), check("production_nonconformances_cancel_ck", sql`(${t.status}='CANCELLED' and ${t.cancelledBy} is not null and ${t.cancelledRequestId} is not null and ${t.cancelledAt} is not null and char_length(btrim(${t.cancelReason})) between 1 and 1000) or (${t.status}<>'CANCELLED' and ${t.cancelledBy} is null and ${t.cancelledRequestId} is null and ${t.cancelledAt} is null and ${t.cancelReason}='')`)]);
...
}, (t) => [index("production_nonconformance_events_ncr_idx").on(t.nonconformanceId, t.id), check("production_nonconformance_events_type_ck", sql`${t.eventType} in ('CREATED','REWORK_RESERVED','REWORK_UPDATED','REWORK_SUBMITTED','REWORK_RETURNED','REWORK_ACCEPTED','REWORK_CANCELLED','SCRAP_DISPOSED','CANCELLED')`), check("production_nonconformance_events_status_ck", sql`(${t.fromStatus} is null or ${t.fromStatus} in ('OPEN','REWORK_PENDING','REWORK_ACCEPTED','DISPOSED','CANCELLED')) and ${t.toStatus} in ('OPEN','REWORK_PENDING','REWORK_ACCEPTED','DISPOSED','CANCELLED') and ${t.quantity}>=0`)]);
