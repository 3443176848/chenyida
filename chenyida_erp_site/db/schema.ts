...
export const materialImportSupplierProfiles = sqliteTable("material_import_supplier_profiles", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  supplierKey: text("supplier_key").notNull(), profileName: text("profile_name").notNull(), profileVersion: integer("profile_version").notNull().default(1), status: text("status").notNull().default("ACTIVE"), templateFingerprint: text("template_fingerprint"),
  structureRulesJson: text("structure_rules_json").notNull().default("{}"), fieldAliasesJson: text("field_aliases_json").notNull().default("{}"), mappingRulesJson: text("mapping_rules_json").notNull().default("{}"),
  createdBy: text("created_by").notNull().references(() => appUsers.username, { onDelete: "restrict" }), updatedBy: text("updated_by").notNull().references(() => appUsers.username, { onDelete: "restrict" }), createdAt: text("created_at").notNull(), updatedAt: text("updated_at").notNull(),
}, (table) => [
  uniqueIndex("material_import_supplier_profiles_version_uq").on(table.supplierKey, table.profileName, table.profileVersion),
  uniqueIndex("material_import_supplier_profiles_active_template_uq").on(table.supplierKey, table.templateFingerprint).where(sql`${table.status}='ACTIVE' AND ${table.templateFingerprint} IS NOT NULL`),
  index("material_import_supplier_profiles_status_idx").on(table.supplierKey, table.status, table.updatedAt, table.id),
  check("material_import_supplier_profiles_identity_ck", sql`length(trim(${table.supplierKey})) BETWEEN 1 AND 255 AND length(trim(${table.profileName})) BETWEEN 1 AND 255 AND ${table.profileVersion}>0`),
  check("material_import_supplier_profiles_status_ck", sql`${table.status} IN ('ACTIVE','INACTIVE')`),
  check("material_import_supplier_profiles_fingerprint_ck", sql`${table.templateFingerprint} IS NULL OR (length(${table.templateFingerprint})=64 AND ${table.templateFingerprint} NOT GLOB '*[^0-9a-f]*')`),
  check("material_import_supplier_profiles_json_ck", sql`json_valid(${table.structureRulesJson}) AND json_type(${table.structureRulesJson})='object' AND json_valid(${table.fieldAliasesJson}) AND json_type(${table.fieldAliasesJson})='object' AND json_valid(${table.mappingRulesJson}) AND json_type(${table.mappingRulesJson})='object'`),
]);
...
export const materialImportMappings = sqliteTable("material_import_mappings", {
  ...
  suggestionAlgorithmVersion: text("suggestion_algorithm_version"), supersedesMappingId: integer("supersedes_mapping_id"), supplierProfileId: integer("supplier_profile_id").references(() => materialImportSupplierProfiles.id, { onDelete: "restrict" }),
  headerStartRowNumber: integer("header_start_row_number"), dataStartRowNumber: integer("data_start_row_number"), structureConfidence: real("structure_confidence").notNull().default(0), structureStatus: text("structure_status").notNull().default("NEEDS_REVIEW"), structureEvidenceJson: text("structure_evidence_json").notNull().default("{}"),
...
}, (table) => [foreignKey({ columns: [table.supersedesMappingId], foreignColumns: [table.id] }).onDelete("restrict"), index("material_import_mappings_batch_run_idx").on(table.batchId, table.parseRunId, table.id), index("material_import_mappings_supplier_profile_idx").on(table.supplierProfileId, table.createdAt, table.id), uniqueIndex("material_import_mappings_current_uq").on(table.parseRunId).where(sql`${table.mappingStatus}<>'SUPERSEDED'`), check("material_import_mappings_header_ck", sql`(${table.headerMode}='SINGLE_ROW' AND ${table.headerRowNumber}>0) OR (${table.headerMode}='NO_HEADER' AND ${table.headerRowNumber} IS NULL)`), check("material_import_mappings_status_ck", sql`${table.mappingStatus} IN ('DRAFT','CONFIRMED','STALE','SUPERSEDED')`), check("material_import_mappings_values_ck", sql`${table.selectedSheetIndex}>=0 AND ${table.mappingVersion}>0 AND (${table.headerStartRowNumber} IS NULL OR ${table.headerStartRowNumber}>0) AND (${table.dataStartRowNumber} IS NULL OR ${table.dataStartRowNumber}>0) AND ${table.structureConfidence} BETWEEN 0 AND 1`), check("material_import_mappings_structure_ck", sql`${table.structureStatus} IN ('HIGH_CONFIDENCE','NEEDS_REVIEW','NO_CANDIDATE','CONFIRMED') AND json_valid(${table.structureEvidenceJson}) AND json_type(${table.structureEvidenceJson})='object'`), check("material_import_mappings_digest_ck", sql`length(${table.metadataDigest})=64 AND ${table.metadataDigest} NOT GLOB '*[^0-9a-f]*'`), check("material_import_mappings_confirm_ck", sql`(${table.mappingStatus}='CONFIRMED' AND ${table.confirmedBy} IS NOT NULL AND ${table.confirmedAt} IS NOT NULL) OR (${table.mappingStatus}<>'CONFIRMED')`)]);
...
export const materialImportMappingItems = sqliteTable("material_import_mapping_items", {
  ...
  sourceColumnIndexesJson: text("source_column_indexes_json"), sourceHeadersJson: text("source_headers_json"), combinationStrategy: text("combination_strategy").notNull().default("FIRST_NON_EMPTY"), combinationSeparator: text("combination_separator").notNull().default(" "), mappingConfidence: real("mapping_confidence").notNull().default(0), adaptiveMappingStatus: text("adaptive_mapping_status").notNull().default("UNMAPPED"), mappingEvidenceJson: text("mapping_evidence_json").notNull().default("[]"),
}, (table) => [index("material_import_mapping_items_source_idx").on(table.mappingId, table.sourceColumnIndex, table.id), uniqueIndex("material_import_mapping_items_target_uq").on(table.mappingId, table.targetNamespace, table.targetCode).where(sql`${table.targetNamespace}<>'ignore'`), check("material_import_mapping_items_namespace_ck", sql`${table.targetNamespace} IN ('basic','attribute','category_hint','supplier_reference','ignore')`), check("material_import_mapping_items_mode_ck", sql`${table.mappingMode} IN ('SOURCE','SOURCE_WITH_DEFAULT','DEFAULT','IGNORE')`), check("material_import_mapping_items_source_ck", sql`(${table.mappingMode}='DEFAULT' AND ${table.sourceColumnIndex} IS NULL) OR (${table.mappingMode}<>'DEFAULT' AND ${table.sourceColumnIndex}>=0)`), check("material_import_mapping_items_default_ck", sql`${table.defaultValueJson} IS NULL OR json_valid(${table.defaultValueJson})`), check("material_import_mapping_items_values_ck", sql`${table.required} IN (0,1) AND ${table.displayOrder}>=0 AND (${table.sourceColumnIndexesJson} IS NULL OR (json_valid(${table.sourceColumnIndexesJson}) AND json_type(${table.sourceColumnIndexesJson})='array' AND json_array_length(${table.sourceColumnIndexesJson}) BETWEEN 1 AND 8)) AND (${table.sourceHeadersJson} IS NULL OR (json_valid(${table.sourceHeadersJson}) AND json_type(${table.sourceHeadersJson})='array' AND json_array_length(${table.sourceHeadersJson}) BETWEEN 1 AND 8)) AND ${table.combinationStrategy} IN ('FIRST_NON_EMPTY','JOIN_NON_EMPTY','SPECIFICATION_EXTRACT') AND length(${table.combinationSeparator}) BETWEEN 0 AND 10 AND ${table.mappingConfidence} BETWEEN 0 AND 1 AND ${table.adaptiveMappingStatus} IN ('EXACT','HIGH_CONFIDENCE','SUGGESTED','UNMAPPED','CONFLICT','CONFIRMED') AND json_valid(${table.mappingEvidenceJson}) AND json_type(${table.mappingEvidenceJson})='array'`)]);
...
export const materialImportNormalizedRows = sqliteTable("material_import_normalized_rows", {
  ...
  mappedValuesJson: text("mapped_values_json"), mappingConfidence: real("mapping_confidence").notNull().default(0), specificationConfidence: real("specification_confidence").notNull().default(0), adaptiveMappingStatus: text("adaptive_mapping_status").notNull().default("UNMAPPED"), reviewStatus: text("review_status").notNull().default("NEEDS_REVIEW"),
}, (table) => [
  uniqueIndex("material_import_normalized_rows_position_uq").on(table.normalizationRunId, table.sourceSheetIndex, table.sourceRowNumber), index("material_import_normalized_rows_status_idx").on(table.normalizationRunId, table.rowStatus, table.id), index("material_import_normalized_rows_adaptive_review_idx").on(table.normalizationRunId, table.reviewStatus, table.adaptiveMappingStatus, table.id),
  check("material_import_normalized_rows_position_ck", sql`${table.sourceSheetIndex}>=0 AND ${table.sourceRowNumber}>0`), check("material_import_normalized_rows_payload_ck", sql`json_valid(${table.normalizedPayloadJson}) AND json_type(${table.normalizedPayloadJson})='object' AND length(CAST(${table.normalizedPayloadJson} AS BLOB))<=262144 AND (${table.mappedValuesJson} IS NULL OR (json_valid(${table.mappedValuesJson}) AND json_type(${table.mappedValuesJson})='object'))`), check("material_import_normalized_rows_hash_ck", sql`length(${table.sourceRawRowHash})=64 AND ${table.sourceRawRowHash} NOT GLOB '*[^0-9a-f]*' AND length(${table.normalizedPayloadHash})=64 AND ${table.normalizedPayloadHash} NOT GLOB '*[^0-9a-f]*'`), check("material_import_normalized_rows_status_ck", sql`((${table.rowStatus}='VALID' AND ${table.errorCount}=0 AND ${table.warningCount}=0) OR (${table.rowStatus}='WARNING' AND ${table.errorCount}=0 AND ${table.warningCount}>0) OR (${table.rowStatus}='ERROR' AND ${table.errorCount}>0)) AND ${table.mappingConfidence} BETWEEN 0 AND 1 AND ${table.specificationConfidence} BETWEEN 0 AND 1 AND ${table.adaptiveMappingStatus} IN ('EXACT','HIGH_CONFIDENCE','SUGGESTED','UNMAPPED','CONFLICT') AND ${table.reviewStatus} IN ('AUTO_ACCEPTABLE','NEEDS_REVIEW','CONFIRMED','REJECTED')`),
]);
