...
export const productionRoutingOperations = pgTable("production_routing_operations", {
  id: bigserial("id", { mode: "number" }).primaryKey(), routingVersionId: bigint("routing_version_id", { mode: "number" }).notNull().references(() => productionRoutingVersions.id, { onDelete: "restrict" }), sequenceNo: integer("sequence_no").notNull(), operationCode: text("operation_code").notNull(), operationName: text("operation_name").notNull(), workCenterId: bigint("work_center_id", { mode: "number" }).notNull().references(() => productionWorkCenters.id, { onDelete: "restrict" }), setupMinutes: numeric("setup_minutes", { precision: 18, scale: 6 }).notNull().default("0"), runMinutesPerUnit: numeric("run_minutes_per_unit", { precision: 18, scale: 6 }).notNull().default("0"), description: text("description").notNull().default(""), createdBy: text("created_by").notNull().references(() => appUsers.username, { onDelete: "restrict" }), requestId: uuid("request_id").notNull(), createdAt: timestamptz("created_at").notNull().defaultNow(),
}, (t) => [uniqueIndex("production_routing_operations_sequence_uq").on(t.routingVersionId, t.sequenceNo), uniqueIndex("production_routing_operations_code_uq").on(t.routingVersionId, t.operationCode), index("production_routing_operations_work_center_idx").on(t.workCenterId, t.routingVersionId), check("production_routing_operations_sequence_ck", sql`${t.sequenceNo}>0`), check("production_routing_operations_time_ck", sql`${t.setupMinutes}>=0 and ${t.runMinutesPerUnit}>=0`), check("production_routing_operations_text_ck", sql`char_length(btrim(${t.operationCode})) between 1 and 40 and char_length(btrim(${t.operationName})) between 1 and 200 and char_length(${t.description})<=2000`)]);
