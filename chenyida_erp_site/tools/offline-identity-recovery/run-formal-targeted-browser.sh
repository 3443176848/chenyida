...
evidence=$runtime_root/targeted-identity-recovery-browser-$run_id-attempt-$attempt.json
lock_file=$runtime_root/offline-identity-recovery.lock
short_id=$(printf '%s' "$run_id" | tr -d '-' | cut -c1-12)
network_name=chenyida-erp-targeted-browser-$short_id-$attempt
...
[ ! -e "$evidence" ] || fail TARGETED_BROWSER_EVIDENCE_EXISTS
[ -f "$module_root/node_modules/playwright/index.mjs" ] || fail TARGETED_BROWSER_MODULE_MISSING
[ -f "$module_root/.tree-sha256" ] || fail TARGETED_BROWSER_MODULE_MISSING
[ "$(stat -c '%u:%g:%a' "$module_root")" = 0:0:700 ] || fail TARGETED_BROWSER_MODULE_METADATA_INVALID
[ "$(stat -c '%u:%g:%a:%h' "$module_root/.tree-sha256")" = 0:0:600:1 ] || fail TARGETED_BROWSER_MODULE_METADATA_INVALID
grep -Eq '"version"[[:space:]]*:[[:space:]]*"1\.51\.1"' "$module_root/node_modules/playwright/package.json" \
  || fail TARGETED_BROWSER_MODULE_VERSION_INVALID
module_digest=$(find "$module_root/node_modules" -type f -print0 | sort -z | xargs -0 sha256sum | sha256sum | cut -d' ' -f1)
[ "$module_digest" = "$(tr -d '\n' <"$module_root/.tree-sha256")" ] || fail TARGETED_BROWSER_MODULE_INTEGRITY_INVALID
[ "$(docker image inspect --format '{{.Id}}' "$expected_browser_image" 2>/dev/null)" = "$expected_browser_image" ] \
  || fail TARGETED_BROWSER_IMAGE_MISMATCH
[ "$(stat -c '%u:%g:%a:%h' /etc/chenyida-erp/uat-role-accounts.txt)" = 0:0:600:1 ] \
  || fail TARGETED_CANONICAL_METADATA_INVALID

web_id=$(docker inspect --format '{{.Id}}' chenyida-erp-parallel-web-1 2>/dev/null) || fail TARGETED_WEB_STATE_INVALID
worker_id=$(docker inspect --format '{{.Id}}' chenyida-erp-parallel-worker-1 2>/dev/null) || fail TARGETED_WORKER_STATE_INVALID
postgres_id=$(docker inspect --format '{{.Id}}' chenyida-erp-parallel-postgres-1 2>/dev/null) || fail TARGETED_POSTGRES_STATE_INVALID
caddy_id=$(docker inspect --format '{{.Id}}' chenyida-erp-parallel-caddy-1 2>/dev/null) || fail TARGETED_CADDY_STATE_INVALID
postgres_image=$(docker inspect --format '{{.Image}}' chenyida-erp-parallel-postgres-1 2>/dev/null) || fail TARGETED_POSTGRES_STATE_INVALID
caddy_image=$(docker inspect --format '{{.Image}}' chenyida-erp-parallel-caddy-1 2>/dev/null) || fail TARGETED_CADDY_STATE_INVALID
...
assert_services() {
  [ "$(docker inspect --format '{{.Id}}:{{.State.Status}}:{{if .State.Health}}{{.State.Health.Status}}{{end}}:{{.Image}}' chenyida-erp-parallel-web-1 2>/dev/null)" \
    = "$web_id:running:healthy:$expected_web_image" ] || fail TARGETED_WEB_STATE_INVALID
  [ "$(docker inspect --format '{{.Id}}:{{.State.Status}}:{{.Image}}' chenyida-erp-parallel-worker-1 2>/dev/null)" \
    = "$worker_id:running:$expected_worker_image" ] || fail TARGETED_WORKER_STATE_INVALID
  [ "$(docker inspect --format '{{.Id}}:{{.State.Status}}:{{if .State.Health}}{{.State.Health.Status}}{{end}}:{{.Image}}' chenyida-erp-parallel-postgres-1 2>/dev/null)" \
    = "$postgres_id:running:healthy:$postgres_image" ] || fail TARGETED_POSTGRES_STATE_INVALID
  [ "$(docker inspect --format '{{.Id}}:{{.State.Status}}:{{.Image}}' chenyida-erp-parallel-caddy-1 2>/dev/null)" \
    = "$caddy_id:running:$caddy_image" ] || fail TARGETED_CADDY_STATE_INVALID
}
...
cleanup() {
  ...
  if [ "$preserve_evidence" -eq 0 ] && [ -e "$evidence" ]; then unlink "$evidence"; fi
  sync -f "$runtime_root" 2>/dev/null || true
}
trap cleanup EXIT HUP INT TERM

assert_services
if docker network inspect "$network_name" >/dev/null 2>&1; then fail TARGETED_BROWSER_NETWORK_EXISTS; fi
docker network create --driver bridge --internal --label chenyida.erp.scope=targeted-identity-browser "$network_name" >/dev/null
network_created=1
...
docker run --rm \
  --pull never \
  --name "chenyida-erp-targeted-browser-$short_id-$attempt" \
  --user 0:0 \
  --read-only \
  ...
  /runner/targeted-browser-verify.mjs \
  --expected-run-id "$run_id" \
  --verification-attempt "$attempt" \
  --evidence-path "/evidence/targeted-identity-recovery-browser-$run_id-attempt-$attempt.json"
...
network_created=0
[ "$(stat -c '%u:%g:%a:%h' "$evidence" 2>/dev/null)" = 0:0:600:1 ] || fail TARGETED_BROWSER_EVIDENCE_INVALID
assert_services
preserve_evidence=1
trap - EXIT HUP INT TERM
printf '%s\n' 'STAGE TARGETED_FORMAL_BROWSER PASS'
