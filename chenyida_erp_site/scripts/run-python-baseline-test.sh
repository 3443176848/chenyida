...
[ -x "$REPOSITORY_ROOT/.venv/bin/python" ] || { echo "project Python runtime is unavailable" >&2; exit 1; }
...
/usr/bin/bwrap \
  ...
  --bind "$TEMP_ROOT/source" /workspace --ro-bind "$SUPERVISOR_SITE_ROOT" /supervisor --ro-bind "$REPOSITORY_ROOT/.venv" /opt/venv \
