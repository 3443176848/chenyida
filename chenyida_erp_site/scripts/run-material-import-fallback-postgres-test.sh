...
NODE_IMAGE='node@sha256:5647be709086c696ff32edaaf1c70cd26d1da6ab2b39c32f3c7b4c4a31957e37'
POSTGRES_IMAGE='postgres@sha256:4f736ae292687621d4dbe0d499ffd024a36bd2ee7d8ca6f2ccd4c800f047b394'
TASK_ROOT=$(mktemp -d /tmp/cyd-material-import-fallback-runtime.XXXXXX)
RUN_ID=${TASK_ROOT##*.}
TASK_LABEL="chenyida.erp.material-import-fallback-test=$RUN_ID"
NODE_CONTAINER="cyd-material-import-fallback-node-$RUN_ID"
POSTGRES_CONTAINER="cyd-material-import-fallback-postgres-$RUN_ID"
NODE_ID=""
POSTGRES_ID=""
...
remove_task_container() {
  ...
  [ "$(/usr/bin/docker inspect --format '{{index .Config.Labels "chenyida.erp.material-import-fallback-test"}}|{{.Name}}' "$container_id" 2>/dev/null || true)" = "$RUN_ID|/$container_name" ] || {
...
cleanup() {
  ...
  case "$TASK_ROOT" in
    /tmp/cyd-material-import-fallback-runtime.*) rm -rf -- "$TASK_ROOT" || status=1 ;;
    *) echo "refusing unsafe material import test cleanup" >&2; status=1 ;;
  esac
  [ "$status" = 0 ] || exit 1
}
...
[ "$(id -u)" = 0 ] || { echo "material import PostgreSQL test requires root" >&2; exit 1; }
...
POSTGRES_ID=$(/usr/bin/docker create --pull=never --label "$TASK_LABEL" --name "$POSTGRES_CONTAINER" \
  ...
  -e ERP_MATERIAL_IMPORT_FALLBACK_POSTGRES_CONTAINER_MODE=YES -e NODE_OPTIONS=--max-old-space-size=384 \
  -v "$SITE_ROOT:/workspace:ro" -v "$NODE_MODULES:/workspace/node_modules:ro" -v "$TASK_ROOT/node:/usr/local/bin/node:ro" \
  --entrypoint /bin/sh "$POSTGRES_IMAGE" /workspace/tests/selfhost-material-import-fallback-postgres.sh)
if ! /usr/bin/docker start --attach "$POSTGRES_ID"; then
  ...
  echo "material import PostgreSQL test failed: exit=$exit_code state=$state_error" >&2
  exit 1
fi
