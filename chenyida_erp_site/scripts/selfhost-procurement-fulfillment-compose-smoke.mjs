...
async function seedAcceptedRequest(pool) {
  ...
  try {
    ...
    const material = await db.query("insert into material_master(internal_material_code,standard_name,category_id,base_uom,base_unit_id,material_status,procurement_type,inventory_type,inspection_type,environmental_requirement,source_type,last_modified_by,created_by,updated_by,request_id) values('MAT-T05-001','TASK05 连接器',$1,'PCS',$2,'ACTIVE','PURCHASED','STOCKED','IQC','RoHS','MANUAL',$3,$3,$3,$4) returning id", [category.rows[0].id, unit.rows[0].id, adminUsername, randomUUID()]);
