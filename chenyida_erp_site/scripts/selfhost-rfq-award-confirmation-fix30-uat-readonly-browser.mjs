...
const REQUIRED_CONFIRMATION = "MAIN_UAT_RFQ1_AWARD_CONFIRMATION_CONTRACT_CANCEL_ONLY";
...
if (process.env.ERP_RFQ_AWARD_CONFIRMATION_FIX30_UAT_CONFIRM !== REQUIRED_CONFIRMATION) {
  throw new Error(`ERP_RFQ_AWARD_CONFIRMATION_FIX30_UAT_CONFIRM=${REQUIRED_CONFIRMATION} is required`);
}
const databaseUrl = process.env.ERP_RFQ_AWARD_CONFIRMATION_DATABASE_URL || "";
...
if (!parsedDatabaseUrl || !["postgres:", "postgresql:"].includes(parsedDatabaseUrl.protocol)
  ...
  || process.env.ERP_RFQ_AWARD_CONFIRMATION_DATABASE_NAME !== REQUIRED_DATABASE
  || decodeURIComponent(parsedDatabaseUrl.pathname.replace(/^\//, "")) !== REQUIRED_DATABASE) {
  throw new Error(`FIX-30 UAT database guards must target the exact ${REQUIRED_DATABASE} database`);
}
...
async function loadChromium() {
  ...
  throw new Error("Playwright is required in the FIX-30 readonly UAT runner");
}

const pool = new Pool({ connectionString: databaseUrl, max: 1, application_name: "rfq-award-confirmation-fix30-uat-readonly" });
...
function assertDetail(detail) {
  ...
  assert.match(version.request_id, /^[0-9a-f-]{36}$/);
  assert.deepEqual(version.supplier_summaries.map((row) => ({
    supplier_id: stableId(row.supplier_id, "Quote Supplier ID"),
    ...
    latest_promised_delivery_date: row.latest_promised_delivery_date,
    delivery_status: row.delivery_status,
    delivery_explanation: row.delivery_explanation,
  })), [{
    supplier_id: "1", supplier_code: "SUP-000001", quote_id: "1", quote_version_no: 1,
    supplier_quote_reference: "UAT-Q-A-042576", total_amount: "480.000000", currency_code: "CNY",
    latest_promised_delivery_date: "2026-10-20", delivery_status: "ON_TIME", delivery_explanation: "提前10天",
  }, {
    supplier_id: "2", supplier_code: "SUP-000002", quote_id: "2", quote_version_no: 1,
    supplier_quote_reference: "UAT-Q-B-042576", total_amount: "400.000000", currency_code: "CNY",
    latest_promised_delivery_date: "2026-11-05", delivery_status: "LATE", delivery_explanation: "延期6天",
  }]);
  assert.ok(version.fixed_quote_inputs.every((row) => typeof row.supplier_id === "string"
    && typeof row.quote_id === "string" && typeof row.comparison_candidate_id === "string"));
...
async function assertConfirmation(page, version, stage) {
  ...
  includesAll(text, [
    ...
    EXPECTED_OUTPUT_DIGEST, version.request_id, ...version.comparison_rows.map((row) => row.basis_digest),
...
try {
  ...
  await page.getByRole("heading", { name: "RFQ-00000001 · Round 1", exact: true }).waitFor();
  const refreshedForm = page.locator("form.award-selection-form");
  await refreshedForm.waitFor();
  assert.equal(await page.locator(".rfq-dialog.award-confirm-dialog[role=dialog]").count(), 0, "reload must clear the local confirmation draft");
  for (const expected of EXPECTED_LINES) {
    assert.equal(await refreshedForm.locator(`select[name="candidate_${expected.rfq_line_id}"]`).inputValue(), "", `reload must clear Candidate ${expected.supplier_a_candidate}`);
  }
  assert.equal(await refreshedForm.locator('select[name="reason_code"]').inputValue(), "");
  assert.equal(await refreshedForm.locator('textarea[name="reason"]').inputValue(), "");
  assert.deepEqual(businessWrites, []);
  assert.deepEqual(directBusinessWrites, []);
  ...
  assert.deepEqual(after, before, "FIX-30 readonly UAT must preserve the exact protected state");
  assert.deepEqual(businessWrites, []);
  assert.deepEqual(directBusinessWrites, []);
  assert.deepEqual(forbiddenGets, []);
  assert.deepEqual(browserErrors, []);
  console.info(`RFQ_AWARD_CONFIRMATION_FIX30_UAT_READONLY_OK database=${REQUIRED_DATABASE} actor=${REQUIRED_USERNAME} rfq=1 rfq_version=6 comparison_version=1 comparison_lines=4 candidates=8 selected_candidates=2,4,6,8 output_digest=${EXPECTED_OUTPUT_DIGEST} business_post=0 award=0 award_line=0 po=0 desktop=1 mobile=1 cancelled=1 reload_draft=0 session=0`);
} finally {
