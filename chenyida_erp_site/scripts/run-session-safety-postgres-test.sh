...
TASK_ROOT=$(mktemp -d /tmp/cyd-session-safety-runtime.XXXXXX)
RUN_ID=${TASK_ROOT##*.}
TASK_LABEL="chenyida.erp.session-safety-test=$RUN_ID"
NODE_CONTAINER="cyd-session-safety-node-$RUN_ID"
POSTGRES_CONTAINER="cyd-session-safety-postgres-$RUN_ID"
NODE_ID=""
POSTGRES_ID=""
...
remove_task_container() {
  ...
  [ "$(/usr/bin/docker inspect --format '{{index .Config.Labels "chenyida.erp.session-safety-test"}}|{{.Name}}' "$container_id" 2>/dev/null || true)" = "$RUN_ID|/$container_name" ] || {
...
cleanup() {
  ...
  case "$TASK_ROOT" in
    /tmp/cyd-session-safety-runtime.*) rm -rf -- "$TASK_ROOT" || status=1 ;;
    *) echo "refusing unsafe session safety test cleanup" >&2; status=1 ;;
  esac
  [ "$status" = 0 ] || exit 1
}
...
[ "$(id -u)" = 0 ] || { echo "session safety PostgreSQL test requires root" >&2; exit 1; }
...
POSTGRES_ID=$(/usr/bin/docker create --pull=never --label "$TASK_LABEL" --name "$POSTGRES_CONTAINER" \
  ...
  -e ERP_SESSION_SAFETY_POSTGRES_CONTAINER_MODE=YES -e NODE_OPTIONS=--max-old-space-size=384 \
  -v "$SITE_ROOT:/workspace:ro" -v "$NODE_MODULES:/workspace/node_modules:ro" -v "$TASK_ROOT/node:/usr/local/bin/node:ro" \
  --entrypoint /bin/sh "$POSTGRES_IMAGE" /workspace/tests/selfhost-session-safety-postgres.sh)
if ! /usr/bin/docker start --attach "$POSTGRES_ID"; then
  ...
  echo "session safety PostgreSQL test failed: exit=$exit_code state=$state_error" >&2
  exit 1
fi
