...
TEMP_ROOT=$(mktemp -d /tmp/cyd-cluster-recovery-runtime.XXXXXX)
RUN_ID=${TEMP_ROOT##*.}
TASK_LABEL="chenyida.erp.cluster-recovery-test=$RUN_ID"
NODE_CONTAINER="cyd-cluster-recovery-node-$RUN_ID"
POSTGRES_CONTAINER="cyd-cluster-recovery-postgres-$RUN_ID"
NODE_ID=""
POSTGRES_ID=""
...
remove_task_container() {
  ...
  [ "$(/usr/bin/docker inspect --format '{{index .Config.Labels "chenyida.erp.cluster-recovery-test"}}|{{.Name}}' "$container_id" 2>/dev/null || true)" = "$RUN_ID|/$container_name" ] || { echo "refusing to remove a container not created by this test" >&2; return 1; }
...
cleanup() {
  ...
  case "$TEMP_ROOT" in /tmp/cyd-cluster-recovery-runtime.*) rm -rf -- "$TEMP_ROOT" || status=1 ;; *) echo "refusing unsafe runtime cleanup" >&2; status=1 ;; esac
  [ "$status" = 0 ] || exit 1
}
...
[ "${ERP_RELEASE_GATE_GIT_COMMIT:-$GIT_COMMIT}" = "$GIT_COMMIT" ] && [ "${ERP_RELEASE_GATE_GIT_TREE:-$GIT_TREE}" = "$GIT_TREE" ] || { echo "cluster recovery snapshot identity mismatch" >&2; exit 1; }
git_candidate diff --quiet --no-ext-diff --no-textconv --
git_candidate diff --cached --quiet --no-ext-diff --no-textconv --
[ -z "$(git_candidate ls-files --others --exclude-standard -- chenyida_erp_site)" ] || { echo "untracked candidate files block cluster recovery test" >&2; exit 1; }
...
[ -f "$SITE_ROOT/tests/selfhost-postgresql-cluster-recovery-postgres.sh" ] || { echo "cluster recovery test snapshot is incomplete" >&2; exit 1; }
...
POSTGRES_ID=$(/usr/bin/docker create --pull=never --label "$TASK_LABEL" --name "$POSTGRES_CONTAINER" \
  ...
  --memory 768m --memory-swap 1g --cpus 1.0 --pids-limit 256 --tmpfs /tmp:rw,exec,nosuid,nodev,size=1280m \
  -e NODE_OPTIONS=--max-old-space-size=384 \
  -v "$SITE_ROOT:/workspace:ro" -v "$NODE_MODULES:/workspace/node_modules:ro" -v "$TEMP_ROOT/node:/usr/local/bin/node:ro" \
  --entrypoint /bin/sh "$POSTGRES_IMAGE" /workspace/tests/selfhost-postgresql-cluster-recovery-postgres.sh)
