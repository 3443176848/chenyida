...
export const RELEASE_TEST_RUNTIME_POLICY_SHA256 = "315caf0531d824c4c2eb4fa3e67369702d2ab1112d78564a28c4c9ebd68ff1bc";
export const RELEASE_TEST_INVENTORY_SHA256 = "92650a1b6c814d5ca4aa49fe1fa8176a5aef27274435d6c3f5c67ba11f46ea4c";
...
const OFFICIAL_RELEASE_GATE_EXECUTORS = ["NODE_CANDIDATE_TEST", "PYTHON_CANDIDATE_TEST", "NODE_CANDIDATE_TEST", "NODE_CANDIDATE_TEST", "POSTGRES_CANDIDATE_TEST", "NODE_CANDIDATE_TEST", "NODE_CANDIDATE_TEST", "NODE_CANDIDATE_TEST", "NODE_CANDIDATE_TEST", "MIGRATION_POSTGRES_TEST", "BACKUP_RECOVERY_POSTGRES_TEST", "PYTHON_CANDIDATE_TEST", "PYTHON_CANDIDATE_TEST", "PYTHON_CANDIDATE_TEST", "COMPOSE_CONFIG_TEST", "SOURCE_CHECK", "EVIDENCE_CHECK", "EVIDENCE_CHECK"];
const OFFICIAL_RELEASE_GATE_ACTIONS = ["CONTRACTS", "SUPERVISOR_CONTRACTS", "CREDENTIALS", "NODE_SOURCE", "POSTGRES_REGRESSION", "BROWSER_E2E", "SPECIAL_POSIX", "TYPECHECK", "LINT", "RELEASE_MIGRATION", "BACKUP_RECOVERY", "SELF_TEST", "SMOKE", "GO_LIVE", "VALIDATE", "DIFF_CHECK", "IMAGE_SBOM", "VULNERABILITY_ASSESSMENT"];
const OFFICIAL_RELEASE_GATE_TIMEOUTS = [600, 600, 600, 14400, 14400, 14400, 7200, 7200, 3600, 7200, 7200, 600, 900, 900, 300, 300, 60, 60];
const OFFICIAL_RELEASE_GATE_RESOURCE_CLASSES = ["LIGHT", "LIGHT", "LIGHT", "HEAVY", "HEAVY", "HEAVY", "HEAVY", "HEAVY", "HEAVY", "HEAVY", "HEAVY", "LIGHT", "LIGHT", "LIGHT", "LIGHT", "LIGHT", "LIGHT", "LIGHT"];
...
export function validateOfficialTestRuntimePolicy(value, raw = null) {
  ...
  if (value.test_inventory.path !== "chenyida_erp_site/release/release-test-inventory-v1.json" || value.test_inventory.sha256 !== RELEASE_TEST_INVENTORY_SHA256 || value.test_inventory.total_tests !== 227 || value.test_inventory.required_tests !== 203 || value.test_inventory.not_applicable_tests !== 24 || canonicalJson(value.test_inventory.category_counts) !== canonicalJson({ BROWSER: 6, HISTORICAL_D1_SITES: 22, POSTGRES: 80, POSTGRES_ALIAS: 2, PURE_NODE: 107, RELEASE_CONTRACT: 6, SPECIAL_HARNESS: 4 })) reject("TEST_RUNTIME_INVENTORY_INVALID");
...
function validateRuntimeServiceStates(value, prefix) {
  ...
  const expected = new Map([["caddy", "none"], ["postgres", "healthy"], ["web", "healthy"], ["worker", "none"]]);
  ...
  return value.length === expected.size && value.every((state) => expected.has(state.service) && state.restart_count === 0 && state.oom_killed === false && state.status === "running" && state.health === expected.get(state.service));
}
