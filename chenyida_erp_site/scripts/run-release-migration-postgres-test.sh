...
TEMP_ROOT=$(mktemp -d /tmp/cyd-release-migration-runtime.XXXXXX)
RUN_ID=${TEMP_ROOT##*.}
TASK_LABEL="chenyida.erp.release-migration-test=$RUN_ID"
NODE_CONTAINER="cyd-release-migration-node-$RUN_ID"
POSTGRES_CONTAINER="cyd-release-migration-postgres-$RUN_ID"
NODE_ID=""; POSTGRES_ID=""
...
remove_task_container() {
  ...
  [ "$(/usr/bin/docker inspect --format '{{index .Config.Labels "chenyida.erp.release-migration-test"}}|{{.Name}}' "$container_id" 2>/dev/null || true)" = "$RUN_ID|/$container_name" ] || { echo "refusing to remove a container not created by this test" >&2; return 1; }
...
cleanup() {
  ...
  case "$TEMP_ROOT" in /tmp/cyd-release-migration-runtime.*) rm -rf -- "$TEMP_ROOT" || status=1 ;; *) echo "refusing unsafe runtime cleanup" >&2; status=1 ;; esac
  [ "$status" = 0 ] || exit 1
}
...
[ "${ERP_RELEASE_GATE_GIT_COMMIT:-$GIT_COMMIT}" = "$GIT_COMMIT" ] && [ "${ERP_RELEASE_GATE_GIT_TREE:-$GIT_TREE}" = "$GIT_TREE" ] || { echo "release migration snapshot identity mismatch" >&2; exit 1; }
git_candidate diff --quiet --no-ext-diff --no-textconv --
git_candidate diff --cached --quiet --no-ext-diff --no-textconv --
[ -z "$(git_candidate ls-files --others --exclude-standard -- chenyida_erp_site)" ] || { echo "untracked candidate files block release migration test" >&2; exit 1; }
...
POSTGRES_ID=$(/usr/bin/docker create --pull=never --label "$TASK_LABEL" --name "$POSTGRES_CONTAINER" \
  ...
  --entrypoint /bin/sh "$POSTGRES_IMAGE" /workspace/tests/selfhost-release-migration-postgres.sh)
if ! /usr/bin/docker start --attach "$POSTGRES_ID"; then
  ...
  echo "release migration PostgreSQL container failed: exit=$exit_code state=$state_error" >&2
  exit 1
fi
