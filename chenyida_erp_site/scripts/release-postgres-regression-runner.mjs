...
const EXPECTED_POSTGRES_TESTS = 80;
...
const TEMPLATE_NAMES = Object.freeze({
  ...
  41: "cyd_release_regression_template_41",
});
...
function databaseConfiguration(entry, source) {
  ...
  let template = TEMPLATE_NAMES[41];
...
async function main() {
  ...
  if (migrations.length !== 41 || migrations.at(-1)?.filename !== "0041_ai_governance_suggestion_evidence.sql") reject("POSTGRES_REGRESSION_MIGRATION_SET_INVALID");
