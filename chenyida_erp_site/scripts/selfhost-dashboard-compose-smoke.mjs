...
try {
  ...
  if (backup.browser_create_enabled !== false || backup.browser_restore_enabled !== false || backup.restore_target !== "NEW_EMPTY_TARGET_ONLY" || backup.verification_status !== expectedBackupStatus) throw new Error(`backup governance mismatch: ${JSON.stringify(backup)}`);
