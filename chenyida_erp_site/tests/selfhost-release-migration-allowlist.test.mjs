...
async function migrationFixture(){const root=await mkdtemp(path.join(os.tmpdir(),"cyd-release-migration-"));const directory=path.join(root,"migrations");await mkdir(directory);await writeFile(path.join(directory,"0001_first.sql"),"create table fixture(id integer);\n");await writeFile(path.join(directory,"0002_second.sql"),"alter table fixture add column payload text;\n");return{root,directory,entries:await buildMigrationAllowlist(directory)};}

test("migration authorization stays bundle-local and independent of application database modules",async()=>{const source=await readFile(new URL("../scripts/release-migration-authorization.ts",import.meta.url),"utf8");assert.doesNotMatch(source,/from\s+["'](?:pg|\.\.\/db\/|\.\.\/app\/)/);});
