...
import { amount, date, documentType, id, settlementType, text, version } from "../app/lib/finance-selfhost/rules.ts";
...
test("finance validation keeps ids, versions, dates and decimal strings exact", () => {
  ...
  assert.equal(settlementType("receipt"), "RECEIPT"); assert.equal(settlementType("PAYMENT"), "PAYMENT"); assert.throws(() => settlementType("TRANSFER"), /RECEIPT 或 PAYMENT/);
