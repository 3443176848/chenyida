...
const names=(await readdir(directory)).filter(name=>/^\d{4}_.+\.sql$/.test(name)&&name<="0020_production_handoff_reservations.sql").sort(),sources=new Map();for(const name of names)sources.set(name,await readFile(new URL(name,directory),"utf8"));
