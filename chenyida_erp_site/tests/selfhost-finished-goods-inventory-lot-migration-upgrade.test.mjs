...
test("empty database and repeated runner reach exactly 0032", async () => {
  assert.equal(names.length, 32); assert.equal(names.at(-1), "0032_finished_goods_inventory_lots.sql");
...
test("0031 to 0032 preserves historical empty-lot balance", async () => {
  await migrate(names.slice(0, 31));
...
test("0032 failure is transactional", async () => {
  await migrate(names.slice(0, 31)); await pool.query("create table inventory_lots(id bigint)");
  await assert.rejects(migrate(names.slice(31)), /inventory_lots.*already exists/i);
  ...
  assert.equal((await pool.query("select count(*)::int n from schema_migrations where version=$1", [names.at(-1)])).rows[0].n, 0);
});
...
test("journal snapshot schema and SHA-256 describe 0032", async () => {
  ...
  assert.equal(snapshot.prevId, previous.id); assert.equal(journal.entries.at(-1).tag, "0032_finished_goods_inventory_lots"); assert.match(checksum(names.at(-1)), /^[0-9a-f]{64}$/);
});
