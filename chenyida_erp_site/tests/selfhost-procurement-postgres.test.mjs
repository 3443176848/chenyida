...
test("PO idempotency update receipt reversal close and financial source stay consistent", async () => {
  ...
  const firstReceipt = await api("/api/purchase-receipts", { method: "POST", role: "purchase", key: "procurement-receipt-0001", body: { purchase_order_id: poId, reason: "首批到货", lines: [{ purchase_order_line_id: lineId, quantity: "4", expected_line_version: 1, expected_balance_version: 0 }] } }); assert.equal(firstReceipt.response.status, 201); assert.match(firstReceipt.payload.receipt_code, /^PR-\d{8}$/);
  const replayReceipt = await api("/api/purchase-receipts", { method: "POST", role: "purchase", key: "procurement-receipt-0001", body: { purchase_order_id: poId, reason: "首批到货", lines: [{ purchase_order_line_id: lineId, quantity: "4", expected_line_version: 1, expected_balance_version: 0 }] } }); assert.equal(replayReceipt.response.headers.get("Idempotency-Replayed"), "true");
  const secondReceipt = await api("/api/purchase-receipts", { method: "POST", role: "purchase", body: { purchase_order_id: poId, reason: "剩余到货", lines: [{ purchase_order_line_id: lineId, quantity: "6", expected_line_version: 2, expected_balance_version: 1 }] } }); assert.equal(secondReceipt.response.status, 201);
...
test("reference, permission, CSRF, over-receipt, concurrent receipt and atomic rollback fail closed", async () => {
  ...
  const failed = await api("/api/purchase-receipts", { method: "POST", role: "purchase", key: "rollback-procurement-key", body: { purchase_order_id: poId, reason: "整体回滚", lines: [{ purchase_order_line_id: lineId, quantity: "1", expected_line_version: 2, expected_balance_version: 1 }] } }); assert.equal(failed.response.status, 500);
