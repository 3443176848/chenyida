...
[ "${ERP_MATERIAL_IMPORT_FALLBACK_POSTGRES_CONTAINER_MODE:-}" = YES ] || exit 2
TASK_ROOT=$(mktemp -d /tmp/cyd-material-import-fallback-postgres.XXXXXX)
...
cleanup() {
  ...
  case "$TASK_ROOT" in
    /tmp/cyd-material-import-fallback-postgres.*) rm -rf -- "$TASK_ROOT" || status=1 ;;
    *) echo "refusing unsafe material import PostgreSQL cleanup" >&2; status=1 ;;
  esac
  [ "$status" = 0 ] || exit 1
}
...
if ! gosu postgres pg_ctl -D "$PGDATA" -l "$PGLOG" -o "-h 127.0.0.1 -p 5432 -c unix_socket_directories='$TASK_ROOT' -c max_connections=16 -c max_locks_per_transaction=1024 -c shared_buffers=48MB -c work_mem=2MB -c maintenance_work_mem=24MB -c fsync=off -c synchronous_commit=off -c full_page_writes=off" -w start >/dev/null; then
  [ ! -f "$PGLOG" ] || tail -n 80 "$PGLOG" >&2
  exit 1
fi
RUNNING=1
gosu postgres createdb -h 127.0.0.1 -p 5432 material_import_fallback_test
cd /workspace
