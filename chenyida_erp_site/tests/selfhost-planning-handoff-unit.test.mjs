...
import { canonicalDigest, normalizeRevisionResponseText, optionalDate, resolutionInput, revisionResponseInput, successorPackageInput, unitResolutionInput } from "../app/lib/planning-handoff-selfhost/validation.ts";
...
test("revision response preserves Chinese punctuation, normalizes NFC and LF, and enforces CAS bounds", () => {
  const decomposed = "  已按计划部退回要求补充：Cafe\u0301，四项物料整批齐套。\r\nProduct A0 保持不变。  ";
  const normalized = normalizeRevisionResponseText(decomposed);
  assert.equal(normalized, "已按计划部退回要求补充：Café，四项物料整批齐套。\nProduct A0 保持不变。");
  assert.match(normalized, /：/);
  assert.match(normalized, /，/);
  const parsed = revisionResponseInput({ expected_head_version: 0, response_text: decomposed });
  assert.equal(parsed.responseText, normalized);
  assert.match(parsed.responseTextDigest, /^[0-9a-f]{64}$/);
  assert.throws(() => revisionResponseInput({ expected_head_version: 0, response_text: " \r\n " }), error => error.code === "REVISION_RESPONSE_REQUIRED");
  assert.throws(() => revisionResponseInput({ expected_head_version: 0, response_text: "不足十字" }), error => error.code === "REVISION_RESPONSE_INVALID");
  assert.throws(() => revisionResponseInput({ expected_head_version: 0, response_text: "长".repeat(2001) }), error => error.code === "REVISION_RESPONSE_INVALID");
  assert.throws(() => revisionResponseInput({ expected_head_version: -1, response_text: "这是足够长度的工程回复正文。" }), error => error.code === "REVISION_VERSION_CONFLICT");
  assert.deepEqual(successorPackageInput({ expected_package_version: 3, expected_response_head_version: 2, revision_response_version_id: 9 }), { expectedPackageVersion: 3, expectedResponseHeadVersion: 2, revisionResponseVersionId: 9 });
});
...
test("planning API owns a bounded transactional boundary and excludes downstream planning", async () => {
  ...
  for (const route of ["requirement-resolutions", "requirement-unit-resolutions", "planning-packages", "planning-handoffs", "revision-responses", "successor", "submit|accept|return"]) assert.match(handler, new RegExp(route));
  ...
  assert.match(service, /package_digest/); assert.match(service, /RESUBMITTED/); assert.match(service, /saveRevisionResponse/); assert.match(service, /createSuccessorPackage/); assert.match(service, /response_text_digest/); assert.match(repository, /client\.query\("begin"\)/); assert.match(repository, /idempotency_keys/); assert.match(repository, /audit_log/);
  assert.doesNotMatch(service, /auditDetail:\s*\{[^}]*response_text:/s);
