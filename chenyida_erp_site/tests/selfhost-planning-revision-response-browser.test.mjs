...
async function assertIsolatedSchema() {
  ...
  assert.deepEqual((await pool.query("select count(*)::integer count,max(version) latest from schema_migrations")).rows[0], { count: 46, latest: "0046_runtime_lock_privilege_boundary.sql" });
...
async function login(page, credentials) {
  ...
  await page.getByRole("button", { name: "登录工作台", exact: true }).click(); assert.equal((await response).status(), 200);
