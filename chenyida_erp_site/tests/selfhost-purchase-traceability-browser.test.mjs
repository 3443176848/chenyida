...
const REQUIRED_DATABASE = "erp_fix16_material_requirement_test";
...
if (confirmation !== "ISOLATED_FIX16_SYNTHETIC_ONLY") throw new Error("ERP_PURCHASE_SUPPLY_BROWSER_CONFIRM=ISOLATED_FIX16_SYNTHETIC_ONLY is required");

const pool = new Pool({ connectionString: databaseUrl, max: 3, application_name: "purchase-supply-browser-fix16" });
let server;
...
test("isolated Chromium cancels without writes, accepts once and reopens the processed credential", { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    const context = await browser.newContext({ viewport: { width: 390, height: 844 }, serviceWorkers: "block" }); const businessPosts = [], detailGets = [];
    ...
    for(const card of await page.locator(".purchase-current-card").all()){const text=await card.innerText();for(const expected of ["当前在手总量","当前正式预留","品质冻结/Hold","当前库存可用","有效计划库存分配","当前未分配库存可用","当前有效在途总量","有效计划在途分配","当前未分配在途可用","模型未单独记录"])assert.match(text,new RegExp(expected));const quantities=card.locator(".planning-quantity");assert.equal(await quantities.count(),9);for(const quantity of await quantities.all()){assert.equal((await quantity.textContent()).replace(/\s+/g," ").trim(),"0 PCS");const geometry=await quantity.evaluate((element)=>{const style=getComputedStyle(element),rect=element.getBoundingClientRect();return{display:style.display,whiteSpace:style.whiteSpace,width:rect.width,height:rect.height}});assert.equal(geometry.display,"inline-flex");assert.equal(geometry.whiteSpace,"nowrap");assert.ok(geometry.width>20);assert.ok(geometry.height<40)}}await noOverflow(page, "purchase detail 390px");
    const acceptPath = `/api/purchase-requests/${fixture.requestId}/accept`;
    for (const cancellation of ["cancel", "close", "escape"]) {
      const getBefore=detailGets.length;await page.getByRole("button", { name: "接收采购申请", exact: true }).click(); const dialog = page.getByRole("dialog", { name: `确认接收 ${fixture.requestCode}`, exact: true }); await dialog.waitFor();assert.equal(detailGets.length,getBefore+1);await page.waitForFunction(() => document.activeElement?.textContent?.trim() === "取消");
      await dialog.getByText(`${fixture.packageId}/v1 · ACCEPT SUCCESS`, { exact: true }).waitFor();
      await dialog.getByText("已重新读取当前供应", { exact: true }).waitFor();await dialog.getByRole("heading", { name: "提交数量与当前供应（4 条）", exact: true }).waitFor();await dialog.getByText("接收不会修改库存、正式预留或 Planning Allocation。", { exact: true }).waitFor();await dialog.getByText("接收不会自动创建 RFQ、Quote、Award、PO、Delivery Plan、Receipt、Ledger、AP 或 Work Order。", { exact: true }).waitFor();assert.equal(await dialog.locator(".purchase-confirm-supply-cards article").count(),4);assert.equal(await dialog.locator(".purchase-confirm-supply-cards").getByText("10 PCS", { exact: true }).count(),4);await noOverflow(page, `accept ${cancellation}`); const before = businessPosts.filter((path) => path === acceptPath).length;
