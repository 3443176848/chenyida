...
test("isolated Chromium cancels without writes, accepts once and reopens the processed credential", { timeout: 240_000 }, async () => {
  ...
  try {
    ...
    async function assertProcessedHistory(stage){await page.getByText("该 PRQ 已处理；关系化快照保持只读。",{exact:true}).waitFor();for(const heading of ["Package 与 ACCEPT 谱系","Material Requirement Plan 谱系","PRQ 提交凭证","采购决策凭证"])await page.getByRole("heading",{name:heading,exact:true}).waitFor();for(const value of [fixture.packageAcceptRequestId,fixture.planGenerateRequestId,fixture.prqSubmitRequestId])await page.getByText(value,{exact:true}).waitFor();const planFact=await page.locator("dt",{hasText:"采购交接状态"}).locator("..").innerText();assert.match(planFact,/ACCEPTED/);const prqFact=await page.locator("dt",{hasText:"PRQ 状态"}).locator("..").innerText();assert.match(prqFact,/ACCEPTED/);assert.match(prqFact,/采购已接收/);await page.getByText(/Plan v1 计算快照、行项目、分配及来源摘要仍不可变/).waitFor();assert.equal(await page.locator(".purchase-snapshot-card").count(),4);assert.equal(await page.locator(".purchase-current-card").count(),4);assert.equal(await page.getByRole("button",{name:"接收采购申请",exact:true}).count(),0);assert.equal(await page.getByRole("button",{name:"退回计划部",exact:true}).count(),0);assert.equal(await page.locator(".planning-decision-actions, textarea").count(),0);await assertDecisionCredential(stage)}
