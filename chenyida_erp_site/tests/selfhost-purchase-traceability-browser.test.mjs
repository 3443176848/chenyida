...
const REQUIRED_DATABASE = "erp_fix17_material_requirement_test";
...
if (confirmation !== "ISOLATED_FIX17_SYNTHETIC_ONLY") throw new Error("ERP_PURCHASE_SUPPLY_BROWSER_CONFIRM=ISOLATED_FIX17_SYNTHETIC_ONLY is required");

const pool = new Pool({ connectionString: databaseUrl, max: 3, application_name: "purchase-confirmation-browser-fix17" });
let server;
...
const meta = (identity, action) => ({ actor: identity, requestId: randomUUID(), operationId: randomUUID(), keyDigest: sha256(randomUUID()), requestDigest: sha256(randomUUID()), method: "POST", route: `/isolated/fix17/${action}`, action });
...
async function state(requestId) {
  ...
  return { request: { status: request.status, version: Number(request.version), accepted_by: request.accepted_by }, acceptedEvents, returnedEvents, downstream };
}
...
test("isolated Chromium cancels without writes, accepts once and reopens the processed credential", { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    async function openAcceptanceDialog(){
      ...
      const dialog=page.getByRole("dialog",{name:`确认接收 ${fixture.requestCode}`,exact:true});await dialog.waitFor();assert.equal(detailGets.length,getBefore+1);await page.waitForFunction(()=>document.activeElement?.textContent?.trim()==="取消");
      for(const heading of ["PRQ 与项目","Package ACCEPT 完整凭证","Plan GENERATE 完整凭证","PRQ SUBMIT 完整凭证","接收前显式决策计数","四项固定数量与九项当前供应（4 个 Material）","供应公式与边界","接收后果","下一阶段"])await dialog.getByRole("heading",{name:heading,exact:true}).waitFor();
      ...
      const counts=await dialog.locator(".purchase-decision-counts").innerText();assert.match(counts,/接收前 ACCEPT\s*0/);assert.match(counts,/接收前 RETURN\s*0/);await dialog.getByText("已重新读取当前供应",{exact:true}).waitFor();assert.equal(await dialog.getByRole("button",{name:"确认接收",exact:true}).isEnabled(),true);
      ...
      for(const consequence of ["不修改 Package、Plan、PRQ 明细、库存、正式预留或 Planning Allocation。","不自动创建 RFQ、Quote、Award、PO、Delivery Plan、Receipt、Ledger、AP 或 Work Order。","采购寻源、询价和报价比较由后续独立受控操作完成；本次接收不会开始上述流程。"])await dialog.getByText(consequence,{exact:true}).waitFor();return dialog;
    }
    ...
    await page.getByRole("heading", { name: "操作完成凭证", exact: true }).waitFor(); await page.getByText(payload.request_id, { exact: true }).waitFor(); assert.equal(businessPosts.filter((path) => path === acceptPath).length, postBefore + 1); const accepted = await state(fixture.requestId); assert.deepEqual(accepted.downstream, initial.downstream); assert.equal(accepted.request.status, "ACCEPTED"); assert.equal(accepted.request.version, 2); assert.equal(accepted.request.accepted_by, fixture.credentials.username); assert.equal(accepted.acceptedEvents, 1);assert.equal(accepted.returnedEvents,0);
    await page.getByRole("button", { name: "从已处理记录查看凭证", exact: true }).click(); await page.getByText("该 PRQ 已处理；关系化快照保持只读。", { exact: true }).waitFor(); await page.getByText(fixture.requestCode, { exact: true }).first().waitFor();for(const value of [fixture.packageAcceptRequestId,fixture.planGenerateRequestId,fixture.prqSubmitRequestId])await page.getByText(value,{exact:true}).waitFor();const processedCounts=await page.locator(".purchase-decision-counts").innerText();assert.match(processedCounts,/ACCEPT 事件数量\s*1/);assert.match(processedCounts,/RETURN 事件数量\s*0/);await noOverflow(page, "processed credential 390px");
    await page.goto(`${REQUIRED_ORIGIN}/`, { waitUntil: "domcontentloaded" }); await page.getByRole("button", { name: "退出", exact: true }).click(); await page.getByRole("heading", { name: "登录晨亿达 ERP", exact: true }).waitFor();assert.equal(Number((await pool.query("select count(*) count from app_sessions where username=$1 and revoked_at is null",[fixture.credentials.username])).rows[0].count),0);
    const protectedContentGone=()=>page.waitForFunction((requestCode)=>!document.body.innerText.includes(requestCode)&&(document.body.innerText.includes("登录晨亿达 ERP")||document.body.innerText.includes("请先登录")),fixture.requestCode);await page.goBack({waitUntil:"domcontentloaded"});await protectedContentGone();await page.goForward({waitUntil:"domcontentloaded"});await protectedContentGone();await page.reload({waitUntil:"domcontentloaded"});await protectedContentGone();assert.equal(await page.getByText(fixture.requestCode,{exact:true}).count(),0);await context.close();
