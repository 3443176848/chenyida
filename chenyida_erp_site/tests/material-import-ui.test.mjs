...
test("UI-089 保存失败不自动覆盖本地 Mapping", () => { const block = workspaceSource.match(/const saveMapping = async \(\) => \{[^]*?\n  \};\n  const previewMapping/)?.[0] || ""; assert.match(block, /catch \(reason\) \{ handleError\(reason\); \}/); assert.doesNotMatch(block, /catch \(reason\)[^]*setItems\(/); });
...
test("UI-097 confirmed 不虚构确认人时间", () => { const panel = workspaceSource.match(/\{batch\.status === "MAPPING_CONFIRMED" && query\.view === "confirmed"[^]*?<\/section> : null\}/)?.[0] || ""; assert.match(panel, /不显示 API 未提供的确认人或确认时间/); assert.doesNotMatch(panel, /confirmed_by|confirmed_at/); });
