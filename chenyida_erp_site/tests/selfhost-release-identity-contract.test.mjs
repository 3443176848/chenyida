...
test("writer pins trusted tools and publishes only after strict four-service reinspection",async()=>{
  ...
  assert.match(writer,/ERP_RELEASE_SUPERVISOR_NODE_RUNTIME/);
  assert.match(writer,/chenyida-erp-runtime-privilege-node\.\*/);
  assert.match(writer,/0:0:555:1/);
  assert.doesNotMatch(writer,/NODE_IMAGE=|docker create|docker cp|NODE_BOOTSTRAP/);
  ...
  assert.ok(writer.indexOf("acquire_chenyida_release_gate_lock") < writer.indexOf("ERP_RELEASE_SUPERVISOR_NODE_RUNTIME"));
