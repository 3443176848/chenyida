...
class ReleaseSupervisorInstallerTest(unittest.TestCase):
    ...
    def test_finalization_interlock_blocks_bundle_switch_until_checkpoint_13_commits(self):
        with tempfile.TemporaryDirectory(prefix="cyd-supervisor-finalization-interlock-") as directory:
            ...
            marker.chmod(0o400)
            operation_id = "promotion-finalization-fixture"
            ...
            intent = {
                **body,
                "finalization_intent_sha256": installer.sha256(installer.canonical_json(body)),
            }
            intent_file = intents / (
                f"{operation_id}.{intent['finalization_intent_sha256']}.json"
            )
            ...
            current.chmod(0o400)
            installer.assert_no_uat_promotion_finalization_interlock(root)
            intent_file.chmod(0o600)
            intent_file.write_bytes(intent_file.read_bytes().replace(b"promotion-fixture", b"promotion-changed"))
            intent_file.chmod(0o400)
            with self.assertRaisesRegex(
                    installer.InstallError,
