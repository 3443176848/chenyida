...
test("versioned test inventory accounts for every top-level test and only excludes explicit legacy or alias evidence", async () => {
  ...
  assert.deepEqual(inventory.category_counts, { BROWSER: 6, HISTORICAL_D1_SITES: 22, POSTGRES: 84, POSTGRES_ALIAS: 2, PURE_NODE: 130, RELEASE_CONTRACT: 11, SPECIAL_HARNESS: 7 });
  assert.deepEqual(inventory.tests.filter((entry) => entry.category === "RELEASE_CONTRACT").map((entry) => entry.path), [
    ...
    "tests/selfhost-uat-promotion-rollback-fixed-executor.test.mjs",
    "tests/selfhost-uat-promotion-rollback-runtime-contract.test.mjs",
  ]);
