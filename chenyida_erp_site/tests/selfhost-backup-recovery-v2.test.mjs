...
test("manifest binds database bytes, honest dump scope, UTC dates, and canonical database rendering",async()=>withTemporaryRoot(async(root)=>{
  ...
  assert.match(reconciliationSql,/FROM pg_largeobject_metadata/);assert.doesNotMatch(reconciliationSql,/FROM pg_largeobject(?:\s|;)/);
}));
...
async function fakeBackupTools(root,migrationText){
  ...
  await executable(path.join(bin,"docker"),`#!/bin/sh\nprintf '%s\\n' "$*" >> '${argvLog}'\nlast=""; for last do :; done\ncase "$1" in\n ps) case "$*" in *com.docker.compose.service=web*) printf '%s\\n' '${webContainerId}' ;; *com.docker.compose.service=worker*) printf '%s\\n' '${workerContainerId}' ;; *) exit 0 ;; esac ;;\n inspect) case "$*" in *'range .Config.Env'*) printf 'ERP_DEPLOYMENT_CLASS=uat\\n' ;; *) if [ "$last" = web-test ]; then printf '%s\\n' '${webContainerId}|false|false|0|2026-08-12T00:00:00Z|2026-08-12T00:01:00Z|${webImageDigest}|erp-uat-source|web|0.1.0-alpha.47|${gitCommit}'; else printf '%s\\n' '${workerContainerId}|false|false|0|2026-08-12T00:00:00Z|2026-08-12T00:01:00Z|${workerImageDigest}|erp-uat-source|worker|0.1.0-alpha.47|${gitCommit}'; fi ;; esac ;; *) exit 1 ;; esac\n`);
  ...
  await executable(path.join(bin,"pg_restore"),'#!/bin/sh\ncase "$*" in *--list*) printf "synthetic dump list\\n" ;; *) exit 1 ;; esac\n');
  await executable(path.join(bin,"psql"),`#!/bin/sh\nprintf '%s\\n' "$*" >> '${argvLog}'\ncase "$*" in\n *backup-reconciliation.sql*) printf 'LARGE_OBJECTS\\t0\\t0\\taaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\\n' ;;\n *pg_database_size*) printf 'source_test|${databaseSystemIdentifier}|${databaseOid}|chenyida-erp-deployment/v2:UAT:erp-uat-source|t|off|0|0|17|UTF8|C|C|libc|NONE|-1|${databaseBytes}|0\\n' ;;\n *rolbypassrls*) printf 'source_test|${databaseOid}|chenyida-erp-deployment/v2:UAT:erp-uat-source|chenyida_erp_backup|chenyida_erp_backup|0|0|0|0|0|1|1|2|0|off|1|0|1|0|1|0\\n' ;;\n *"max(split_part"*) printf 'source_test|${databaseSystemIdentifier}|${databaseOid}|chenyida-erp-deployment/v2:UAT:erp-uat-source|1|on|-1|0|0|9:1:0:0:0:1:0:0\\n' ;;\n *pg_largeobject_metadata*) printf '0\\n' ;;\n *"current_setting('default_transaction_read_only')"*) printf 'source_test|${databaseSystemIdentifier}|${databaseOid}|chenyida-erp-deployment/v2:UAT:erp-uat-source|0|-1|0|off|4|9:5:0:0:4:1:0:0\\n' ;;\n *aclexplode*) printf '9:5:0:0:4:1:0:0\\n' ;;\n *"checksum||'  '||version"*) cat '${migrationFile}' ;;\n *) exit 0 ;; esac\n`);
...
test("backup wrapper uses a quiesced snapshot guard, runtime identity, and secret-free argv",async()=>withTemporaryRoot(async(root)=>{
  ...
  const credentialRoot=await dedicatedRoot(root,"credentials",".chenyida-erp-credential-root-v2","chenyida-erp-credential-root/v2"),controlServiceFile=path.join(credentialRoot,"pg_control_service.conf"),captureServiceFile=path.join(credentialRoot,"pg_capture_service.conf"),controlSecret="fixture-only-control-secret",captureSecret="fixture-only-capture-secret",sourceMachineIdentityFile=await machineIdentityFile(root,"source-machine-id","1".repeat(32));await writeFile(controlServiceFile,`[backup_control]\nhost=127.0.0.1\ndbname=source_test\nuser=control\npassword=${controlSecret}\n`,{mode:0o600});await chmod(controlServiceFile,0o600);await writeFile(captureServiceFile,`[backup_capture]\nhost=127.0.0.1\ndbname=source_test\nuser=chenyida_erp_backup\npassword=${captureSecret}\n`,{mode:0o600});await chmod(captureServiceFile,0o600);
  ...
  run(path.join(scripts,"backup-selfhost.sh"),["--credential-root",credentialRoot,"--control-db-service-file",controlServiceFile,"--control-db-service","backup_control","--capture-db-service-file",captureServiceFile,"--capture-db-service","backup_capture","--deployment-class","UAT","--deployment-id","erp-uat-source","--expected-database","source_test","--uploads",sources.uploads,"--attachments",sources.attachments,"--backup-status",receiptRoot,"--migrations",migrations,"--backup-root",backupRoot,"--receipt-root",receiptRoot,"--receipt-reader-gid",String(process.getgid()),"--web-container","web-test","--worker-container","worker-test","--location-id","source-host","--policy-id","daily-rpo-v1","--rpo-hours","24","--machine-identity-file",sourceMachineIdentityFile,"--confirm","UAT_BACKUP_V2_AUTHORIZED"],{env:{...fake.env,NODE_ENV:"test",ERP_RELEASE_GATE_LOCK_FILE:path.join(root,"release-gate.lock")}});
  const argv=await readFile(fake.argvLog,"utf8");assert.equal(argv.includes(controlSecret)||argv.includes(captureSecret),false);assert.doesNotMatch(argv,/--schema=public/);assert.match(argv,/default_transaction_read_only/);assert.match(argv,/service=backup_control/);assert.match(argv,/service=backup_capture/);assert.match(argv,/--no-large-objects/);const receipt=JSON.parse(await readFile(path.join(receiptRoot,"latest.json"),"utf8"));assert.equal(receipt.result,"LOCAL_VERIFIED");assert.equal((await stat(path.join(receiptRoot,"latest.json"))).mode&0o777,0o640);assert.equal((await stat(path.join(receiptRoot,"latest.json"))).gid,process.getgid());const backups=(await readdir(backupRoot)).filter((name)=>name.startsWith("backup-"));assert.equal(backups.length,1);assert.equal((await stat(path.join(receiptRoot,`${backups[0]}.local.json`))).mode&0o777,0o640);const manifest=JSON.parse(await readFile(path.join(backupRoot,backups[0],"manifest.json"),"utf8"));assert.equal(manifest.consistency.database_guard,"DEFAULT_TRANSACTION_READ_ONLY_DEFENSE_IN_DEPTH");assert.equal(manifest.consistency.content_reconciliation,"BEFORE_AFTER_FULL_RELATION_CONTENT_DIGESTS");assert.equal(manifest.consistency.dump_scope,"COMPLETE_APPLICATION_DATABASE_LOGICAL_DUMP_NO_OWNER_OR_ACL");assert.equal(manifest.deployment.database_system_identifier,databaseSystemIdentifier);assert.equal(manifest.deployment.database_bytes,databaseBytes);assert.equal(manifest.application.web_image_digest,webImageDigest);assert.equal(manifest.application.worker_image_digest,workerImageDigest);assert.equal(manifest.reconciliation.contract,"chenyida-erp-backup-reconciliation/v1");
}));

test("shell contracts require signed encrypted offhost provenance before a distinct marked restore",async()=>{const protectedScripts=["backup-selfhost.sh","recover-backup-guard.sh","restore-selfhost.sh","verify-backup-selfhost.sh","publish-restore-receipt-selfhost.sh"];for(const name of protectedScripts){const source=await readFile(path.join(scripts,name),"utf8");assert.match(source,/acquire_chenyida_release_gate_lock/);assert.doesNotMatch(source,/--database-url|DATABASE_URL|--status-output|--receipt-output|--verification-level/);}const restore=await readFile(path.join(scripts,"restore-selfhost.sh"),"utf8"),publisher=await readFile(path.join(scripts,"publish-restore-receipt-selfhost.sh"),"utf8"),lockHelper=await readFile(path.join(scripts,"release-gate-lock.sh"),"utf8");assert.match(lockHelper,/chenyida-erp-release-gate-v1\.lock/);assert.match(restore,/RESTORE_SIGNED_ENCRYPTED_OFFHOST_TO_MARKED_DISPOSABLE_TEST_TARGET/);assert.match(restore,/materialize-for-restore/);assert.match(restore,/verify-materialized-for-restore/);assert.match(restore,/cleanup-materialized-for-restore/);assert.match(restore,/REMOVE_EXACT_VERIFIED_MATERIALIZATION/);assert.doesNotMatch(restore,/RESTORE_TO_MARKED_DISPOSABLE_TEST_TARGET/);assert.match(restore,/CREATE DATABASE %I WITH TEMPLATE template0/);assert.match(restore,/reconcile_create_result/);assert.match(restore,/DURING_DATABASE_CREATE_RESPONSE/);assert.match(restore,/expected-target-system-identifier/);assert.match(restore,/chenyida-erp-restore-cluster\/v2:TEST/);assert.match(restore,/--reflink=never/);assert.match(restore,/read-database-bytes/);assert.match(restore,/prepare-restore/);assert.match(restore,/publish-prepared-restore/);assert.doesNotMatch(restore,/finalize-restore|--reconciliation-sha256/);assert.match(publisher,/\.prepared-\$BACKUP_ID-\$RESTORE_RUN_ID\.json/);assert.match(publisher,/publish-prepared-restore/);assert.doesNotMatch(publisher,/psql|pg_restore|verify-restored-files/);});
