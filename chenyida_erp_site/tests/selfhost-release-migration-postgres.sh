...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/membership.log" 2>&1; then echo "migration role with inherited membership unexpectedly passed" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/reverse-membership.log" 2>&1; then echo "migration role granted to another role unexpectedly passed" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/role-setting.log" 2>&1; then echo "migration role with persistent GUC unexpectedly passed" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/schema-create-acl.log" 2>&1; then echo "public schema CREATE granted to an external role unexpectedly passed" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/schema-owner.log" 2>&1; then echo "public schema with a concrete role owner unexpectedly passed" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/precreated-history.log" 2>&1; then echo "pre-created empty migration history unexpectedly passed EMPTY authorization" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/untracked-empty.log" 2>&1; then echo "non-empty target without migration history unexpectedly migrated" >&2; exit 1; fi
...
if run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/untracked-collation.log" 2>&1; then echo "untracked public collation unexpectedly migrated" >&2; exit 1; fi
...
if run_controlled "$UNLOGGED_DB" "$UNLOGGED_DEPLOYMENT" "$EMPTY_ARTIFACTS" 0001_selfhost_baseline.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/unlogged-history.log" 2>&1; then echo "unlogged migration history unexpectedly passed" >&2; exit 1; fi
...
if run_controlled "$DEFAULT_DB" "$DEFAULT_DEPLOYMENT" "$EMPTY_ARTIFACTS" 0001_selfhost_baseline.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/default-history.log" 2>&1; then echo "side-effect-capable migration history default unexpectedly passed" >&2; exit 1; fi
...
if run_controlled "$ACL_DB" "$ACL_DEPLOYMENT" "$EMPTY_ARTIFACTS" 0001_selfhost_baseline.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/acl-history.log" 2>&1; then echo "shared migration history unexpectedly passed" >&2; exit 1; fi
...
if MIGRATION_DB_USER=cyd_release_migrator run_controlled "$EMPTY_DB" "$EMPTY_DEPLOYMENT" "$EMPTY_ARTIFACTS" 0046_runtime_lock_privilege_boundary.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/lock-rejected.log" 2>&1; then echo "concurrent migration unexpectedly succeeded" >&2; exit 1; fi
grep -q '^MIGRATION_ADVISORY_LOCK_UNAVAILABLE$' "$TASK_ROOT/lock-rejected.log"
kill "$LOCK_HOLDER_PID" >/dev/null 2>&1 || true
wait "$LOCK_HOLDER_PID" >/dev/null 2>&1 || true
LOCK_HOLDER_PID=""

# Existing data upgrades from the declared 0045 prefix, retains data, and is idempotent at 0046.
UPGRADE_DB=cyd_upgrade_release_test; UPGRADE_DEPLOYMENT=upgrade-release-test
create_target "$UPGRADE_DB" "$UPGRADE_DEPLOYMENT"
PREVIOUS="$TASK_ROOT/previous"; mkdir "$PREVIOUS"; cp -a "$SITE_ROOT/drizzle-postgres" "$PREVIOUS/drizzle-postgres"; rm "$PREVIOUS/drizzle-postgres/0046_runtime_lock_privilege_boundary.sql"
...
run_controlled "$INHERITED_DB" "$INHERITED_DEPLOYMENT" "$PREVIOUS_ARTIFACTS" EMPTY 0045_runtime_worker_readiness.sql "$PREVIOUS" > "$TASK_ROOT/inherited-base.log"
MIGRATION_0046_SHA=$(sha256sum "$SITE_ROOT/drizzle-postgres/0046_runtime_lock_privilege_boundary.sql" | awk '{print $1}')
psql -d "$INHERITED_DB" -v ON_ERROR_STOP=1 -c "create schema injected_history; create table injected_history.child() inherits (public.schema_migrations); insert into injected_history.child(version,checksum) values('0046_runtime_lock_privilege_boundary.sql','$MIGRATION_0046_SHA')" >/dev/null
if run_controlled "$INHERITED_DB" "$INHERITED_DEPLOYMENT" "$EMPTY_ARTIFACTS" 0045_runtime_worker_readiness.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/inherited-history.log" 2>&1; then echo "inherited migration history unexpectedly passed" >&2; exit 1; fi
grep -q '^MIGRATION_HISTORY_STRUCTURE_INVALID$' "$TASK_ROOT/inherited-history.log"
[ "$(psql -d "$INHERITED_DB" -Atc 'select count(*) from only public.schema_migrations')" = 45 ]
[ "$(psql -d "$INHERITED_DB" -Atc "select pg_catalog.to_regprocedure('public.cyd_web_lock_finance_ar_source(bigint)') is null")" = t ]
...
run_controlled "$SHADOW_NOW_DB" "$SHADOW_NOW_DEPLOYMENT" "$PREVIOUS_ARTIFACTS" EMPTY 0045_runtime_worker_readiness.sql "$PREVIOUS" > "$TASK_ROOT/shadow-now-base.log"
...
run_controlled "$SHADOW_NOW_DB" "$SHADOW_NOW_DEPLOYMENT" "$SHADOW_NOW_ARTIFACTS" 0045_runtime_worker_readiness.sql 0046_shadow_now_probe.sql "$SHADOW_NOW" > "$TASK_ROOT/shadow-now-upgrade.log"
...
run_controlled "$UPGRADE_DB" "$UPGRADE_DEPLOYMENT" "$PREVIOUS_ARTIFACTS" EMPTY 0045_runtime_worker_readiness.sql "$PREVIOUS" > "$TASK_ROOT/previous.log"
...
run_controlled "$UPGRADE_DB" "$UPGRADE_DEPLOYMENT" "$UPGRADE_ARTIFACTS" 0045_runtime_worker_readiness.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/upgrade.log"
[ "$(psql -d "$UPGRADE_DB" -Atc 'select payload from release_upgrade_sentinel where id=1')" = retained ]
run_controlled "$UPGRADE_DB" "$UPGRADE_DEPLOYMENT" "$UPGRADE_ARTIFACTS" 0046_runtime_lock_privilege_boundary.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/repeat.log"
...
if run_controlled "$UPGRADE_DB" "$UPGRADE_DEPLOYMENT" "$UPGRADE_ARTIFACTS" 0046_runtime_lock_privilege_boundary.sql 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/checksum.log" 2>&1; then echo "checksum conflict unexpectedly succeeded" >&2; exit 1; fi
grep -q '^APPLIED_MIGRATION_CHECKSUM_MISMATCH$' "$TASK_ROOT/checksum.log"
[ "$(psql -d "$UPGRADE_DB" -Atc 'select count(*) from public.schema_migrations')" = 46 ]
...
DISALLOWED="$TASK_ROOT/disallowed"; mkdir "$DISALLOWED"; cp -a "$SITE_ROOT/drizzle-postgres" "$DISALLOWED/drizzle-postgres"; printf 'select 1;\n' > "$DISALLOWED/drizzle-postgres/0047_disallowed.sql"
if run_controlled "$DISALLOWED_DB" "$DISALLOWED_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$DISALLOWED" > "$TASK_ROOT/disallowed.log" 2>&1; then echo "disallowed migration unexpectedly succeeded" >&2; exit 1; fi
...
run_controlled "$ROLLBACK_DB" "$ROLLBACK_DEPLOYMENT" "$EMPTY_ARTIFACTS" EMPTY 0046_runtime_lock_privilege_boundary.sql "$SITE_ROOT" > "$TASK_ROOT/rollback-base.log"
ROLLBACK="$TASK_ROOT/rollback"; mkdir "$ROLLBACK"; cp -a "$SITE_ROOT/drizzle-postgres" "$ROLLBACK/drizzle-postgres"
printf 'create table release_failure_probe(id integer);\nselect 1/0;\n' > "$ROLLBACK/drizzle-postgres/0047_failure_probe.sql"
...
if run_controlled "$ROLLBACK_DB" "$ROLLBACK_DEPLOYMENT" "$ROLLBACK_ARTIFACTS" 0046_runtime_lock_privilege_boundary.sql 0047_failure_probe.sql "$ROLLBACK" > "$TASK_ROOT/rollback.log" 2>&1; then echo "failing migration unexpectedly succeeded" >&2; exit 1; fi
...
[ "$(psql -d "$ROLLBACK_DB" -Atc "select count(*) from public.schema_migrations where version='0047_failure_probe.sql'")" = 0 ]
