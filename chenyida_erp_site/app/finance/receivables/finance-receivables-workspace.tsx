...
export function FinanceReceivablesWorkspace() {
  ...
  return <main className="sourcing-shell">
    ...
    <header className="sourcing-header"><div><Link href="/" className="sourcing-back">← 经营工作台</Link><p className="sourcing-kicker">销售应收交接</p><h1>销售发货应收交接</h1><p>Customer、Currency、Amount 全部继承 Shipment Source，浏览器不提交总金额或客户。</p></div></header>
    ...
    <section className="sourcing-panel"><h2>AR 未结</h2>{ars.map((row) => <article className="sourcing-card" key={row.id}><b>{row.doc_code} · {statusLabel(row.status)}</b><p>应收 {row.total_amount} · 已结 {row.settled_amount}</p></article>)}</section>
