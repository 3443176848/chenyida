...
type TraceEvent = { id: number; action: string; type: string; actor: string; occurred_at: string; timezone: "Asia/Shanghai"; request_id: string; result: "SUCCESS"; evidence_source: string };
...
type RequestDetail = {
  ...
  decision_counts: { accept_count: number; return_count: number };
...
export function isPurchaseAcceptancePreviewComplete(detail: RequestDetail) {
  return Boolean(
    ...
    && Number.isSafeInteger(detail.decision_counts.accept_count) && detail.decision_counts.accept_count >= 0
...
function Status({ value }: { value: string }) { return <span className={`planning-status status-${value.toLowerCase()}`}>{labels[value] || statusLabel(value)}</span>; }
...
function ConfirmationDialog({ dialogId, title, kicker, className = "", busy, confirmDisabled = false, confirmLabel, busyLabel = "正在提交…", confirmClassName = "", onCancel, onConfirm, children }: { dialogId: string; title: string; kicker: string; className?: string; busy: boolean; confirmDisabled?: boolean; confirmLabel: string; busyLabel?: string; confirmClassName?: string; onCancel: () => void; onConfirm: () => void; children: ReactNode }) {
...
export function MaterialRequirementWorkspace() {
  ...
  function generate(event: FormEvent<HTMLFormElement>) { event.preventDefault(); if (selected) void write(`/api/planning-packages/${selected.id}/material-requirement-plans`, { required_date: String(new FormData(event.currentTarget).get("required_date") || "") }, "已重新核算并保存草稿预览"); }
...
export function PurchaseRequestWorkspace() {
  ...
  const [decisionPrompt, setDecisionPrompt] = useState<DecisionPrompt | null>(null), [decisionRefresh, setDecisionRefresh] = useState<DecisionRefresh | null>(null), [receipt, setReceipt] = useState<DecisionReceipt | null>(null), [busy, setBusy] = useState(false), [notice, setNotice] = useState(""), [error, setError] = useState("");
  const mutate = useSessionMutation(session); const decisionInFlight = useRef(false); const decisionRefreshInFlight = useRef(false); const decisionRefreshGeneration = useRef(0);
  ...
  async function requestDecision(kind: "accept" | "return", reason = "") {
    ...
    const refreshGeneration = ++decisionRefreshGeneration.current;
    decisionRefreshInFlight.current = true; setDecisionRefresh({ requestId: detail.header.id, requestCode: detail.header.request_code }); setBusy(true);
    try {
      const refreshed = (await api<{ data: RequestDetail }>(`/api/purchase-requests/${detail.header.id}`)).data;
      if (refreshGeneration !== decisionRefreshGeneration.current) return;
      if (refreshed.header.status !== "SUBMITTED") { setDetail(refreshed); setError("采购申请状态已变化，请重新核对后处理。"); return; }
      if (!isPurchaseAcceptancePreviewComplete(refreshed)) { setError("采购接收确认所需的完整追溯、决策计数或九项当前供应未全部取得，已禁止继续接收。"); return; }
      setDetail(refreshed); setDecisionPrompt({ kind, reason, detail: refreshed });
    } catch (refreshError) { setError(errorMessage(refreshError)); }
    finally { decisionRefreshInFlight.current = false; setDecisionRefresh((current) => refreshGeneration === decisionRefreshGeneration.current ? null : current); setBusy(false); }
  }
  ...
  const closeDecisionRefresh = useCallback(() => { decisionRefreshGeneration.current += 1; setDecisionRefresh(null); }, []);
  async function confirmDecision() {
    if (!session || !decisionPrompt || decisionInFlight.current) return;
    if (decisionPrompt.kind === "accept" && !isPurchaseAcceptancePreviewComplete(decisionPrompt.detail)) { setError("确认资料不完整，已禁止接收采购申请。"); return; }
...
function PurchaseSupplyRefreshDialog({ refresh, onCancel }: { refresh: DecisionRefresh; onCancel: () => void }) {
  return <ConfirmationDialog dialogId="purchase-supply-refresh-title" title={`接收前复核 ${refresh.requestCode}`} kicker="实时供应刷新" className="planning-handoff-dialog" busy={false} confirmDisabled confirmLabel="等待完整数据" onCancel={onCancel} onConfirm={() => undefined}><section className="planning-confirm-section purchase-supply-refresh" aria-live="polite"><h3>正在重新读取当前供应</h3><p>正在按采购申请 ID {refresh.requestId} 重新读取三段权威事件、接收/退回决策计数和每个物料的九项当前供应。</p><p>完整数据返回前不能确认接收；取消、关闭或按 ESC 不会发出业务写请求。</p></section></ConfirmationDialog>;
}
...
function EventEvidence({ title, event }: { title: string; event: TraceEvent | null }) {
  ...
  return <article className="planning-event"><div className="planning-event-heading"><div><code className="planning-event-code">{event.action}</code><b>{title}</b></div><span className="planning-event-result success">{statusLabel(event.result)}</span></div><dl><div><dt>动作 / 类型</dt><dd>{event.action} / {event.type}</dd></div><div><dt>操作者</dt><dd>{event.actor}</dd></div><div><dt>时间</dt><dd>{formatShanghaiTime(event.occurred_at)}</dd></div><div><dt>展示时区</dt><dd>{event.timezone}</dd></div><div><dt>结果</dt><dd>成功 · 不可变事件已提交</dd></div><div><dt>证据来源</dt><dd>{event.evidence_source === "PACKAGE_EVENT" ? "Package Event" : "Material Requirement Event"}</dd></div></dl><CopyValue label="请求号" value={event.request_id} /></article>;
}
...
function PurchaseDecisionEvidence({ detail }: { detail: RequestDetail }) {
  ...
  return <section className="planning-card purchase-trace-section purchase-decision-evidence purchase-decision-counts" data-purchase-decision-evidence="complete"><div className="planning-event-heading"><div><code className="planning-event-code">{event.action}</code><h3>采购决策凭证</h3></div><span className="planning-event-result success">{statusLabel(event.result)}</span></div><dl className="planning-facts planning-package-facts"><div><dt>Purchase Request ID</dt><dd>{detail.purchase_request.id}</dd></div><div><dt>PRQ</dt><dd>{detail.purchase_request.request_code}</dd></div><div><dt>决策</dt><dd>{accepted ? "采购接收" : "采购退回"}</dd></div><div><dt>业务事件类型</dt><dd>{labels[event.type] || statusLabel(event.type)}</dd></div><div><dt>操作者</dt><dd>{event.actor}</dd></div><div><dt>时间</dt><dd>{formatShanghaiTime(event.occurred_at)}</dd></div><div><dt>展示时区</dt><dd>{event.timezone}</dd></div><div><dt>结果</dt><dd>{statusLabel(event.result)}</dd></div><div><dt>接收事件数量</dt><dd>{detail.decision_counts.accept_count}</dd></div><div><dt>退回事件数量</dt><dd>{detail.decision_counts.return_count}</dd></div></dl><CopyValue label="请求号" value={event.request_id} /><p className="planning-evidence-note">结果为成功，表示该服务专用、不可变的采购申请决策事件已随同一事务提交；它不是计划交接包接收，也不是从当前登录用户、页面状态或队列数量推断。</p></section>;
}
...
function PurchaseRequestView({ detail, canDecide, busy, onAccept, onReturn }: { detail: RequestDetail; canDecide: boolean; busy: boolean; onAccept: () => void; onReturn: (event: FormEvent<HTMLFormElement>) => void }) {
  const request = detail.purchase_request;
  return <><div className="planning-title"><div><p className="planning-kicker">采购申请追溯</p><h2>{request.request_code} · {detail.header.project_name}</h2></div><Status value={request.status} /></div>
    <section className="planning-card purchase-trace-section"><h3>Package 与接收谱系</h3><dl className="planning-facts planning-package-facts"><div><dt>来源 Package</dt><dd>ID {detail.package.id}/v{detail.package.version_no}</dd></div><div><dt>Package 状态</dt><dd>{statusLabel(detail.package.status)}</dd></div><div><dt>项目</dt><dd>{detail.package.project_code}</dd></div><div><dt>固定产品行</dt><dd>{detail.package.items.length} 行</dd></div></dl><CopyValue label="完整 Package SHA-256 摘要" value={detail.package.digest} /><div className="purchase-source-items">{detail.package.items.map((item) => <article key={item.id}><b>Product {item.product_version_code}</b><span>{item.product_name} · 内部产品编码 {item.product_code}</span><span>BOM {item.bom_version_code}（{item.bom_code}）</span><span>Unit Resolution {item.unit_resolution_version_no ? `v${item.unit_resolution_version_no}` : "未记录"} · {item.unit_code}</span></article>)}</div><EventEvidence title="Package 接收" event={detail.package.accept_event} /><p className="planning-gate-evidence">该接收动作仅确认工程 Package 进入计划阶段，不会自动生成采购单据。</p></section>
    <section className="planning-card purchase-trace-section"><h3>Material Requirement Plan 谱系</h3><dl className="planning-facts planning-package-facts"><div><dt>稳定 ID</dt><dd>Material Requirement Plan ID {detail.plan.id}</dd></div><div><dt>计划版本</dt><dd>v{detail.plan.version_no}</dd></div><div><dt>来源 Package</dt><dd>ID {detail.plan.source_package_id}/v{detail.plan.source_package_version_no}</dd></div><div><dt>采购交接状态</dt><dd>{labels[detail.plan.status] || statusLabel(detail.plan.status)}</dd></div><div><dt>创建 / 计算人</dt><dd>{detail.plan.prepared_by}</dd></div><div><dt>计算时间</dt><dd>{formatShanghaiTime(detail.plan.calculated_at)}</dd></div><div><dt>数据快照截止时间</dt><dd>{formatShanghaiTime(detail.plan.snapshot_cutoff_at)}</dd></div><div><dt>提交锁定复核时间</dt><dd>{formatShanghaiTime(detail.plan.submission_revalidated_at)}</dd></div></dl><EventEvidence title="计划生成事件" event={detail.plan.generated_event} /><p className="planning-gate-evidence">该字段记录计划部→采购部交接状态；Plan v{detail.plan.version_no} 计算快照、行项目、分配及来源摘要仍不可变，“已接收”不表示快照被改写。</p><p className="planning-empty-fact">该版本未采集计划说明</p></section>
    ...
    <section className="planning-card purchase-trace-section purchase-supply-section" data-supply-section="current"><h3>2. 当前供应状态</h3><p className="purchase-query-time"><b>查询时间：</b>{formatShanghaiTime(detail.current_supply_observed_at)}</p><p className="planning-evidence-note">只汇总本 PRQ 已授权行对应物料与单位。库存位置域为 MAIN，并汇总该域的全部无批次与批次位置；计划分配不计入正式库存预留。</p><div className="purchase-line-cards">{detail.lines.map((line) => <CurrentSupplyCard key={line.id} line={line} />)}</div><details className="purchase-supply-formulas"><summary>查看权威供应公式与在途边界</summary><div><p><code>库存可用 = Σ在手 - Σ正式预留 - Σ品质冻结</code>（每个库存位置由数据库约束保证结果非负）</p><p><code>未分配库存可用 = max(库存可用 - 有效计划库存分配, 0)</code></p><p><code>未分配在途可用 = max(有效在途 - 有效计划在途分配, 0)</code></p><p>有效计划仅指已提交 / 已接收。有效在途仅含截至 PRQ 需求日的处理中 / 部分收货 PO 剩余量；存在交付计划时，仅待处理 / 部分完成的计划剩余量有效，已完成 / 已取消 / 已关闭排除。</p><p>已过账收货已通过已收数量从在途扣除并进入库存。当前模型没有“已到货但未完成入库”的独立数量字段，也没有“其他不可用”独立字段，因此均不伪造数量。</p></div></details></section>
    ...
    <section className="planning-card purchase-trace-section"><h3>PRQ 提交凭证</h3><dl className="planning-facts planning-package-facts"><div><dt>稳定 ID</dt><dd>Purchase Request ID {request.id}</dd></div><div><dt>编号</dt><dd>{request.request_code}</dd></div><div><dt>PRQ 状态</dt><dd>{labels[request.status] || statusLabel(request.status)}</dd></div><div><dt>来源计划</dt><dd>ID {request.source_plan_id}/v{request.source_plan_version_no}</dd></div><div><dt>项目</dt><dd>{request.project_code}</dd></div><div><dt>需求日期</dt><dd>{dateOnly(request.required_date)}</dd></div><div><dt>提交人</dt><dd>{request.submitted_by}</dd></div><div><dt>提交时间</dt><dd>{formatShanghaiTime(request.submitted_at)}</dd></div><div><dt>行数 / 合计</dt><dd>{request.line_count} 行 · <Quantity value={request.total_requested_quantity} /></dd></div></dl><EventEvidence title="PRQ 提交" event={request.submit_event} /><p className="planning-gate-evidence">PRQ未单独版本化；固定引用需求计划v{request.source_plan_version_no}</p><dl className="planning-facts planning-package-facts purchase-empty-facts"><div><dt>供应商</dt><dd>未选择供应商</dd></div><div><dt>价格</dt><dd>未填写价格</dd></div><div><dt>接收人</dt><dd>未指定接收人</dd></div><div><dt>处理时限</dt><dd>未配置处理时限</dd></div></dl><p className="planning-empty-fact">该版本未采集采购交接说明</p></section>
    {request.status === "SUBMITTED" ? <section className="planning-card purchase-trace-section purchase-decision-counts"><h3>不可变采购决策计数</h3><dl className="planning-facts"><div><dt>接收事件数量</dt><dd>{detail.decision_counts.accept_count}</dd></div><div><dt>退回事件数量</dt><dd>{detail.decision_counts.return_count}</dd></div></dl><p className="planning-evidence-note">计数直接来自当前 PRQ 的不可变采购接收 / 采购退回事件，不以状态或队列数量推断。</p></section> : <PurchaseDecisionEvidence detail={detail} />}
...
function AcceptanceSupplyCard({ line }: { line: RequestLine }) {
  const supply = line.current_supply;
  return <article className="purchase-accept-material-card"><MaterialHeading line={line} />{supply ? <>
    <h4>提交时固定数量</h4><dl className="purchase-supply-grid purchase-accept-snapshot-grid">
      <div><dt>毛需求</dt><dd><Quantity value={line.gross_requirement} unit={line.unit_code} /></dd></div>
      <div><dt>快照库存可用</dt><dd><Quantity value={line.stock_available} unit={line.unit_code} /></dd></div>
      <div><dt>快照库存分配</dt><dd><Quantity value={line.stock_allocated} unit={line.unit_code} /></dd></div>
      <div><dt>快照在途可用</dt><dd><Quantity value={line.eligible_inbound} unit={line.unit_code} /></dd></div>
      <div><dt>快照在途分配</dt><dd><Quantity value={line.inbound_allocated} unit={line.unit_code} /></dd></div>
      <div><dt>净采购</dt><dd><Quantity value={line.net_purchase_requirement} unit={line.unit_code} /></dd></div>
      <div className="purchase-prq-quantity"><dt>PRQ 申请量</dt><dd><Quantity value={line.requested_quantity} unit={line.unit_code} /></dd></div>
    </dl>
    <h4>当前供应九项</h4><dl className="purchase-supply-grid purchase-current-nine" data-current-supply-nine="complete">
      <div data-supply-field="on_hand_qty"><dt>当前在手总量</dt><dd><Quantity value={supply.on_hand_qty} unit={line.unit_code} /></dd></div>
      <div data-supply-field="reserved_qty"><dt>当前正式预留量</dt><dd><Quantity value={supply.reserved_qty} unit={line.unit_code} /></dd></div>
      <div data-supply-field="frozen_qty"><dt>当前品质冻结量</dt><dd><Quantity value={supply.frozen_qty} unit={line.unit_code} /></dd></div>
      <div data-supply-field="inventory_available_qty"><dt>当前库存可用量</dt><dd><Quantity value={supply.inventory_available_qty} unit={line.unit_code} /></dd></div>
      <div data-supply-field="stock_allocated_to_active_plans_qty"><dt>当前计划库存分配量</dt><dd><Quantity value={supply.stock_allocated_to_active_plans_qty} unit={line.unit_code} /></dd></div>
      <div className="purchase-key-current" data-supply-field="unallocated_inventory_available_qty"><dt>当前未分配库存可用量</dt><dd><Quantity value={supply.unallocated_inventory_available_qty} unit={line.unit_code} /></dd></div>
      <div data-supply-field="effective_inbound_qty"><dt>当前有效在途总量</dt><dd><Quantity value={supply.effective_inbound_qty} unit={line.unit_code} /></dd></div>
      <div data-supply-field="inbound_allocated_to_active_plans_qty"><dt>当前计划在途分配量</dt><dd><Quantity value={supply.inbound_allocated_to_active_plans_qty} unit={line.unit_code} /></dd></div>
      <div className="purchase-key-current" data-supply-field="unallocated_inbound_available_qty"><dt>当前未分配在途可用量</dt><dd><Quantity value={supply.unallocated_inbound_available_qty} unit={line.unit_code} /></dd></div>
    </dl>
  </> : <State error>九项当前供应未完整取得；确认接收保持禁用。</State>}</article>;
}
...
function PurchaseDecisionDialog({ prompt, busy, onCancel, onConfirm }: { prompt: DecisionPrompt; busy: boolean; onCancel: () => void; onConfirm: () => void }) {
  ...
  if (prompt.kind === "return") return <ConfirmationDialog dialogId="purchase-return-title" title="确认退回计划部门修订" kicker="退回确认" className="planning-handoff-dialog" busy={busy} confirmLabel="确认退回" confirmClassName="planning-danger" onCancel={onCancel} onConfirm={onConfirm}><section className="planning-confirm-section"><h3>PRQ 与来源</h3><dl className="planning-facts"><div><dt>PRQ</dt><dd>{request.request_code} · ID {request.id}</dd></div><div><dt>项目</dt><dd>{request.project_code}</dd></div><div><dt>来源 Package</dt><dd>ID {detail.package.id}/v{detail.package.version_no}</dd></div><div><dt>来源计划</dt><dd>ID {detail.plan.id}/v{detail.plan.version_no}</dd></div></dl></section><section className="planning-confirm-section"><h3>退回后果</h3><div className="planning-confirm-reason"><b>必填退回原因</b><p>{prompt.reason}</p></div><ul className="planning-consequence-list"><li>PRQ 返回计划部门修订。</li><li>写入一次不可变采购退回事件。</li><li>不修改原需求计划及提交时分配快照。</li><li>不创建 RFQ、定标、PO、收货、库存流水或 AP。</li></ul></section></ConfirmationDialog>;
  const complete = isPurchaseAcceptancePreviewComplete(detail);
  return <ConfirmationDialog dialogId="purchase-accept-title" title={`确认接收 ${request.request_code}`} kicker="采购接收确认" className="planning-handoff-dialog purchase-supply-confirm-dialog" busy={busy} confirmDisabled={!complete} confirmLabel={complete ? "确认接收" : "资料不完整，禁止接收"} onCancel={onCancel} onConfirm={onConfirm}>
    <section className="planning-confirm-section"><h3>PRQ 与项目</h3><dl className="planning-facts"><div><dt>Purchase Request ID</dt><dd>{request.id}</dd></div><div><dt>PRQ 编号</dt><dd>{request.request_code}</dd></div><div><dt>项目</dt><dd>{request.project_code}</dd></div><div><dt>需求日期</dt><dd>{dateOnly(request.required_date)}</dd></div><div><dt>状态</dt><dd>{labels[request.status] || statusLabel(request.status)}</dd></div><div><dt>当前供应查询时间</dt><dd>{formatShanghaiTime(detail.current_supply_observed_at)}</dd></div><div><dt>接收前刷新</dt><dd>已重新读取当前供应</dd></div></dl></section>
    <section className="planning-confirm-section"><h3>Package 接收完整凭证</h3><dl className="planning-facts"><div><dt>Package</dt><dd>ID {detail.package.id}/v{detail.package.version_no}</dd></div><div><dt>状态</dt><dd>{statusLabel(detail.package.status)}</dd></div></dl><CopyValue label="完整 Package SHA-256 摘要" value={detail.package.digest} /><EventEvidence title="Package 接收" event={detail.package_accept_event} /></section>
    <section className="planning-confirm-section"><h3>计划生成完整凭证</h3><dl className="planning-facts"><div><dt>物料需求计划</dt><dd>ID {detail.plan.id}/v{detail.plan.version_no}</dd></div><div><dt>计算时间</dt><dd>{formatShanghaiTime(detail.plan.calculated_at)}</dd></div><div><dt>快照截止时间</dt><dd>{formatShanghaiTime(detail.plan.snapshot_cutoff_at)}</dd></div></dl><EventEvidence title="计划生成" event={detail.plan_generate_event} /></section>
    <section className="planning-confirm-section"><h3>PRQ 提交完整凭证</h3><EventEvidence title="PRQ 提交" event={detail.prq_submit_event} /></section>
    <section className="planning-confirm-section purchase-decision-counts"><h3>接收前显式决策计数</h3><dl className="planning-facts"><div><dt>接收前接收事件</dt><dd>{detail.decision_counts.accept_count}</dd></div><div><dt>接收前退回事件</dt><dd>{detail.decision_counts.return_count}</dd></div></dl><p>以上计数来自当前 PRQ 的不可变决策事件，不以“待接收”或“已处理”队列数量代替。</p></section>
    <section className="planning-confirm-section"><h3>四项固定数量与九项当前供应（{detail.lines.length} 个 Material）</h3><div className="purchase-confirm-supply-cards">{detail.lines.map((line) => <AcceptanceSupplyCard key={line.id} line={line} />)}</div><p className="planning-warning">当前供应不会改写已提交快照、净采购或 PRQ 申请量。</p></section>
    <section className="planning-confirm-section purchase-accept-boundaries"><h3>供应公式与边界</h3><ul className="planning-consequence-list"><li><b>库存可用 = 在手 - 正式预留 - 品质冻结。</b></li><li><b>未分配库存 = max（库存可用 - 计划库存分配，0）。</b></li><li><b>未分配在途 = max（有效在途 - 计划在途分配，0）。</b></li><li>计划分配不等于正式 reserved_qty。</li><li>有效在途排除已收货、完成、取消及关闭来源。</li><li>当前供应只用于本次复核，不会改写已提交快照。</li></ul></section>
    <section className="planning-confirm-section"><h3>接收后果</h3><ul className="planning-consequence-list"><li>只新增一条不可变采购接收事件。</li><li>不修改交接包、计划、PRQ 明细、库存、正式预留或计划分配。</li><li><b>不自动创建 RFQ、报价、定标、PO、交付计划、收货单、库存流水、AP 或工单。</b></li><li>当前未指定具体处理人，也未配置处理时限。</li></ul></section>
    <section className="planning-confirm-section planning-next-stage"><h3>下一阶段</h3><p><b>采购寻源、询价和报价比较由后续独立受控操作完成；本次接收不会开始上述流程。</b></p></section>
    {!complete ? <State error>完整追溯、显式决策计数或九项供应缺失；确认按钮保持禁用。</State> : null}
  </ConfirmationDialog>;
}
...
function DecisionReceiptView({ receipt, onOpen }: { receipt: DecisionReceipt; onOpen: () => void }) {
  ...
  return <div className="planning-receipt" role="status"><p className="planning-kicker">采购决策凭证</p><h2>操作完成凭证</h2><dl className="planning-facts"><div><dt>结果</dt><dd>成功</dd></div><div><dt>项目</dt><dd>{request.project_code}</dd></div><div><dt>采购交接状态</dt><dd>{labels[receipt.detail.plan.status] || statusLabel(receipt.detail.plan.status)}</dd></div><div><dt>PRQ 状态</dt><dd>{labels[request.status] || statusLabel(request.status)}</dd></div></dl><PurchaseDecisionEvidence detail={receipt.detail} />{accepted ? <p>下一队列：采购寻源与询价；接收本身未创建 RFQ、定标、PO、收货或 AP。</p> : <><div className="planning-confirm-reason"><b>数据库保存的退回原因</b><p>{receipt.detail.header.return_reason}</p></div><p>下一队列：计划部门修订；原计划快照未修改。</p></>}<button type="button" onClick={onOpen}>从已处理记录查看凭证</button></div>;
}
