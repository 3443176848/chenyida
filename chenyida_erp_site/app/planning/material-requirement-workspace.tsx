...
function PurchaseSupplyRefreshDialog({ refresh, onCancel }: { refresh: DecisionRefresh; onCancel: () => void }) {
  return <ConfirmationDialog dialogId="purchase-supply-refresh-title" title={`接收前复核 ${refresh.requestCode}`} kicker="LIVE SUPPLY REFRESH" className="planning-handoff-dialog" busy={false} confirmDisabled confirmLabel="等待完整数据" onCancel={onCancel} onConfirm={() => undefined}><section className="planning-confirm-section purchase-supply-refresh" aria-live="polite"><h3>正在重新读取当前供应</h3><p>正在按 Purchase Request ID {refresh.requestId} 重新读取三段权威事件、ACCEPT/RETURN 决策计数和每个 Material 的九项当前供应。</p><p>完整数据返回前不能确认接收；取消、关闭或按 ESC 不会发出业务写请求。</p></section></ConfirmationDialog>;
}
...
function PurchaseDecisionDialog({ prompt, busy, onCancel, onConfirm }: { prompt: DecisionPrompt; busy: boolean; onCancel: () => void; onConfirm: () => void }) {
  ...
  return <ConfirmationDialog dialogId="purchase-accept-title" title={`确认接收 ${request.request_code}`} kicker="PURCHASE ACCEPT CONFIRMATION" className="planning-handoff-dialog purchase-supply-confirm-dialog" busy={busy} confirmDisabled={!complete} confirmLabel={complete ? "确认接收" : "资料不完整，禁止接收"} onCancel={onCancel} onConfirm={onConfirm}>
    <section className="planning-confirm-section"><h3>PRQ 与项目</h3><dl className="planning-facts"><div><dt>Purchase Request ID</dt><dd>{request.id}</dd></div><div><dt>PRQ 编号</dt><dd>{request.request_code}</dd></div><div><dt>项目</dt><dd>{request.project_code}</dd></div><div><dt>需求日期</dt><dd>{dateOnly(request.required_date)}</dd></div><div><dt>状态</dt><dd>{labels[request.status] || request.status}</dd></div><div><dt>当前供应查询时间</dt><dd>{formatShanghaiTime(detail.current_supply_observed_at)}</dd></div><div><dt>接收前刷新</dt><dd>已重新读取当前供应</dd></div></dl></section>
    <section className="planning-confirm-section"><h3>Package ACCEPT 完整凭证</h3><dl className="planning-facts"><div><dt>Package</dt><dd>ID {detail.package.id}/v{detail.package.version_no}</dd></div><div><dt>状态</dt><dd>{detail.package.status}</dd></div></dl><CopyValue label="完整 Package SHA-256 摘要" value={detail.package.digest} /><EventEvidence title="Package ACCEPT" event={detail.package_accept_event} /></section>
    <section className="planning-confirm-section"><h3>Plan GENERATE 完整凭证</h3><dl className="planning-facts"><div><dt>Material Requirement Plan</dt><dd>ID {detail.plan.id}/v{detail.plan.version_no}</dd></div><div><dt>计算时间</dt><dd>{formatShanghaiTime(detail.plan.calculated_at)}</dd></div><div><dt>快照截止时间</dt><dd>{formatShanghaiTime(detail.plan.snapshot_cutoff_at)}</dd></div></dl><EventEvidence title="Plan GENERATE" event={detail.plan_generate_event} /></section>
    <section className="planning-confirm-section"><h3>PRQ SUBMIT 完整凭证</h3><EventEvidence title="PRQ SUBMIT" event={detail.prq_submit_event} /></section>
    <section className="planning-confirm-section purchase-decision-counts"><h3>接收前显式决策计数</h3><dl className="planning-facts"><div><dt>接收前 ACCEPT</dt><dd>{detail.decision_counts.accept_count}</dd></div><div><dt>接收前 RETURN</dt><dd>{detail.decision_counts.return_count}</dd></div></dl><p>以上计数来自当前 PRQ 的不可变决策事件，不以“待接收”或“已处理”队列数量代替。</p></section>
    ...
    <section className="planning-confirm-section"><h3>接收后果</h3><ul className="planning-consequence-list"><li>只新增一条不可变 Purchase ACCEPT（PURCHASE_ACCEPTED）事件。</li><li>不修改 Package、Plan、PRQ 明细、库存、正式预留或 Planning Allocation。</li><li><b>不自动创建 RFQ、Quote、Award、PO、Delivery Plan、Receipt、Ledger、AP 或 Work Order。</b></li><li>当前未指定具体处理人，也未配置处理时限。</li></ul></section>
