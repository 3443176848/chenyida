...
export type DashboardSnapshot = Readonly<{
  ...
  pending_planning_handoffs:number;accepted_planning_packages:number;pending_purchase_requests:number;pending_jobs:number;failed_jobs:number;migrations:MigrationStatus[];recent_events:SafeBusinessEvent[];recent_audits:SafeAuditEvent[];generated_at:string;
}>;
...
export type DashboardSummary = Readonly<{generated_at:string;authority:"Node/PostgreSQL";consistency:"REPEATABLE_READ_READ_ONLY";inventory_quantity_aggregated:false;groups:Partial<Record<DashboardDomain,unknown>>;modules:DashboardModule[];
  total_items?:number;total_mappings?:number;pending?:number;auto_count?:number;suspect_count?:number;new_count?:number;total_customers?:number;total_suppliers?:number;total_products?:number;total_boms?:number;pending_planning_handoffs?:number;accepted_planning_packages?:number;pending_purchase_requests?:number;total_pos?:number;open_pos?:number;total_work_orders?:number;active_work_orders?:number;total_quotations?:number;open_quotations?:number;total_sales_orders?:number;open_sales_orders?:number;total_quality_inspections?:number;open_quality_issues?:number;receivable_balance?:string;payable_balance?:string;pending_jobs?:number;failed_jobs?:number}>;
