...
import type { RequirementMutationMeta, RequirementMutationResult, RequirementWork } from "./types.ts";
...
export class MaterialRequirementRepository {
  ...
  private async consumeWriteRate(actor: string, keyDigest: string) {
    ...
    catch (error) { await client.query("rollback").catch(() => undefined); throw mapMaterialRequirementError(error); } finally { client.release(); }
    if (limited) throw new MaterialRequirementError("RATE_LIMITED", "物料需求计划写操作过于频繁，请稍后重试", 429);
  }

  async execute(meta: RequirementMutationMeta, work: RequirementWork): Promise<RequirementMutationResult> {
    ...
    try {
      ...
      if (existing.rows[0]) { if (existing.rows[0].request_digest !== meta.requestDigest) throw new MaterialRequirementError("IDEMPOTENCY_CONFLICT", "同一 Idempotency-Key 不能用于不同请求", 409); await client.query("commit"); const body = existing.rows[0].response as Record<string, unknown>; return { status: Number(existing.rows[0].status_code), body, objectId: Number(body.plan_id || body.purchase_request_id || 0), replayed: true }; }
      await client.query("select set_config('cyd.material_requirement_service_write','allowed',true)"); const result = await work(client);
      await client.query("insert into audit_log(username,action,detail,request_id,result,route_code,operation_id,idempotency_key_digest,old_version,new_version,retention_until) values($1,$2,$3,$4,'success','MATERIAL_REQUIREMENT',$5,$6,$7,$8,now()+interval '1095 days')", [meta.actor.username, meta.action, { object_id: result.objectId }, meta.requestId, meta.operationId, meta.keyDigest, result.oldVersion ?? null, result.newVersion ?? null]);
