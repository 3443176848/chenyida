...
export async function handleMasterDataApi(request: Request, dependencies: Dependencies): Promise<Response | null> {
  ...
  try {
    ...
    else if (path === "/api/mappings" && request.method === "POST") { action = "SUPPLIER_MAPPING_CREATED"; requirePermission(dependencies.actor, "master.supplier_mapping.manage"); result = await service.createMapping(mutationMeta(request, dependencies, path, action, parsed.digest), parsed.value); }
    ...
    else if (/^\/api\/mappings\/[1-9]\d*\/status$/.test(path) && request.method === "PATCH") { const id = Number(path.split("/")[3]); action = "SUPPLIER_MAPPING_STATUS_CHANGED"; requirePermission(dependencies.actor, "master.supplier_mapping.manage"); result = await service.setMappingStatus(id, mutationMeta(request, dependencies, path, action, parsed.digest), parsed.value); }
