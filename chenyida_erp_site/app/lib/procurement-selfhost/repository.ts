...
import type { ProcurementMeta, ProcurementResult, ProcurementWork } from "./types.ts";
export class ProcurementRepository {
  ...
  async consumeWriteRate(actor: string, keyDigest: string) {
    ...
    const client = await this.pool.connect(); let limited = false;
    try {
      await client.query("begin");
      const result = await client.query<{ attempt_count: number; new_key_count: number }>(`insert into identity_write_rate_limit_buckets(username,bucket_start,attempt_count,new_key_count,rejected_count,updated_at)
    ...
    } catch (error) { await client.query("rollback").catch(() => undefined); throw mapProcurementError(error); } finally { client.release(); }
    if (limited) throw new ProcurementError("RATE_LIMITED", "采购写操作过于频繁，请稍后重试", 429);
  }
  async nextCode(client: PoolClient, sequence: "PURCHASE_ORDER" | "PURCHASE_RECEIPT", prefix: "PO" | "PR") { const row = await client.query<{ current_value: string }>(`insert into business_code_sequences(sequence_code,current_value,version,updated_at) values($1,1,1,now()) on conflict(sequence_code) do update set current_value=business_code_sequences.current_value+1,version=business_code_sequences.version+1,updated_at=now() returning current_value`, [sequence]); return `${prefix}-${String(row.rows[0].current_value).padStart(8, "0")}`; }
  async execute(meta: ProcurementMeta, work: ProcurementWork): Promise<ProcurementResult> {
    ...
    try {
      ...
      if (existing.rows[0]) { if (existing.rows[0].request_digest !== meta.requestDigest) throw new ProcurementError("IDEMPOTENCY_CONFLICT", "同一 Idempotency-Key 不能用于不同请求", 409); await client.query("commit"); return { status: Number(existing.rows[0].status_code), body: existing.rows[0].response, replayed: true }; }
      await client.query("select set_config('cyd.procurement_service_write','allowed',true),set_config('cyd.inventory_service_write','allowed',true),set_config('cyd.inventory_lot_service_write','allowed',true),set_config('cyd.procurement_fulfillment_service_write','allowed',true)");
      const result = await work(client);
      await client.query("insert into audit_log(username,action,detail,request_id,result,route_code,operation_id,idempotency_key_digest,retention_until) values($1,$2,$3,$4,'success','PROCUREMENT',$5,$6,now()+interval '1095 days')", [meta.actor.username, meta.action, { object_id: result.objectId }, meta.requestId, meta.operationId, meta.keyDigest]);
