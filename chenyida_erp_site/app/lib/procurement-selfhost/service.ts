...
export class ProcurementService {
  ...
  async reverseReceipt(receiptId: number, meta: ProcurementMeta, input: Record<string, unknown>): Promise<ProcurementResult> {
    ...
    return this.repository.execute(meta, (client) => this.reverseReceiptInTransaction(client, receiptId, meta, reason, balances, lineVersions));
  }

  async reverseReceiptInTransaction(client: PoolClient, receiptId: number, meta: ProcurementMeta, reason: string, balances: { materialId: number; expectedBalanceVersion: number }[], lineVersions: Map<number, number>): Promise<ProcurementResult> {
