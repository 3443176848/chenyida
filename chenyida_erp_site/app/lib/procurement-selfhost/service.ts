...
export class ProcurementService {
  ...
  async createOrder(meta: ProcurementMeta, input: Record<string, unknown>): Promise<ProcurementResult> {
    ...
    return this.repository.execute(meta, async (client) => { const data = await this.createOrderInTransaction(client, meta, supplierId, currencyCode, "MANUAL", expectedAt, remark, parsedLines); return { status: 201, body: { ok: true, data, request_id: meta.requestId }, objectId: Number(data.id) }; });
  }
  ...
  async createReceiptInTransaction(client: PoolClient, meta: ProcurementMeta, purchaseOrderId: number, parsedLines: ReceiptLineInput[], reason: string): Promise<ProcurementResult> {
    ...
    const lineIds = parsedLines.map((line) => line.purchaseOrderLineId); const locked = await client.query<LockedPoLine & { material_status: string; inventory_type: string; inspection_type: string }>(`select pol.*,m.material_status,m.inventory_type,m.inspection_type from purchase_order_lines pol join material_master m on m.id=pol.material_id where pol.purchase_order_id=$1 and pol.id=any($2::bigint[]) order by pol.id for update of pol,m`, [purchaseOrderId, lineIds]);
    ...
    const lotManaged: boolean[] = [];
    for (const input of parsedLines) { const row = byId.get(input.purchaseOrderLineId)!; if (Number(row.version) !== input.expectedLineVersion) throw new ProcurementError("PURCHASE_ORDER_LINE_VERSION_CONFLICT", "采购明细版本已变化，请刷新后重试", 409); const allowed = await client.query("select $1::numeric>0 and $1::numeric<=($2::numeric-$3::numeric) ok", [input.quantity, row.order_qty, row.received_qty]); if (!allowed.rows[0].ok) throw new ProcurementError("PURCHASE_RECEIPT_OVER_QUANTITY", "收货数量超过未收数量", 409); const managed = row.material_status === "ACTIVE" && row.inventory_type === "STOCKED" && row.inspection_type === "IQC"; if (managed && !input.supplierLotCode) throw new ProcurementError("SUPPLIER_LOT_CODE_REQUIRED", "IQC 管理物料收货必须填写供应商 Lot", 400); if (!managed && input.supplierLotCode) throw new ProcurementError("SUPPLIER_LOT_NOT_APPLICABLE", "非 IQC 物料不能建立供应商来料 Lot", 422); lotManaged.push(managed); }
    if (lotManaged.some(Boolean) && !lotManaged.every(Boolean)) throw new ProcurementError("MIXED_RECEIPT_LOT_MODE_NOT_SUPPORTED", "IQC Lot 物料与非 IQC 物料必须分开收货", 422);

    const code = await this.repository.nextCode(client, "PURCHASE_RECEIPT", "PR");
    let receipt: { rows: Record<string, unknown>[] }; let inventoryResult: InventoryMutationResult; const savedLines: Record<string, unknown>[] = []; const lots: Record<string, unknown>[] = [];
    if (lotManaged.every(Boolean)) {
      receipt = await client.query(`insert into purchase_receipts(receipt_code,purchase_order_id,inventory_adjustment_id,reason,operation_id,created_by,request_id) values($1,$2,null,$3,$4,$5,$6) returning *`, [code, purchaseOrderId, reason, meta.operationId, meta.actor.username, meta.requestId]);
      const receiptId = Number(receipt.rows[0].id);
      for (let index = 0; index < parsedLines.length; index += 1) {
        const input = parsedLines[index], row = byId.get(input.purchaseOrderLineId)!;
        const saved = await client.query(`insert into purchase_receipt_lines(purchase_receipt_id,line_no,purchase_order_line_id,material_id,unit_id,quantity,inventory_ledger_entry_id,line_amount) values($1,$2,$3,$4,$5,$6,null,round($6::numeric*$7::numeric,6)) returning *`, [receiptId, index + 1, input.purchaseOrderLineId, Number(row.material_id), Number(row.unit_id), input.quantity, row.unit_price]);
        const sequence = await client.query<{ current_value: string }>(`insert into business_code_sequences(sequence_code,current_value,version,updated_at) values('INVENTORY_SUPPLIER_LOT',1,1,now()) on conflict(sequence_code) do update set current_value=business_code_sequences.current_value+1,version=business_code_sequences.version+1,updated_at=now() returning current_value`);
        const lotCode = `RML-${String(sequence.rows[0].current_value).padStart(8, "0")}`;
        const lot = await client.query(`insert into inventory_lots(lot_code,lot_type,material_id,unit_id,source_purchase_receipt_line_id,supplier_id,supplier_lot_code,received_at,status,operation_id,created_by,request_id) values($1,'SUPPLIER_RECEIPT',$2,$3,$4,$5,$6,now(),'FROZEN',$7,$8,$9) returning *`, [lotCode, Number(row.material_id), Number(row.unit_id), Number(saved.rows[0].id), Number(po.supplier_id), input.supplierLotCode, randomUUID(), meta.actor.username, meta.requestId]);
        savedLines.push(saved.rows[0]); lots.push(lot.rows[0]);
      }
      inventoryResult = await this.inventory.postLotPositionsInTransaction(client, this.inventoryMeta(meta, "PURCHASE_RECEIPT_IQC_INVENTORY_POSTED"), { operationType: "IQC_RECEIPT", reason, lines: parsedLines.map((input, index) => ({ inventoryLotId: Number(lots[index].id), quantity: input.quantity, expectedBalanceVersion: input.expectedBalanceVersion })) });
      const inventoryLines = (inventoryResult.body.data as { lines: Record<string, unknown>[] }).lines; const ledgerByLot = new Map(inventoryLines.map((line) => [Number(line.inventory_lot_id), Number(line.ledger_entry_id)]));
      await client.query("update purchase_receipts set inventory_adjustment_id=$2 where id=$1", [receiptId, Number(inventoryResult.adjustmentId)]);
      for (let index = 0; index < savedLines.length; index += 1) { const ledgerId = ledgerByLot.get(Number(lots[index].id)); await client.query("update purchase_receipt_lines set inventory_ledger_entry_id=$2 where id=$1", [Number(savedLines[index].id), ledgerId]); savedLines[index] = { ...savedLines[index], inventory_ledger_entry_id: ledgerId, inventory_lot_id: Number(lots[index].id), lot_code: lots[index].lot_code, supplier_lot_code: lots[index].supplier_lot_code }; await client.query(`insert into inventory_lot_events(inventory_lot_id,event_type,inventory_adjustment_id,inventory_ledger_entry_id,purchase_receipt_line_id,quantity,from_status,to_status,reason,actor,request_id) values($1,'SUPPLIER_RECEIVED',$2,$3,$4,$5,null,'FROZEN',$6,$7,$8)`, [Number(lots[index].id), Number(inventoryResult.adjustmentId), ledgerId, Number(savedLines[index].id), parsedLines[index].quantity, reason, meta.actor.username, meta.requestId]); }
      receipt.rows[0] = { ...receipt.rows[0], inventory_adjustment_id: Number(inventoryResult.adjustmentId) };
    } else {
      inventoryResult = await this.inventory.postInTransaction(client, this.inventoryMeta(meta, "PURCHASE_RECEIPT_INVENTORY_POSTED"), { operation_type: "RECEIPT", reason, lines: parsedLines.map((input) => { const row = byId.get(input.purchaseOrderLineId)!; return { material_id: Number(row.material_id), unit_id: Number(row.unit_id), quantity: input.quantity, expected_balance_version: input.expectedBalanceVersion }; }) });
      receipt = await client.query(`insert into purchase_receipts(receipt_code,purchase_order_id,inventory_adjustment_id,reason,operation_id,created_by,request_id) values($1,$2,$3,$4,$5,$6,$7) returning *`, [code, purchaseOrderId, Number(inventoryResult.adjustmentId), reason, meta.operationId, meta.actor.username, meta.requestId]);
      const receiptId = Number(receipt.rows[0].id), inventoryLines = (inventoryResult.body.data as { lines: Record<string, unknown>[] }).lines, ledgerByMaterial = new Map(inventoryLines.map((line) => [Number(line.material_id), Number(line.ledger_entry_id)]));
      for (let index = 0; index < parsedLines.length; index += 1) { const input = parsedLines[index], row = byId.get(input.purchaseOrderLineId)!; const saved = await client.query(`insert into purchase_receipt_lines(purchase_receipt_id,line_no,purchase_order_line_id,material_id,unit_id,quantity,inventory_ledger_entry_id,line_amount) values($1,$2,$3,$4,$5,$6,$7,round($6::numeric*$8::numeric,6)) returning *`, [receiptId, index + 1, input.purchaseOrderLineId, Number(row.material_id), Number(row.unit_id), input.quantity, ledgerByMaterial.get(Number(row.material_id)), row.unit_price]); savedLines.push(saved.rows[0]); }
    }
    await this.fault?.("after_inventory"); const adjustmentId = Number(inventoryResult.adjustmentId); const receiptId = Number(receipt.rows[0].id);
    for (let index = 0; index < parsedLines.length; index += 1) { const input = parsedLines[index];
    ...
    const inventoryLines = (inventoryResult.body.data as { lines: Record<string, unknown>[] }).lines; const firstInventoryLine = inventoryLines.length === 1 ? inventoryLines[0] : null;
    await this.fault?.("before_result"); const body = { ok: true, data: { ...receipt.rows[0], lines: savedLines, inventory_lots: lots, financial_source: financial.rows[0], inventory_adjustment_id: adjustmentId }, receipt_id: receiptId, receipt_code: code, inventory_lot_id: lots.length === 1 ? Number(lots[0].id) : undefined, lot_code: lots.length === 1 ? lots[0].lot_code : undefined, before_qty: firstInventoryLine?.before_on_hand_qty, after_qty: firstInventoryLine?.after_on_hand_qty, request_id: meta.requestId };
  ...
  async reverseReceipt(receiptId: number, meta: ProcurementMeta, input: Record<string, unknown>): Promise<ProcurementResult> {
    ...
    return this.repository.execute(meta, (client) => this.reverseReceiptInTransaction(client, receiptId, meta, reason, balances, lineVersions, expectedLotVersion));
  }

  async reverseReceiptInTransaction(client: PoolClient, receiptId: number, meta: ProcurementMeta, reason: string, balances: { materialId: number; expectedBalanceVersion: number }[], lineVersions: Map<number, number>, expectedLotVersion: number | null = null): Promise<ProcurementResult> {
    ...
    const lots = await client.query(`select l.*,b.on_hand_qty,b.reserved_qty,b.frozen_qty,b.version balance_version,prl.quantity receipt_quantity from inventory_lots l join purchase_receipt_lines prl on prl.id=l.source_purchase_receipt_line_id join inventory_stock_balances b on b.inventory_lot_id=l.id and b.location_code='MAIN' where prl.purchase_receipt_id=$1 order by l.id for update of l,b`, [receiptId]);
    if (lots.rows.length) {
      if (expectedLotVersion === null || lots.rows.some((lot) => Number(lot.version) !== expectedLotVersion)) throw new ProcurementError("INVENTORY_LOT_VERSION_CONFLICT", "Supplier Receipt Lot 版本已变化，请刷新后重试", 409);
      if ((await client.query(`select 1 from quality_inspections qi join inventory_lots l on l.id=qi.inventory_lot_id where l.source_purchase_receipt_line_id=any($1::bigint[]) limit 1`, [source.rows.map((row) => Number(row.id))])).rows[0]) throw new ProcurementError("RECEIPT_REVERSAL_BLOCKED_BY_IQC", "该收货已创建 IQC，不能冲销", 409);
      for (const lot of lots.rows) { const safe = await client.query(`select $1::numeric=$2::numeric and $3::numeric=0 and $4::numeric=$2::numeric and (select count(*) from inventory_ledger_entries where inventory_lot_id=$5)=1 ok`, [lot.on_hand_qty, lot.receipt_quantity, lot.reserved_qty, lot.frozen_qty, Number(lot.id)]); if (!safe.rows[0].ok) throw new ProcurementError("RECEIPT_REVERSAL_LOT_STATE_CONFLICT", "Supplier Receipt Lot 已产生其他库存下游或数量不再可完整冲销", 409); }
    }
    ...
    const inventoryLines = (inventoryResult.body.data as { lines: Record<string, unknown>[] }).lines; const ledgerByMaterial = new Map(inventoryLines.map((line) => [Number(line.material_id), Number(line.ledger_entry_id)])); const ledgerByLot = new Map(inventoryLines.filter((line) => line.inventory_lot_id != null).map((line) => [Number(line.inventory_lot_id), Number(line.ledger_entry_id)])); const lotByReceiptLine = new Map(lots.rows.map((lot) => [Number(lot.source_purchase_receipt_line_id), lot])); const savedLines = [];
    for (let index = 0; index < source.rows.length; index += 1) { const row = source.rows[index], lot = lotByReceiptLine.get(Number(row.id)); const ledgerId = lot ? ledgerByLot.get(Number(lot.id)) : ledgerByMaterial.get(Number(row.material_id)); const saved = await client.query(`insert into purchase_receipt_lines(purchase_receipt_id,line_no,purchase_order_line_id,material_id,unit_id,quantity,inventory_ledger_entry_id,reversal_of_receipt_line_id,line_amount) values($1,$2,$3,$4,$5,$6,$7,$8,$9) returning *`, [reversalId, index + 1, Number(row.purchase_order_line_id), Number(row.material_id), Number(row.unit_id), row.quantity, ledgerId, Number(row.id), row.line_amount]); savedLines.push(saved.rows[0]); const expected = lineVersions.get(Number(row.purchase_order_line_id)); const updated = await client.query(`update purchase_order_lines set received_qty=received_qty-$2::numeric,status=case when received_qty-$2::numeric=0 then 'OPEN' else 'PARTIALLY_RECEIVED' end,version=version+1,updated_at=now() where id=$1 and version=$3 and received_qty>=$2::numeric returning *`, [Number(row.purchase_order_line_id), row.quantity, expected]); if (!updated.rows[0]) throw new ProcurementError("PURCHASE_ORDER_LINE_VERSION_CONFLICT", "采购明细版本已变化或累计收货不足", 409); }
    for (const lot of lots.rows) { const ledgerId = ledgerByLot.get(Number(lot.id)); const updated = await client.query("update inventory_lots set status='REVERSED',version=version+1,updated_at=now() where id=$1 and version=$2 returning *", [Number(lot.id), expectedLotVersion]); if (!updated.rows[0]) throw new ProcurementError("INVENTORY_LOT_VERSION_CONFLICT", "Supplier Receipt Lot 并发更新冲突", 409); await client.query(`insert into inventory_lot_events(inventory_lot_id,event_type,inventory_adjustment_id,inventory_ledger_entry_id,purchase_receipt_line_id,quantity,from_status,to_status,reason,actor,request_id) values($1,'SUPPLIER_RECEIPT_REVERSED',$2,$3,$4,$5,$6,'REVERSED',$7,$8,$9)`, [Number(lot.id), Number(inventoryResult.adjustmentId), ledgerId, Number(lot.source_purchase_receipt_line_id), lot.receipt_quantity, lot.status, reason, meta.actor.username, meta.requestId]); }
    ...
    const body = { ok: true, data: { ...reversal.rows[0], lines: savedLines, inventory_lots: lots.rows.map((lot) => ({ ...lot, status: "REVERSED", version: Number(lot.version) + 1 })), financial_source: financial.rows[0], inventory_adjustment_id: Number(inventoryResult.adjustmentId) }, receipt_id: reversalId, receipt_code: code, reversal_of_receipt_id: receiptId, request_id: meta.requestId }; return { status: 201, body, objectId: reversalId };
  }
}
