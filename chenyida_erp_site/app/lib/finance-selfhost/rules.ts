...
export function settlementType(value: unknown): "RECEIPT" | "PAYMENT" { const result = String(value ?? "").trim().toUpperCase(); if (result !== "RECEIPT" && result !== "PAYMENT") throw new FinanceError("REQUEST_VALIDATION_FAILED", "settlement_type 必须是 RECEIPT 或 PAYMENT"); return result; }
