...
export async function handleSalesApi(request: Request, dependencies: Dependencies): Promise<Response | null> {
  ...
  const fixed = ["/api/quotations", "/api/quotations/to-sales-order", "/api/sales-orders", "/api/delivery-instructions", "/api/shipments", "/api/shipments/from-order", "/api/sales/financial-sources", "/api/sales/shipment-lot-options"];
  ...
  try {
    if (request.method === "GET") {
      ...
      if(path==="/api/sales/shipment-lot-options"){requirePermission(dependencies.actor,"sales.delivery.read");const result=await service.shipmentLotOptions(id(url.searchParams.get("sales_order_line_id"),"sales_order_line_id"));return response({rows:result.rows,data:result.rows,request_id:dependencies.requestId},200,dependencies.requestId);}
