...
export async function handleSalesApi(request: Request, dependencies: Dependencies): Promise<Response | null> {
  ...
  const fixed = ["/api/quotations", "/api/quotations/to-sales-order", "/api/sales-orders", "/api/shipments", "/api/shipments/from-order", "/api/sales/financial-sources"];
  if (!fixed.includes(path) && !quotation && !quotationVersion && !quotationVersions && !quotationAction && !order && !orderAction && !orderAvailability && !shipment && !shipmentReversal) return null;
  ...
  try {
    ...
    else if (path === "/api/shipments") { action = "SALES_SHIPMENT_POSTED"; requirePermission(dependencies.actor, "sales.ship"); result = await service.createShipment(mutationMeta(request, dependencies, path, action, parsed.digest), parsed.value); }
    ...
    else if (shipmentReversal) { action = "SALES_SHIPMENT_REVERSED"; requirePermission(dependencies.actor, "sales.reverse"); result = await service.reverseShipment(Number(shipmentReversal[1]), mutationMeta(request, dependencies, path, action, parsed.digest), parsed.value); }
