...
export class SalesService {
  ...
  async availableToShip(salesOrderId: number) {
    return this.repository.pool.query(`select l.*,p.product_code,p.product_name,pv.version_code product_version_code,m.internal_material_code finished_item_code,m.standard_name finished_item_name,u.code uom,
      (l.ordered_qty-l.shipped_qty)::text remaining_qty,coalesce(b.on_hand_qty-b.reserved_qty-b.frozen_qty,0)::text available_qty,coalesce(b.version,0)::int balance_version
      ...
      left join inventory_stock_balances b on b.material_id=l.finished_material_id and b.location_code='MAIN' and b.lot_code='' where so.id=$1 order by l.line_no`, [salesOrderId]);
  }
  ...
  private async createShipmentInTransaction(client: PoolClient, salesOrderId: number, meta: SalesMeta, inputs: ShipmentLineInput[], shipDate: Date | null, receiver: string, reason: string): Promise<SalesResult> {
    ...
    for (const input of inputs) { const row = byId.get(input.salesOrderLineId)!; if (Number(row.version) !== input.expectedLineVersion) throw new SalesError("SALES_ORDER_LINE_VERSION_CONFLICT", "销售明细版本已变化，请刷新后重试", 409); const allowed = await client.query("select $1::numeric>0 and $1::numeric<=($2::numeric-$3::numeric) ok", [input.quantity, row.ordered_qty, row.shipped_qty]); if (!allowed.rows[0].ok) throw new SalesError("SHIPMENT_OVER_QUANTITY", "出货数量超过订单未出数量", 409); }
