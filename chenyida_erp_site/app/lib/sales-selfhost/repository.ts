...
import type { SalesMeta, SalesResult, SalesWork } from "./types.ts";
...
export class SalesRepository {
  ...
  async consumeWriteRate(actor: string, keyDigest: string) {
    ...
    } catch (error) { await client.query("rollback").catch(() => undefined); throw mapSalesError(error); } finally { client.release(); }
  ...
  async execute(meta: SalesMeta, work: SalesWork): Promise<SalesResult> {
  ...
  async failureAudit(actor: string, requestId: string, action: string, code: string) { await this.pool.query("insert into audit_log(username,action,detail,request_id,result,route_code,error_code,retention_until) values($1,$2,'{}'::jsonb,$3,'failed','SALES',$4,now()+interval '1095 days')", [actor, action, requestId, code]).catch(() => undefined); }
}
