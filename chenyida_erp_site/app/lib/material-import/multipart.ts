...
export function validateSingleFileMultipartHeaders(
  request: Request,
  maximumBytes = MATERIAL_IMPORT_MAX_FILE_BYTES,
): Readonly<{ boundary: string; body: ReadableStream<Uint8Array> }> {
  if (!Number.isSafeInteger(maximumBytes) || maximumBytes <= 0) {
    throw new MaterialImportMultipartError("INVALID_REQUEST", "文件大小上限无效");
  }
  const contentLength = request.headers.get("Content-Length");
  if (contentLength !== null) {
    if (!/^\d+$/.test(contentLength)) {
      throw new MaterialImportMultipartError("INVALID_REQUEST", "Content-Length 无效");
    }
  ...
  return { boundary, body: request.body };
}
...
export async function readSingleFilePart(
  ...
  const { boundary, body } = validateSingleFileMultipartHeaders(request, maximumBytes);
  ...
  try {
    while (pending.byteLength <= MAX_MULTIPART_HEADER_BYTES) {
      ...
      if (headerEnd < 0) {
        if (pending.byteLength > MAX_MULTIPART_HEADER_BYTES) break;
        continue;
      }
      if (headerEnd > MAX_MULTIPART_HEADER_BYTES || pending.byteLength < initialPrefix.byteLength) break;
      ...
      const finish = async (controller: ReadableStreamDefaultController<Uint8Array>, suffix: Uint8Array) => {
        ...
        if (remainder.byteLength > 4) throw new MaterialImportMultipartError("INVALID_REQUEST", "multipart 结束后包含额外数据");
        while (true) {
          const next = await reader.read();
          if (next.done) break;
          if (next.value?.byteLength) remainder = concatBytes(remainder, next.value);
