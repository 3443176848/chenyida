...
export class QualityService {
  ...
  async createAllocation(meta: QualityMeta, input: Record<string, unknown>): Promise<QualityResult> {
    ...
    return this.repository.execute(meta,async(client)=>{
      const source=await client.query("select * from public.cyd_web_lock_quality_completion_allocation_source($1)",[completionLineId]);
  ...
  private async lockFqcSources(client: PoolClient, inspection: Pick<InspectionRow, "production_completion_line_id" | "sales_order_line_id" | "fqc_allocation_id">) {
    ...
    const completion = await client.query("select * from public.cyd_web_lock_quality_completion_capacity($1)", [inspection.production_completion_line_id]);
  ...
  private async resolveSource(client: PoolClient, type: InspectionType, input: Record<string, unknown>): Promise<QualitySource> {
    ...
    if (type === "IPQC" && input.operation_run_report_id != null && input.operation_run_report_id !== "") { const sourceId = id(input.operation_run_report_id, "operation_run_report_id"); const found = await client.query("select * from public.cyd_web_lock_quality_operation_source($1)", [sourceId]); if (!found.rows[0]) throw new QualityError("QUALITY_OPERATION_SOURCE_INVALID", "工序 Run Report 不存在、已取消、已冲销或未配置 IPQC", 422); return { purchaseReceiptLineId: null, productionReportId: null, operationRunReportId: sourceId, productionCompletionLineId: null, salesOrderLineId: null, fqcAllocationId:null, inventoryLotId:null, workOrderId: Number(found.rows[0].work_order_id), ...found.rows[0] } as QualitySource; }
    if (type === "IPQC") { const sourceId = id(input.production_report_id, "production_report_id"); const found = await client.query("select * from public.cyd_web_lock_quality_legacy_report_source($1)", [sourceId]); if (!found.rows[0]) throw new QualityError("QUALITY_SOURCE_INVALID", "生产报工不存在、已冲销或不可检验", 422); return { purchaseReceiptLineId: null, productionReportId: sourceId, operationRunReportId: null, productionCompletionLineId: null, salesOrderLineId: null, fqcAllocationId:null, inventoryLotId:null, ...found.rows[0] } as QualitySource; }
    const allocationId=id(input.allocation_id,"allocation_id");
    const found=await client.query("select * from public.cyd_web_lock_quality_fqc_source($1)",[allocationId]);
