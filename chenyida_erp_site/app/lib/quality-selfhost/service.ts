...
export class QualityService {
  ...
  async list(actor: IdentityActor, limit: number, offset: number, requestedType?: string, lifecycle?: string) {
    ...
    return this.repository.pool.query(`select qi.*,coalesce(batch_run.production_batch_id,lot.source_production_batch_id) production_batch_id,coalesce(batch.batch_code,fqc_batch.batch_code) batch_code,lot.lot_code,lot.status lot_status,m.internal_material_code material_code,m.internal_material_code item_code,m.internal_material_code product_code,m.standard_name material_name,m.standard_name item_name,m.standard_name product_name,u.code unit_code,u.code uom,
      ...
      from quality_inspections qi left join production_operation_run_reports batch_rr on batch_rr.id=qi.production_operation_run_report_id left join production_operation_runs batch_run on batch_run.id=batch_rr.run_id left join production_batches batch on batch.id=batch_run.production_batch_id left join inventory_lots lot on lot.id=qi.inventory_lot_id left join production_batches fqc_batch on fqc_batch.id=lot.source_production_batch_id join material_master m on m.id=qi.material_id join units u on u.id=qi.unit_id
  ...
  async get(actor: IdentityActor, inspectionId: number) {
    const found = await this.repository.pool.query(`select qi.*,coalesce(batch_run.production_batch_id,lot.source_production_batch_id) production_batch_id,coalesce(batch.batch_code,fqc_batch.batch_code) batch_code,lot.lot_code,lot.status lot_status,m.internal_material_code material_code,m.standard_name material_name,u.code unit_code from quality_inspections qi left join production_operation_run_reports batch_rr on batch_rr.id=qi.production_operation_run_report_id left join production_operation_runs batch_run on batch_run.id=batch_rr.run_id left join production_batches batch on batch.id=batch_run.production_batch_id left join inventory_lots lot on lot.id=qi.inventory_lot_id left join production_batches fqc_batch on fqc_batch.id=lot.source_production_batch_id join material_master m on m.id=qi.material_id join units u on u.id=qi.unit_id where qi.id=$1`, [inspectionId]);
