  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      const readiness = await dependencies.pool.query(`select l.id,(l.quantity_per*$2::numeric*(1+l.loss_rate))::numeric(24,6)::text required_qty,
        ...
        case when m.material_status<>'ACTIVE' then 'STRUCTURE_INVALID' when l.unit_id is distinct from m.base_unit_id then 'UNIT_CONVERSION_REQUIRED'
        ...
        join material_master m on m.id=l.material_id left join inventory_stock_balances ib on ib.material_id=l.material_id and ib.location_code='MAIN' and ib.lot_code=''
