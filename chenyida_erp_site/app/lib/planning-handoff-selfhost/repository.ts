...
import type { PlanningMutationMeta, PlanningMutationResult, PlanningMutationWork } from "./types.ts";

export class PlanningHandoffRepository {
  readonly pool: Pool;
  ...
  private async consumeWriteRate(actor: string, keyDigest: string) {
    ...
    catch (error) { await client.query("rollback").catch(() => undefined); throw mapPlanningHandoffError(error); } finally { client.release(); }
    if (limited) throw new PlanningHandoffError("RATE_LIMITED", "计划交接写操作过于频繁，请稍后重试", 429);
  }

  async execute(meta: PlanningMutationMeta, work: PlanningMutationWork): Promise<PlanningMutationResult> {
    ...
    try {
      ...
      if (existing.rows[0]) { if (existing.rows[0].request_digest !== meta.requestDigest) throw new PlanningHandoffError("IDEMPOTENCY_CONFLICT", "同一 Idempotency-Key 不能用于不同请求", 409); await client.query("commit"); const body = existing.rows[0].response as Record<string, unknown>; return { status: Number(existing.rows[0].status_code), body, objectId: Number(body.package_id || body.project_id || 0), replayed: true }; }
      await client.query("select set_config('cyd.planning_service_write','allowed',true)"); const result = await work(client);
      await client.query("insert into audit_log(username,action,detail,request_id,result,route_code,operation_id,idempotency_key_digest,old_version,new_version,retention_until) values($1,$2,$3,$4,'success','PLANNING_HANDOFF',$5,$6,$7,$8,now()+interval '1095 days')", [meta.actor.username, meta.action, { object_id: result.objectId, ...(result.auditDetail || {}) }, meta.requestId, meta.operationId, meta.keyDigest, result.oldVersion ?? null, result.newVersion ?? null]);
