...
export default function OperationsPage() {
  ...
  return <main className="sourcing-shell">
    ...
    <section className="sourcing-panel">{rows.map((run) => <article className="sourcing-card" key={run.id}>
      <b>{run.run_kind === "REWORK" ? "返工" : "正常生产"} · {run.batch_code??"按工单"} · {run.run_code} · {run.work_order_code} · {run.sequence_no} {run.operation_code} {run.operation_name}</b>
      <small>Snapshot Operation #{run.snapshot_operation_id} · Work Center {run.work_center_code} · 门禁 {run.quality_gate_mode === "NONE" ? "无" : run.quality_gate_mode} · {run.assigned_operator_name} · {statusLabel(run.status)}</small>
      <p>派工/处理/良品/报废 {run.dispatched_qty}/{run.processed_qty}/{run.good_qty}/{run.scrap_qty}</p>
      ...
      <p>待检/已检/已放行/质量冻结 {run.quality_required_qty}/{run.quality_inspected_qty}/{run.quality_released_qty}/{run.quality_hold_qty} · 下工序可用 {run.available_for_next_qty} · 最终报工可用 {run.final_output_available_qty}</p>
