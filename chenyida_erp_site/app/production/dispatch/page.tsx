...
export default function DispatchPage() {
  ...
  return <main className="sourcing-shell">
    <header className="sourcing-header"><div><Link href="/">← 经营工作台</Link><h1>生产派工准备（NORMAL 按 Batch 派工）</h1><p>本页只创建派工，不提供开工、完工或报工按钮。Batch 模式必须选择同工单已发布 Manufacturing Batch；ORDER 与 LEGACY_UNSTRUCTURED 模式保持历史兼容。成品 Manufacturing Batch 已绑定 Inventory Lot；原材料和供应商批次仍未启用。</p><nav><Link href="/production/batches">Batch genealogy</Link> · <Link href="/production/rework-requests">REWORK 继承原 Batch</Link> · <Link href="/production/operations">NORMAL / REWORK 执行</Link> · <Link href="/production/wip">WIP 看板</Link></nav></div></header>
    {error ? <div className="sourcing-state sourcing-error">{error}</div> : null}
    <section className="sourcing-panel"><h2>第一待执行工序与就绪、进行中工序</h2>{rows.map((row) => <article className="sourcing-card" key={row.snapshot_operation_id}>
      <b>{row.work_order_code} · {row.sequence_no} {row.operation_code} {row.operation_name}</b>
      <small>Snapshot Operation #{row.snapshot_operation_id} · Work Center {row.work_center_code} · 门禁 {row.quality_gate_mode === "NONE" ? "无" : row.quality_gate_mode} · {statusLabel(row.status)} · 可派 {row.waiting_input_qty}</small>
      <p>需检/已检/合格放行/品质冻结 {row.quality_required_qty}/{row.quality_inspected_qty}/{row.quality_released_qty}/{row.quality_hold_qty} · 下工序可用 {row.available_for_next_qty}</p>
      {["READY", "IN_PROGRESS"].includes(row.status) && Number(row.waiting_input_qty) > 0 ? <form onSubmit={(event) => void submit(event, row)}>{batches.some(batch=>batch.work_order_id===row.work_order_id)?<label>生产批次 <select name="production_batch_id" required>{batches.filter(batch=>batch.work_order_id===row.work_order_id&&!["COMPLETED","CANCELLED"].includes(batch.batch_status)).map(batch=><option key={batch.id} value={batch.id}>{batch.batch_code} · 计划 {batch.planned_qty}</option>)}</select></label>:<p>订单模式（不猜测或自动生成批次）</p>}<label>数量 <input name="quantity" type="number" min="0.000001" step="0.000001" max={row.waiting_input_qty} required /></label><label>执行人 <select name="assigned_operator" required>{operators.map((item) => <option value={item.username} key={item.username}>{item.display_name} ({item.username})</option>)}</select></label><label>计划开始 <input name="planned_start" type="datetime-local" /></label><label>计划结束 <input name="planned_end" type="datetime-local" /></label><button type="submit">创建派工</button></form> : <p>{row.quality_gate_mode === "IPQC" && Number(row.quality_hold_qty) > 0 ? "品质冻结尚未形成可消费放行额度。" : "当前没有可派投入。"}</p>}
    </article>)}</section>
  </main>;
}
