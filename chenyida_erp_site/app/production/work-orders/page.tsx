...
type Order={id:number;work_order_code:string;product_name:string;status:string;version:number;planned_qty:string};
...
export default function WorkOrdersPage(){
  ...
  return <main className="sourcing-shell"><header className="sourcing-header"><div><Link href="/">← 经营工作台</Link><h1>生产工单与齐套释放</h1><p>生产接收交接后创建唯一 DRAFT 工单；显式释放时复制 BOM、生成需求并原子预留库存。</p></div></header>
    ...
    <section className="sourcing-panel"><h2>工单</h2>{orders.map(order=><article className="sourcing-card" key={order.id}><b>{order.work_order_code} · {order.product_name}</b><small>{order.status} · 计划 {order.planned_qty}</small>{order.status==="DRAFT"&&can(session,"production.plan")?<button onClick={()=>post(`/api/work-orders/${order.id}/release`,{expected_version:order.version})}>齐套校验并释放</button>:null}</article>)}</section>
