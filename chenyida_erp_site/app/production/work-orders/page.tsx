...
export default function WorkOrdersPage(){
  ...
  return <main className="sourcing-shell"><header className="sourcing-header"><div><Link href="/">← 经营工作台</Link><h1>生产工单与齐套释放</h1><p>生产接收交接后创建唯一 DRAFT 工单；显式释放时复制 BOM、生成需求并原子预留库存。</p><nav><Link href="/production/reporting">生产报工</Link> · <Link href="/warehouse/production-completions">完工入库</Link></nav></div></header>
    <section className="sourcing-panel"><h2>生产交接</h2>{handoffs.map(h=><article className="sourcing-card" key={h.id}><b>{h.handoff_code}</b><small>{h.status} · {h.item_count} 行</small>{h.status==="SUBMITTED"&&can(session,"production.handoff.decide")?<div><button onClick={()=>post(`/api/production-handoffs/${h.id}/accept`,{expected_version:h.version})}>接收</button><button onClick={()=>post(`/api/production-handoffs/${h.id}/return`,{expected_version:h.version,reason:"生产资料需修订"})}>退回</button></div>:null}{h.status==="ACCEPTED"&&can(session,"production.handoff.work_order")?items[h.id]?.filter(i=>!i.work_order_id).map(item=><button key={item.id} onClick={()=>post(`/api/production-handoff-items/${item.id}/work-order`,{})}>创建 {item.product_name} 工单（{item.planned_quantity}）</button>):null}</article>)}</section>
    <section className="sourcing-panel"><h2>工单</h2>{orders.map(order=><article className="sourcing-card" key={order.id}><b>{order.work_order_code} · {order.product_name}</b><small>{order.status} · planned {order.planned_qty} · issued {order.issued_supported_qty} · reported {order.reported_qty} · good {order.good_qty} · scrap {order.scrap_qty} · completed {order.completed_qty} · waiting for receipt {order.waiting_receipt_qty}</small>{order.status==="DRAFT"&&can(session,"production.plan")?<button onClick={()=>post(`/api/work-orders/${order.id}/release`,{expected_version:order.version})}>齐套校验并释放</button>:null}</article>)}</section>
