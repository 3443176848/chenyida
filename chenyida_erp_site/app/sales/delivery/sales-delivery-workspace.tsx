...
export function SalesDeliveryWorkspace(){
 ...
 return <main className="sourcing-shell">
  ...
  <header className="sourcing-header"><div><Link href="/" className="sourcing-back">← 经营工作台</Link><p className="sourcing-kicker">销售发货指令</p><h1>销售发货指令</h1><p>来源仅限开放销售订单；服务端校验订单余量与未占用 FQC 额度。</p></div></header>
  ...
  <section className="sourcing-panel"><h2>发货指令</h2>{rows.map(row=><article className="sourcing-card" key={row.id}><div><b>{row.delivery_code} · {row.sales_order_code}</b><span className="sourcing-status status-pending">{statusLabel(row.status)}</span></div><p>{row.customer_name} · 指令 {row.total_qty} · 已执行 {row.executed_qty} · 待执行 {row.remaining_qty}</p>{row.status==="DRAFT"&&can(session.user,"sales.delivery.submit")&&<button disabled={busy} onClick={()=>void transition(row,"submit")}>提交仓库</button>}{["DRAFT","RETURNED","SUBMITTED"].includes(row.status)&&can(session.user,"sales.delivery.cancel")&&<button disabled={busy} onClick={()=>void transition(row,"cancel")}>受控取消</button>}</article>)}{!rows.length&&<div className="sourcing-state">尚无发货指令。</div>}</section>
