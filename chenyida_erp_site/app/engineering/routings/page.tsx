...
export default function RoutingsPage() {
  ...
  return <main className="sourcing-shell">
    <header className="sourcing-header"><div><Link href="/">← 经营工作台</Link><h1>产品工艺路线与质量门禁</h1><p>engineering 仅在 DRAFT 配置 NONE/IPQC；门禁进入 canonical digest，发布版本与 Work Order Snapshot 不可修改。</p><nav><Link href="/operations/work-centers">工作中心</Link> · <Link href="/production/dispatch">生产派工</Link></nav></div></header>
    <section className="sourcing-panel"><h2>路线版本</h2>{rows.map((row) => <article className="sourcing-card" key={row.routing_version_id}><b>{row.routing_code} · {row.product_code} {row.product_name} · {row.version_code}</b><small>{row.status} · {row.operation_count} 工序 · v{row.version}</small>{row.canonical_digest ? <code>{row.canonical_digest}</code> : null}<div><button onClick={() => void inspect(row.routing_version_id)}>查看工序与门禁</button> {row.status === "DRAFT" && can(session, "production.routing.manage") ? <button onClick={() => void write(`/api/production/routing-versions/${row.routing_version_id}/submit`, { expected_version: row.version })}>提交审核</button> : null}{row.status === "SUBMITTED" && can(session, "production.routing.review") ? <><button onClick={() => void write(`/api/production/routing-versions/${row.routing_version_id}/release`, { expected_version: row.version })}>异人发布</button><button onClick={() => void write(`/api/production/routing-versions/${row.routing_version_id}/return`, { expected_version: row.version, reason: "工艺资料需修订" })}>退回</button></> : null}</div></article>)}</section>
    {detail ? <section className="sourcing-panel"><h2>Routing Version #{detail.header.id} · {detail.header.status}</h2>{detail.operations.map((operation, index) => <article className="sourcing-card" key={`${operation.sequence_no}-${operation.operation_code}`}><b>{operation.sequence_no} · {operation.operation_code} {operation.operation_name}</b><small>Work Center {operation.work_center_code} {operation.work_center_name}</small><label>quality_gate_mode <select value={operation.quality_gate_mode} disabled={detail.header.status !== "DRAFT" || !can(session, "production.routing.manage")} onChange={(event) => setDetail({ ...detail, operations: detail.operations.map((item, position) => position === index ? { ...item, quality_gate_mode: event.target.value as "NONE" | "IPQC" } : item) })}><option value="NONE">NONE（直通）</option><option value="IPQC">IPQC（受控放行）</option></select></label></article>)}{detail.header.status === "DRAFT" && can(session, "production.routing.manage") ? <button onClick={() => void saveGates()}>保存 DRAFT 工序门禁</button> : <p>当前版本只读；发布门禁已固化。</p>}</section> : null}
