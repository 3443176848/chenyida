...
type Operation = Readonly<{ operation_type: "NORMALIZE" | "RETRY" | "CANCEL"; key: string; method: "POST"; endpoint: string; frozen_body: string; payload_digest: string; status: "PENDING" | "COMPLETED" | "FAILED" | "RESULT_UNKNOWN"; started_at: string; last_error_code: string }>;
type Dialog = "START" | "CANCEL" | "RERUN" | null;
type RunsResponse = { batch_id: number; items: NormalizationRun[]; next_cursor: string | null; request_id: string };
...
function CandidateCard({ label, candidate }: { label: string; candidate: FieldCandidate | null | undefined }) {
  ...
  if (candidate === null) return <article className="min-candidate"><h5>{label}</h5><p>字段对象为空</p></article>;
  const source = candidate.source;
  return <article className="min-candidate" id={`min-field-${cryptoSafeId(candidate.target_code)}`}>
    <h5>{label} <code>{candidate.target_code}</code></h5><p><strong>{statusLabel(candidate.status)}</strong></p>
    <BoundedValueView value={boundedValue(candidate.candidate)} />
    <dl className="min-inline-facts"><div><dt>来源</dt><dd>{statusLabel(source.kind)}</dd></div><div><dt>列</dt><dd>{source.column_index === null ? "无" : source.column_index}</dd></div><div><dt>单元格</dt><dd>{statusLabel(source.cell_type)}</dd></div><div><dt>值状态</dt><dd>{statusLabel(source.value_state)}</dd></div><div><dt>空值类型</dt><dd>{statusLabel(source.blank_kind)}</dd></div></dl>
...
function RunProgress({ run }: { run: NormalizationRun }) {
  ...
  return <section className="min-progress" aria-labelledby="min-progress-title"><h3 id="min-progress-title">{normalizationStageLabel(run.current_stage)}</h3>
    <p>运行状态：{statusLabel(run.run_status)}</p>{progress ? <><label htmlFor="min-row-progress">{progress.label}</label><progress id="min-row-progress" value={progress.percent} max={100}>{progress.percent}%</progress><p>{progress.percent}% · {run.processed_rows} / {run.total_rows} 行</p></> : <p>总行数尚不可用于安全百分比。</p>}
...
function SummaryPanel({ run }: { run: NormalizationRun }) {
  ...
  return <section className="min-summary"><h3>{run.error_rows ? `规范化已完成，其中 ${run.error_rows} 行需要处理` : "规范化结果已发布"}</h3>
    <div className="min-metrics"><span>总行 <strong>{run.total_rows}</strong></span><span>有效 <strong>{run.valid_rows}</strong></span><span>警告 <strong>{run.warning_rows}</strong></span><span>错误 <strong>{run.error_rows}</strong></span><span>问题总数 <strong>{run.issue_count}</strong></span><span>警告问题 <strong>{run.warning_count}</strong></span><span>错误问题 <strong>{run.error_count}</strong></span></div>
    <p>错误行表示该行仍需处理，不表示任务失败；“规范化结果已发布”不表示分类、匹配、物料草稿或正式导入完成。</p>
...
export function MaterialImportNormalizationReview({ batchId, batch, permissions, csrfToken, onBatch }: { batchId: number; batch: MaterialImportBatch; permissions: string[]; csrfToken: string; onBatch: (batch: MaterialImportBatch) => void }) {
  ...
  const [history, setHistory] = useState<NormalizationRun[]>([]);
  ...
  const focusReturn = useRef<HTMLElement | null>(null); const mainTitle = useRef<HTMLHeadingElement>(null); const runId = query?.run_id ?? summary?.selected_run?.id ?? summary?.current_run?.id ?? null;
  ...
  const compositeRead = useCallback(async (signal?: AbortSignal) => {
    const requestedRun = strictPositiveInteger(new URLSearchParams(window.location.search).get("run_id"));
    const normalizationEndpoint = `/api/material-master/import-batches/${batchId}/normalization${requestedRun ? `?run_id=${requestedRun}` : ""}`;
    const [detailResponse, summaryResponse] = await Promise.all([
      ...
      api<NormalizationSummary>(normalizationEndpoint, { cache: "no-store", signal }),
    ]);
  ...
  useEffect(() => {
    ...
    api<RunsResponse>(`/api/material-master/import-batches/${batchId}/normalization/runs?limit=50`, { cache: "no-store", signal: controller.signal })
      .then((value) => setHistory(value.items))
      .catch((reason) => handleError(reason, true));
    return () => controller.abort();
  }, [batchId, handleError, summary?.latest_attempt?.id]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    api<NormalizedRowDetail>(`/api/material-master/import-batches/${batchId}/normalized-rows/${detailRowId}?run_id=${runId}`, { cache: "no-store", signal: controller.signal }).then((response) => {
  ...
  const execute = async (kind: "NORMALIZE" | "RETRY" | "CANCEL", payload?: Record<string, unknown>, replay = false) => {
    const targetRun = summary?.latest_attempt?.id;
    const endpoint = kind === "NORMALIZE"
      ? `/api/material-master/import-batches/${batchId}/normalize`
      : `/api/material-master/import-batches/${batchId}/normalization/runs/${targetRun}/${kind === "RETRY" ? "retry" : "cancel"}`;
  ...
  const current = summary.current_run; const latest = summary.latest_attempt; const selected = summary.selected_run ?? current; const active = activeNormalizationRun(latest); const unknownOperation = Object.values(operations).find((item) => item?.status === "RESULT_UNKNOWN"); const unknown = Boolean(unknownOperation);
  const canStart = canNormalize && batch.status === "MAPPING_CONFIRMED" && !current && !latest && !active && !unknown;
  const canRetry = canNormalize && ["MAPPING_CONFIRMED", "NORMALIZED"].includes(batch.status) && latest?.run_status === "FAILED" && !unknown;
  const canRerun = canNormalize && batch.status === "NORMALIZED" && current && !active && !unknown;
  ...
  return <section className="min-review"><h2 ref={mainTitle} tabIndex={-1}>数据归一化与结果审阅</h2>
    <div className="min-live sr-only" aria-live="polite">{latest ? `${statusLabel(latest.run_status)}：${normalizationStageLabel(latest.current_stage)}` : "数据归一化尚未启动"}</div>
    <nav className="min-tabs" aria-label="规范化工作区"><button aria-current={query.view === "normalize" ? "page" : undefined} onClick={() => navigate({ view: "normalize", row: null })}>数据归一化</button>{selected ? <><button aria-current={query.view === "normalized" ? "page" : undefined} onClick={() => navigate({ view: "normalized", row: null })}>结果行</button><button aria-current={query.view === "issues" ? "page" : undefined} onClick={() => navigate({ view: "issues", row: null })}>问题</button><button onClick={() => window.location.assign(`/materials/imports/${batchId}?view=review&run_id=${selected.id}`)}>人工复核</button></> : null}<button onClick={() => window.location.assign(`/materials/imports/${batchId}?view=confirmed`)}>回看已确认字段映射</button></nav>
    {history.length ? <div className="min-filters"><label>运行历史<select value={query.run_id ?? current?.id ?? ""} onChange={(event) => { const value = Number(event.target.value); setRows(null); setIssues(null); setDetail(null); navigate({ run_id: Number.isSafeInteger(value) && value > 0 ? value : null, row_cursor: "", issue_cursor: "", row: null }); window.setTimeout(() => void refresh(), 0); }}>{history.map((run) => <option key={run.id} value={run.id}>运行 v{run.run_version ?? run.id} · {statusLabel(run.run_status)} · 映射 v{run.mapping_version}</option>)}</select></label><p>历史运行只读；切换不会改变当前发布指针。</p></div> : null}
    ...
    {query.view === "normalize" ? <section className="min-normalize-panel">
      {latest && active ? <RunProgress run={latest} /> : !current ? <><h3>{latest?.run_status === "FAILED" ? "规范化任务未完成" : latest?.run_status === "CANCELLED" ? "首次数据归一化已取消" : "数据归一化尚未启动"}</h3><p>{latest?.safe_failure_message || "将生成只读候选快照；不会创建物料，结果可以包含错误行。"}</p></> : <><h3>当前结果已发布</h3><p>{current.processor_version === MATERIAL_IMPORT_NORMALIZATION_PROCESSOR_VERSION ? "当前结果已使用本版本处理。" : "页面发布版本已变化，可以提供理由后重新运行。"}</p></>}
      <div className="min-actions"><button onClick={() => void refresh()} disabled={busy}>手动刷新</button>{canStart ? <button className="mi-primary" onClick={() => void prepare("START")} disabled={busy}>启动数据归一化</button> : null}{canRetry ? <button className="mi-primary" onClick={() => void execute("RETRY", { expected_version: latest!.expected_version })} disabled={busy}>重试同一运行</button> : null}{canRerun ? <button className="mi-primary" onClick={() => void prepare("RERUN")} disabled={busy}>重新运行规范化</button> : null}{canStop ? <button className="danger" onClick={() => void prepare("CANCEL")} disabled={busy}>取消任务</button> : null}</div>
    </section> : null}
    {selected && query.view === "normalized" ? <><SummaryPanel run={selected} /><section id="min-rows" className="min-list" tabIndex={-1}><h3>规范化结果行</h3><div className="min-filters"><label>行状态<select value={query.row_status} onChange={(event) => { setRows(null); setRowPrevious(false); navigate({ row_status: event.target.value as NormalizationQuery["row_status"], row_cursor: "", row: null }); }}><option value="">全部</option><option value="VALID">有效</option><option value="WARNING">警告</option><option value="ERROR">错误</option><option value="SKIPPED">已跳过</option></select></label><label>每页<select value={query.row_limit} onChange={(event) => { setRows(null); setRowPrevious(false); navigate({ row_limit: Number(event.target.value) as 50 | 100, row_cursor: "", row: null }); }}><option value="50">50</option><option value="100">100</option></select></label></div>
      <div className="min-table-wrap"><table><caption>当前规范化运行的结果行</caption><thead><tr><th scope="col">工作表</th><th scope="col">来源行</th><th scope="col">状态</th><th scope="col">警告</th><th scope="col">错误</th><th scope="col">原始摘要</th><th scope="col">操作</th></tr></thead><tbody>{rows?.items.map((row) => <tr key={row.id}><td>{row.source_sheet_index}</td><td>{row.source_row_number}</td><td><span className={`min-level min-level-${row.row_status.toLowerCase()}`}>{statusLabel(row.row_status)}</span></td><td>{row.warning_count}</td><td>{row.error_count}</td><td><code>{shortDigest(row.source_raw_row_hash)}</code></td><td><button onClick={(event) => openRow(row.id, event.currentTarget)}>查看详情</button></td></tr>)}</tbody></table></div>{rows?.items.length === 0 ? <p>当前筛选条件下没有结果；这不表示全部校验通过。</p> : null}<div className="min-page-actions"><button disabled={!rowPrevious} onClick={() => window.history.back()}>上一页</button><button disabled={!rows?.next_cursor} onClick={() => { setRowPrevious(true); navigate({ row_cursor: rows!.next_cursor || "", row: null }, NORMALIZATION_ROW_PAGE_MARKER); }}>下一页</button></div></section></> : null}
    {selected && query.view === "issues" ? <section id="min-issues" className="min-list" tabIndex={-1}><h3>规范化问题</h3><form key={`${query.issue_level}|${query.issue_code}|${query.issue_target}|${query.issue_row}|${query.issue_limit}`} className="min-filters" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const level = String(form.get("issue_level") || ""); const code = String(form.get("issue_code") || "").trim().toUpperCase(); const target = String(form.get("issue_target") || "").trim(); const rowText = String(form.get("issue_row") || "").trim(); const parsedRow = rowText ? strictPositiveInteger(rowText) : null; if ((code && !/^[A-Z][A-Z0-9_]{2,99}$/.test(code)) || (target && (target.length < 3 || target.length > 160)) || (rowText && parsedRow === null)) { setNotice("问题筛选值不符合 API 约束，未发送请求。"); return; } setNotice(""); setIssues(null); setIssuePrevious(false); navigate({ issue_level: ["ERROR", "WARNING"].includes(level) ? level as NormalizationQuery["issue_level"] : "", issue_code: code, issue_target: target, issue_row: parsedRow, issue_limit: form.get("issue_limit") === "100" ? 100 : 50, issue_cursor: "", row: null }); }}><label>级别<select name="issue_level" defaultValue={query.issue_level}><option value="">全部</option><option value="ERROR">错误</option><option value="WARNING">警告</option></select></label><label>问题代码<input name="issue_code" defaultValue={query.issue_code} maxLength={100} /></label><label>目标字段代码<input name="issue_target" defaultValue={query.issue_target} maxLength={160} /></label><label>来源行<input name="issue_row" inputMode="numeric" defaultValue={query.issue_row ?? ""} /></label><label>每页<select name="issue_limit" defaultValue={String(query.issue_limit)}><option value="50">50</option><option value="100">100</option></select></label><div className="min-filter-actions"><button type="submit">应用筛选</button><button type="button" onClick={() => { setIssues(null); setIssuePrevious(false); navigate({ issue_level: "", issue_code: "", issue_target: "", issue_row: null, issue_limit: 50, issue_cursor: "", row: null }); }}>清除</button></div></form>
      {query.issue_row ? <p className="min-filter-note">该筛选可能同时包含其他工作表中的相同行号。</p> : null}<div className="min-table-wrap"><table><caption>当前规范化运行的问题列表</caption><thead><tr><th scope="col">级别</th><th scope="col">问题</th><th scope="col">工作表 / 行 / 列</th><th scope="col">目标字段</th><th scope="col">安全说明</th><th scope="col">操作</th></tr></thead><tbody>{issues?.items.map((issue) => <tr key={issue.id}><td><span className={`min-level min-level-${issue.issue_level.toLowerCase()}`}>{statusLabel(issue.issue_level)}</span></td><td id={`min-issue-${issue.id}`}><strong>{issueLabel(issue.issue_code)}</strong><br/><code>{issue.issue_code}</code></td><td>{issue.source_sheet_index} / {issue.source_row_number} / {issue.source_column_index ?? "—"}</td><td><code>{issue.target_code}</code></td><td><span className="min-message">{issue.safe_message.slice(0, 180)}</span>{issue.safe_message.length > 180 || safeIssueDetails(issue.safe_details).length ? <details><summary>查看有界详情</summary><p>{issue.safe_message.slice(0, 500)}</p><dl>{safeIssueDetails(issue.safe_details).map(([key, value]) => <div key={key}><dt>{key}</dt><dd><BoundedValueView value={boundedValue(value)} /></dd></div>)}</dl></details> : null}</td><td><button aria-describedby={`min-issue-${issue.id}`} onClick={(event) => openRow(issue.normalized_row_id, event.currentTarget, issue)}>查看行</button></td></tr>)}</tbody></table></div>{issues?.items.length === 0 ? <p>当前筛选条件下没有问题；这不表示全部校验通过。</p> : null}<div className="min-page-actions"><button disabled={!issuePrevious} onClick={() => window.history.back()}>上一页</button><button disabled={!issues?.next_cursor} onClick={() => { setIssuePrevious(true); navigate({ issue_cursor: issues!.next_cursor || "", row: null }, NORMALIZATION_ISSUE_PAGE_MARKER); }}>下一页</button></div></section> : null}
    {query.row && detail ? <FocusTrap labelledBy="min-drawer-title" onClose={closeDrawer}><header><h3 id="min-drawer-title" tabIndex={-1}>行详情：工作表 {detail.row.source_sheet_index} / 来源行 {detail.row.source_row_number} / {statusLabel(detail.row.row_status)}</h3><button onClick={closeDrawer} aria-label="关闭行详情">关闭</button></header><div className="min-drawer-body"><section><h4>行概览</h4><dl className="min-inline-facts"><div><dt>状态</dt><dd>{statusLabel(detail.row.row_status)}</dd></div><div><dt>警告</dt><dd>{detail.row.warning_count}</dd></div><div><dt>错误</dt><dd>{detail.row.error_count}</dd></div><div><dt>原始摘要</dt><dd>{shortDigest(detail.row.source_raw_row_hash)}</dd></div></dl></section><section><h4>不可变原始行</h4><BoundedValueView value={boundedValue(detail.raw_row)} /></section><section><h4>基础字段</h4>{orderedBasic.length ? orderedBasic.map(([code, value]) => <CandidateCard key={code} label={code} candidate={value} />) : <p>未生成基础字段候选。</p>}</section><section><h4>动态属性</h4><p>稳定属性代码与运行时显示名称快照分开保存。</p>{Object.entries(detail.normalized_payload.attributes).sort(([a], [b]) => a.localeCompare(b, "en")).map(([code, value]) => <CandidateCard key={code} label={code} candidate={value} />)}</section><section><h4>分类提示（非正式分类）</h4><p>该提示不是 category_id，也未通过分类绑定校验。</p><CandidateCard label="CATEGORY_HINT" candidate={detail.normalized_payload.category_hint} /></section><section><h4>供应商引用候选</h4><p>候选不等于内部物料 ID 或正式供应商 ID，也不会创建关联。</p>{Object.entries(detail.normalized_payload.supplier_reference).map(([code, value]) => <CandidateCard key={code} label={code} candidate={value} />)}</section><section><h4>延后校验</h4>{detail.normalized_payload.deferred_validation.length ? <ul>{detail.normalized_payload.deferred_validation.map((code) => <li key={code}>{DEFERRED_LABELS[code] || `后续校验提示（${code}）`}</li>)}</ul> : <p>当前结果未列出延后校验；不能据此声称全部校验通过。</p>}</section><section><h4>问题上下文</h4><p>{detail.normalized_payload.issue_summary.issue_count} 个问题摘要：{detail.normalized_payload.issue_summary.error_count} 个错误 / {detail.normalized_payload.issue_summary.warning_count} 个警告</p>{selectedIssue ? <article><strong>当前选中问题：{issueLabel(selectedIssue.issue_code)}</strong><p>{selectedIssue.safe_message.slice(0, 500)}</p></article> : <p>刷新或直接访问链接不会恢复所选问题正文。</p>}<button onClick={() => { closeDrawer(); navigate({ view: "issues", issue_row: detail.row.source_row_number, issue_cursor: "", row: null }); }}>按来源行查看问题</button></section><section><h4>字段谱系</h4>{detail.lineage?.length ? <div className="min-table-wrap"><table><thead><tr><th>目标</th><th>来源工作表/行/列</th><th>来源字段</th><th>规则</th></tr></thead><tbody>{detail.lineage.map((item, index) => <tr key={String(item.id ?? index)}><td><code>{String(item.target_namespace)}.{String(item.target_field_code)}</code></td><td>{String(item.source_sheet_name)} / {String(item.source_row_number)} / {item.source_column_index == null ? "默认值" : String(item.source_column_index)}</td><td>{String(item.source_column_name ?? item.source_field_key ?? "—")}</td><td><code>{String(item.transformation_rule_code)}</code></td></tr>)}</tbody></table></div> : <p>未产生字段谱系。</p>}<details><summary>运行级支持信息</summary><dl className="min-run-facts">{Object.entries(detail.normalized_payload.lineage).map(([key, value]) => <div key={key}><dt>{key}</dt><dd>{key.includes("digest") || key.includes("hash") ? shortDigest(value) : String(value)}</dd></div>)}</dl></details></section></div></FocusTrap> : null}
    {dialog === "START" ? <MaterialImportDialog title="启动数据归一化" busy={busy} primaryLabel="确认启动" onClose={() => setDialog(null)} onPrimary={() => void execute("NORMALIZE", { expected_version: batch.current_version, processor_version: MATERIAL_IMPORT_NORMALIZATION_PROCESSOR_VERSION })}><ul><li>生成规范化候选快照，结果可能包含错误行。</li><li>不执行公式，不调用 AI，不自动分类或判断重复物料。</li><li>不创建物料草稿，也不生成正式编码。</li></ul></MaterialImportDialog> : null}
    ...
    {dialog === "CANCEL" ? <MaterialImportDialog title="取消数据归一化" danger busy={busy} primaryLabel="确认取消" onClose={() => setDialog(null)} onPrimary={() => void execute("CANCEL", { expected_version: latest?.expected_version })}><p>后台任务可能短暂继续，但取消胜出后不会发布该 Attempt 的暂存结果。重跑取消胜出时保留上一次已发布结果。</p></MaterialImportDialog> : null}
  </section>;
}
