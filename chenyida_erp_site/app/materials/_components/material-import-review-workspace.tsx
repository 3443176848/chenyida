...
export function MaterialImportReviewWorkspace({ batchId, batch, permissions, csrfToken }: {
  ...
  if (!session) return <section className="min-review" data-review-workspace><h2>数据归一化人工复核</h2>
    <p>人工复核固定引用一个已发布的数据归一化运行；不会修改解析器原始行、字段映射快照或机器候选。</p>
  ...
  return <section className="min-review" data-review-workspace>
    <header><h2>{batch.batch_no} · 数据归一化人工复核 · v{session.review_version}</h2><p>运行 {session.normalization_run_id} / v{session.normalization_run_version} · {statusLabel(session.status)} · CAS {session.expected_version}</p></header>
    ...
    <div className="min-metrics"><span>待复核 <strong>{session.pending_rows}</strong></span><span>已复核 <strong>{session.reviewed_rows}</strong></span><span>排除 <strong>{session.excluded_rows}</strong></span><span>绑定已生效物料 <strong>{session.bind_existing_rows}</strong></span><span>待建草稿 <strong>{session.create_draft_rows}</strong></span><span>失败 <strong>{session.failed_rows}</strong></span></div>
    <details><summary>复核历史版本（只读）</summary><ul>{history.map((item) => <li key={item.review_session_id}>v{item.review_version} · {statusLabel(item.status)} · 运行 {item.normalization_run_id}{item.review_session_id !== session.review_session_id ? "（历史）" : "（当前）"}</li>)}</ul>{["FINALIZED", "FINALIZE_FAILED"].includes(session.status) && canCreate ? <button disabled={busy} onClick={() => void createSession(session.review_session_id)}>基于同一运行创建新复核版本</button> : null}</details>
    <div className="min-filters">
      <label>行状态<select value={rowStatus} onChange={(e) => { setAfterId(0); setRowStatus(e.target.value); }}><option value="">全部</option><option value="PENDING">待处理</option><option value="REVIEWED">已复核</option><option value="COMPLETED">已完成</option><option value="FAILED">失败</option></select></label>
      <label>最终决定<select value={disposition} onChange={(e) => { setAfterId(0); setDisposition(e.target.value); }}><option value="">全部</option><option value="PENDING">待处理</option><option value="KEEP">保留</option><option value="EXCLUDE">排除</option><option value="BIND_EXISTING">绑定已有物料</option><option value="CREATE_DRAFT">创建物料草稿</option></select></label>
      <label>问题<select value={issueLevel} onChange={(e) => { setAfterId(0); setIssueLevel(e.target.value); }}><option value="">全部</option><option value="ERROR">错误</option><option value="WARNING">警告</option></select></label>
    </div>
    <div className="min-table-wrap"><table><caption>服务端分页复核行</caption><thead><tr><th>选择</th><th>来源行</th><th>归一化状态</th><th>复核状态</th><th>决定</th><th>问题</th><th>结果</th><th>操作</th></tr></thead><tbody>{rows.map((row) => <tr key={row.review_row_id}>
      ...
      <td>{row.source_row_number}</td><td>{statusLabel(row.normalization_row_status)}</td><td>{statusLabel(row.row_status)}</td><td>{statusLabel(row.disposition)}</td><td>{row.error_count} 错误 / {row.warning_count} 警告</td>
      <td>{row.material_draft_id ? <Link href={`/materials/${row.material_draft_id}`}>草稿 #{row.material_draft_id}</Link> : row.existing_material_id ? <Link href={`/materials/${row.existing_material_id}`}>已生效 #{row.existing_material_id}</Link> : row.failure_message_safe || "—"}</td>
    ...
    <section><h3>最终提交</h3><p>“保留”只是暂存待决定，最终提交前必须改为精确绑定已生效物料或创建物料草稿；未解决错误和未确认警告会阻止提交。</p>
    ...
    {detail ? <div className="min-drawer-backdrop"><aside className="min-drawer" role="dialog" aria-modal="true" aria-labelledby="mir-row-title"><header><h3 id="mir-row-title">来源行 {detail.row.source_row_number} · 三层值复核</h3><button onClick={closeRow}>关闭</button></header><div className="min-drawer-body">
      ...
      <section><h4>问题与确认</h4>{detail.normalization_issues.map((issue) => <article key={issue.id}><strong>{statusLabel(issue.issue_level)} · {issue.issue_code}</strong><p>{issue.safe_message}</p><p>处理状态：{statusLabel(resolutionByIssue.get(issue.id) ?? "UNRESOLVED")}</p>{issue.issue_level === "WARNING" && canEdit && !readOnly ? <button onClick={() => void mutateRow("issue", { resolution_status: "WARNING_ACKNOWLEDGED", resolution_code: "WARNING_REVIEWED", comment }, issue.id)}>确认警告</button> : null}{issue.issue_level === "ERROR" && canEdit && !readOnly ? <button onClick={() => void mutateRow("issue", { resolution_status: "RESOLVED_BY_OVERRIDE", resolution_code: "OVERRIDE_REVALIDATED", comment }, issue.id)}>标记由人工覆盖解决</button> : null}</article>)}
      {detail.review_validation_issues.map((issue) => <p role="alert" key={issue.id}>{statusLabel(issue.issue_level)} · {issue.issue_code} · {issue.safe_message}</p>)}</section>
      ...
      {canDecide && !readOnly ? <section><h4>最终决定</h4><label>原因或备注<textarea value={comment} maxLength={1000} onChange={(e) => setComment(e.target.value)} /></label><div className="min-actions"><button onClick={() => void mutateRow("decision", { disposition: "KEEP", decision_comment: comment })}>保留待决定</button><button onClick={() => void mutateRow("decision", { disposition: "EXCLUDE", decision_reason_code: "USER_EXCLUDED", decision_comment: comment || "人工排除" })}>排除</button><button onClick={() => void mutateRow("decision", { disposition: "CREATE_DRAFT", decision_comment: comment })}>最终提交时创建 Material Draft</button></div>
        <div className="min-filters"><label>精确搜索已生效物料<input value={materialQuery} onChange={(e) => setMaterialQuery(e.target.value)} /></label><button onClick={() => void searchMaterials()}>搜索</button></div>
        <ul>{materials.map((material) => <li key={material.id}><code>{material.internal_material_code}</code> · {material.standard_name} · {material.manufacturer_part_number || "无 MPN"} <button onClick={() => void mutateRow("decision", { disposition: "BIND_EXISTING", existing_material_id: material.id, decision_comment: comment })}>明确绑定此已生效物料</button></li>)}</ul>
      </section> : null}
    </div></aside></div> : null}
    <p>本页面没有创建已生效物料、自动审批、自动编码、自动匹配或 AI 修正入口。新建结果只会进入现有物料草稿流程。</p>
  </section>;
}
