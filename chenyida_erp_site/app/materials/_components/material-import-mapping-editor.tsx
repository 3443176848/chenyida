...
export function MaterialImportMappingPreview({ rows }: { rows: { row_number: number; values: { target_namespace: string; target_code: string; source_column_index: number | null; status: string; raw_value: unknown; candidate_value: unknown; issues: { code: string; message: string }[] }[] }[] }) {
  return <section className="mi-mapping-preview"><h3>Mapping 有界预览</h3><p>只显示原始样本、目标字段、默认值候选与安全问题；不执行清洗、分类、匹配、AI 或草稿创建。</p><div className="mm-table-wrap"><table><thead><tr><th>行</th><th>目标</th><th>原始值</th><th>候选值</th><th>状态与问题</th></tr></thead><tbody>{rows.flatMap((row) => row.values.map((value, index) => <tr key={`${row.row_number}-${value.target_namespace}-${value.target_code}-${index}`}><td>{row.row_number}</td><td>{value.target_namespace}.{value.target_code}</td><td>{String(value.raw_value ?? "—")}</td><td>{String(value.candidate_value ?? "—")}</td><td>{statusLabel(value.status)}{value.issues.length ? `：${value.issues.map((item) => item.message).join("；")}` : ""}</td></tr>))}</tbody></table></div></section>;
}
