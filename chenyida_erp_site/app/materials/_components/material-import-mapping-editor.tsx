...
export function MaterialImportMappingEditor({ columnCount, rows, headerMode, headerRow, targets, catalogQuery, catalogHasMore, items, dirty, readOnly, busy, previewValid, onCatalogQuery, onCatalogMore, onItems, onReset, onSave, onPreview, onConfirm }: {
  ...
  const changeTarget = (index: number, target: ImportMappingTarget | null) => {
    ...
    const next: ImportMappingItem = { source_column_index: index, source_header: sourceHeader(rows, index, headerMode, headerRow), target_namespace: target.target_namespace, target_code: target.target_code, mapping_mode: target.target_namespace === "ignore" ? "IGNORE" : "SOURCE", required: target.required_for_confirm, display_order: index };
