...
import { statusLabel as erpStatusLabel } from "../../../public/erp/status-localization.js";
import {
...
function AuditHistory({ data, loading, copied, onCopy }: { data: AuditItem[]; loading: boolean; copied: string; onCopy: (id: string) => void }) {
  ...
  return <div className="mm-table-wrap mm-history-table-wrap" aria-busy={loading}>{loading ? <div className="mm-updating" role="status">正在加载审计历史…</div> : null}<table className="mm-history-table mm-audit-table"><caption>物料操作审计历史</caption><thead><tr><th>时间</th><th>操作者</th><th>动作</th><th>结果</th><th>原版本</th><th>新版本</th><th>错误代码</th><th>请求编号</th><th>操作编号</th></tr></thead><tbody>{data.map((item) => <tr className="mm-history-row" key={item.id}><td>{formatShanghaiDate(item.created_at, true)}</td><td>{actor(item.username)}</td><td>{erpStatusLabel(item.action)}<small>{displayValue(item.route_code)}</small></td><td>{erpStatusLabel(item.result)}</td><td>{valueVersion(item.old_version)}</td><td>{valueVersion(item.new_version)}</td><td>{displayValue(item.error_code)}</td><td><span className="mm-tracking">{compactTrackingId(item.request_id)}</span>{item.request_id ? <button className="mm-copy" onClick={() => onCopy(item.request_id || "")}>{copied === item.request_id ? "已复制" : "复制"}</button> : null}</td><td><span className="mm-tracking">{compactTrackingId(item.operation_id)}</span>{item.operation_id ? <button className="mm-copy" onClick={() => onCopy(item.operation_id || "")}>{copied === item.operation_id ? "已复制" : "复制"}</button> : null}</td></tr>)}</tbody></table><p className="mm-muted">审计页只展示安全元数据，不显示请求正文、凭证或内部错误堆栈。</p></div>;
}
