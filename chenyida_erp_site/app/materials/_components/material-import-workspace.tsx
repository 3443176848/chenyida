...
import { MaterialStandardizationPreview } from "./material-standardization-preview";
...
type MappingAggregate = { id: number; batch_id: number; parse_run_id: number; selected_sheet_index: number; header_mode: "SINGLE_ROW" | "NO_HEADER"; header_row_number: number | null; header_start_row_number?: number | null; data_start_row_number?: number | null; structure_confidence?: number; structure_status?: string; supplier_profile_id?: number | null; mapping_status: string; mapping_version: number; metadata_digest: string; items: ImportMappingItem[] };
...
export function MaterialImportWorkspace({ batchId }: { batchId: number }) {
  ...
  const initialLoad = useCallback(async () => {
    ...
    try {
      ...
      if (["PARSED", "AWAITING_MAPPING"].includes(detail.batch.status) || (detail.batch.status === "MAPPING_CONFIRMED" && ["standardize", "sheet", "mapping", "confirmed"].includes(requestedView || "")) || (["QUEUED_FOR_NORMALIZATION", "NORMALIZING", "NORMALIZED"].includes(detail.batch.status) && ["standardize", "confirmed"].includes(requestedView || ""))) sheets = await readSheets();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (!batch || !sheetsData || query.sheet === null || query.view === "standardize") { setRowsData(null); return; }
  ...
  }, [batch, batchId, handleError, query.row_page, query.row_page_size, query.sheet, query.view, sheetsData]);

  const loadMapping = useCallback(async () => {
    if (!batch || query.view === "standardize" || !["AWAITING_MAPPING", "MAPPING_CONFIRMED", "QUEUED_FOR_NORMALIZATION", "NORMALIZING", "NORMALIZED"].includes(batch.status) || (batch.status === "MAPPING_CONFIRMED" && query.view === "normalize") || (!["AWAITING_MAPPING", "MAPPING_CONFIRMED"].includes(batch.status) && query.view !== "confirmed")) return;
    try {
  ...
  return <section className="mi-workspace"><div className="mm-breadcrumb"><Link href="/materials">物料主数据</Link><span>/</span><Link href="/materials/imports">导入批次</Link><span>/</span><span>{batch.batch_no}</span></div>
    ...
    {!statusPage && (["PARSED", "AWAITING_MAPPING", "MAPPING_CONFIRMED"].includes(batch.status) || (["QUEUED_FOR_NORMALIZATION", "NORMALIZING", "NORMALIZED"].includes(batch.status) && ["standardize", "confirmed"].includes(query.view))) && sheetsData ? <>
      {batch.status === "PARSED" ? <div className={`mi-preparation mi-preparation-${sheetsData.mapping_preparation_status.toLowerCase()}`}>{sheetsData.mapping_preparation_status === "FAILED" ? "解析结果已发布，但字段映射准备失败" : `解析结果已发布，字段映射准备状态：${statusLabel(sheetsData.mapping_preparation_status)}`}</div> : null}
      {query.view === "standardize" ? <MaterialStandardizationPreview batchId={batchId} onAdvancedMapping={() => navigate({ view: batch.status === "AWAITING_MAPPING" ? (canMap ? "mapping" : "sheet") : "confirmed" })} /> : null}
      {query.view !== "standardize" ? <section className="mi-sheet-workspace"><MaterialImportSheetSelector sheets={sheetsData.sheets} selected={query.sheet} onSelect={(sheet) => navigate({ sheet: sheet.sheet_index, row_page: 1, view: "sheet" })} />
      ...
      {query.view !== "standardize" && mapping ? <section className="mi-mapping-history" aria-label="Mapping 版本与复用">
        <h3>Mapping 版本与复用</h3>
        <p>当前 v{mapping.mapping_version} · {statusLabel(mapping.mapping_status)}。已确认版本为不可变快照；复用只会复制到草稿，始终需要重新预览和确认。</p>
        {mappingVersions.length ? <ul>{mappingVersions.map((version) => <li key={version.id}>v{version.mapping_version} · {statusLabel(version.mapping_status)}{version.confirmed_by ? ` · ${version.confirmed_by}` : ""}{version.confirmed_at ? ` · ${formatShanghaiDate(version.confirmed_at, true)}` : ""}{version.stale_reason ? ` · ${statusLabel(version.stale_reason)}` : ""}</li>)}</ul> : <p>暂无版本历史。</p>}
