...
export function MaterialImportUploadFlow({ existingBatch }: { existingBatch?: MaterialImportBatch }) {
  ...
  const createdBatchRef = useRef<MaterialImportBatch | null>(existingBatch ?? null);
  ...
  const choose = (selected: File | null) => {
    if (operationRef.current?.state === "RESULT_UNKNOWN") return;
    hashController.reset(); operationRef.current = null; setUnresolved(false); createdBatchRef.current = existingBatch ?? null; fileRef.current = selected; setFile(selected); setProgress(null); setError(null); setDuplicateBatch(null); setDuplicateConfirm(false);
  ...
  const createBatch = async (retryOfBatchId: number | null): Promise<MaterialImportBatch> => {
    ...
    operationRef.current = { ...operation, state: "COMPLETED" }; createdBatchRef.current = response.data; return response.data;
  };
  ...
  const upload = async (batch: MaterialImportBatch, action: UploadAction, reuse?: ImportWriteOperation): Promise<UploadResponse> => {
    ...
    const response = await materialMultipart<UploadResponse>(operation.endpoint, {
      ...
      headers: {
        "X-Expected-Version": batch.current_version,
        "X-File-Name": encodeURIComponent(preflight.filename),
        "X-File-Mime": selected.type || "application/octet-stream",
        "X-File-SHA256": hash.sha256,
        "X-Duplicate-Action": action,
        "X-File-Size": selected.size,
      },
  ...
  const execute = async (action: UploadAction, retryOfBatchId: number | null = null) => {
    if (!fileRef.current || hash.state !== "COMPLETED" || busy || operationRef.current?.state === "RESULT_UNKNOWN") return; setBusy(true); setError(null);
    try {
      const retained = action === "REJECT" && retryOfBatchId === null ? createdBatchRef.current : null;
      const batch = retained ?? await createBatch(retryOfBatchId);
      const reuse = operationRef.current?.type === "UPLOAD" ? operationRef.current : undefined;
      const response = await upload(batch, action, reuse); const uploadedBatchId = response.data.batch.id;
      ...
      if (normalized.code === "IMPORT_FILE_DUPLICATE") {
        ...
        const unknown = normalized.resultUnknown;
        if (operationRef.current) operationRef.current = { ...operationRef.current, state: unknown ? "RESULT_UNKNOWN" : "COMPLETED" };
  ...
  const recoverUnknown = async () => {
    ...
    try {
      if (operation.type === "CREATE") {
        const response = await api<BatchResponse>(operation.endpoint, { method: "POST", body: JSON.stringify(operation.payload), protectedWrite: { idempotencyKey: operation.key, csrfToken: session.csrf_token || "" } });
        createdBatchRef.current = response.data;
        operationRef.current = null;
        const retryPayload = operation.payload as { retry_of_batch_id?: number | null };
        const uploaded = await upload(response.data, retryPayload.retry_of_batch_id == null ? "REJECT" : "ALLOW_DUPLICATE");
      ...
      const normalized = normalizeImportUiError(reason);
      if (operationRef.current) operationRef.current = { ...operationRef.current, state: normalized.resultUnknown ? "RESULT_UNKNOWN" : "COMPLETED" };
      setUnresolved(normalized.resultUnknown);
  ...
  return <section className="mi-upload-flow">
    <div className="mi-upload-card"><label htmlFor="material-import-file">选择一个供应商物料文件</label><input id="material-import-file" type="file" accept=".xlsx,.xls,.csv" disabled={busy || unresolved} onChange={(event) => choose(event.currentTarget.files?.length === 1 ? event.currentTarget.files[0] : null)} />
      ...
      <button className="mi-primary" disabled={busy || unresolved || hash.state !== "COMPLETED"} onClick={() => setConfirm(true)}>{existingBatch ? "确认并上传到当前批次" : "确认并创建批次"}</button>
    </div>
    ...
    {confirm && !unresolved ? <MaterialImportDialog title={existingBatch ? "确认上传文件" : "确认创建导入批次"} busy={busy} primaryLabel={existingBatch ? "上传文件" : "创建并上传"} onClose={() => setConfirm(false)} onPrimary={() => void execute("REJECT")}><p>将先创建或使用一个批次，再以默认 REJECT 策略上传。文件安全和内容合法性仍由服务端判断。</p></MaterialImportDialog> : null}
